...
require(['jquery', 'sakai/sakai.api.core'], function($, sakai) {
    ...
    sakai_global.htmlblock = function(tuid, showSettings, widgetData) {
        ...
        var loadTinyMCE = function() {
            ...
            // SAKIII-6151 We need to update the default tinyMCE settings if the editor needs to be reinitialised,
            // because only the last tinymce editor settings are stored which causes problems with multiple instances
            $(window).on('getTinyMCESettings.htmlblock.sakai', function(ev, widgetId) {
                if (widgetId === tuid) {
                    $(window).trigger('setTinyMCESettings.htmlblock.sakai', tinyMCESettings);
                }
            });
