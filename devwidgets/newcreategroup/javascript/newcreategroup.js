    ...
    sakai_global.newcreategroup = function(tuid, showSettings, widgetData){
        ...
        var doCreateSimpleGroup = function(){
            ...
            sakai.api.Groups.createGroup(groupid, grouptitle, groupdescription, sakai.data.me, currentTemplate, widgetData.category, function(success, nameTaken){
                if (success) {
                    ...
                    // TODO create public activity (ad602@caret.cam.ac.uk)
                    var activityData = {
                        "sakai:activity-privacy": visible
                    };
                    sakai.api.Activity.createActivity("/group/" + groupid, groupid, "default", activityData);
        ...
        var createGroupDocs = function(groupid, currentTemplate){
            replaceTemplateParameters({
                "groupid": groupid
            }, groupid, currentTemplate, function(groupid, currentTemplate){
        ...
        var setSakaiDocPermissions = function(groupid, currentTemplate, callback){
            ...
            for (var doc in currentTemplate.structure) {
                ...
                if ($.inArray("anonymous", definition._view) !== -1 && $newcreategroupCanBeFoundIn.val() === "public") {
                    ...
                    if ($.inArray("everyone", definition._view) !== -1 && ($newcreategroupCanBeFoundIn.val() === "public" || $newcreategroupCanBeFoundIn.val() === "logged-in-only")) {
                        permission = "everyone";
                    }
