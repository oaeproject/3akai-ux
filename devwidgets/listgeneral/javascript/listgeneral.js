    ...
    addPage = function(tuid, pageNumber, searchQuery) {
        ...
        var handleSuccess = function(rawData) {
            ...
            for (var j = 0, jl = rawData.results.length; j < jl; j++) {
                ...
                // Content
                if (result["mimeType"] && result["jcr:primaryType"] === "nt:file") {

                    // Add type property
                    result.listgeneral.resultType = "content";

                    // Get mimetype icon and human readable mimetype description
                    if (sakai.config.MimeTypes[result["mimeType"]]) {
                        result.listgeneral.avatar = sakai.config.MimeTypes[result["mimeType"]].URL;
                        result.listgeneral.mimeTypeDescripton = sakai.api.i18n.General.getValueForKey(sakai.config.MimeTypes[result["mimeType"]].description);
                    } else {
