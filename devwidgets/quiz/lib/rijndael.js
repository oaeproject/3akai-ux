...
var RND = function() {
    ...
    return {
        ...
        rijndaelEncrypt: function(PT, Y, M) {
            var EY, i, abk;
            var bpb = BB / 8;
            var ct;
            if (!PT || !Y) return;
        ...
        rijndaelDecrypt: function(CT, Y, M) {
            var EY;
            var bpb = BB / 8;
            var pt = [];
            var abk;
            ...
            for (bk = (CT.length / bpb) - 1; bk > 0; bk--) {
                ...
                if (M == "CBC") for (vari = 0; i < bpb; i++) pt[(bk - 1) * bpb + i] = abk[i] ^ CT[(bk - 1) * bpb + i];
