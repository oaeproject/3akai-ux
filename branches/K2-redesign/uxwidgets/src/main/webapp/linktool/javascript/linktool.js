...
sakai.linktool = function(tuid, placement, showSettings){
	...
	Config.URL.SDATA_FETCH_URL = "/sdata/f/__PLACEMENT__/__TUID__/__NAME__";
	...
	var renderIframeSettings = function(complete){
		if(complete){
			...
			$(linktoolSettingsPreviewFrame).attr("height", json.height + "px");
			$(linktoolSettingsPreviewFrame).attr("width", json.width + json.width_unit);
	...
	var saveLinkTool = function() {		
		if (json.url !== "") {
			...
			sdata.widgets.WidgetPreference.save("/sdata/f/" + placement + "/" + tuid, "linktool", str, savedDataToJCR);
		}
	};
