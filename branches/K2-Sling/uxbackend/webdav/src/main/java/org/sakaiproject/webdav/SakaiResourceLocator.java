 ...
 * not here.
 */
public class SakaiResourceLocator implements DavResourceLocator {
  ...
  private final SakaiLocatorFactory factory;

  private final String href;

  public SakaiResourceLocator(String prefix, String workspaceName,
      String resourcePath, SakaiLocatorFactory factory) {
  ...
  /**
   * Returns the resource path which always starts with the workspace path, if a
   * workspace resource exists. For the top most resource (request handle '/'),
   ...
   * represents the '/' request handle, which does not contain a workspace path.
   * 
   ...
   * 
   * @param locator
   * @return true if the workspace name obtained from the given locator refers
   *         to the same workspace as the workspace name of this locator.
  ...
  /**
   * Returns true if the specified string equals to this workspace name or if
   * both names are null.
   * 
   * @param workspaceName
  ...
  /**
   * Returns an 'href' consisting of prefix and resource path (which starts with
   * the workspace path). It assures a trailing '/' in case the href is used for
   * collection. Note, that the resource path is {@link Text#escapePath(String)
   * escaped}.
   * 
   * @param isCollection
  ...
  /**
   * Uses {@link #getResourcePath()} to build the repository path.
   * 
  ...
  /**
   * Computes the hash code from the href, that is built from the prefix, the
   * workspace name and the resource path all of them representing final
   * instance fields.
   * 
  ...
  /**
   * Returns true, if the given object is a <code>SlingResourceLocator</code>
  ...
  public boolean equals(Object obj) {
    ...
    if (obj instanceof SakaiResourceLocator) {
      SakaiResourceLocator other = (SakaiResourceLocator) obj;
