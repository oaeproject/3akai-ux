 ...
 *
 */
public class JcrChatMessagingService implements ChatMessagingService {
  ...
  @Inject
  public JcrChatMessagingService(JCRNodeFactoryService jcrNodeFactory,
      ChatMessageConverter chatMessageConverter, Injector injector,
   ...
   * {@inheritDoc}
   *
   * @see org.sakaiproject.kernel.api.messaging.ChatMessagingService#send(javax.jms.Message)
   */
  public void send(ChatMessage msg) throws MessagingException {
    // establish the send date and add it to the message
    String date = DateUtils.rfc2822();
    msg.setHeader(ChatMessage.Field.DATE, date);

    try {
      // convert message to the storage format (json)
      String json = chatMessageConverter.toString(msg);
      ...
      /* TODO put the chats inside the KernelContstants */
      String path = userFactory.getUserPrivatePath(
    		  msg.getFrom()) + "chats/" + KernelConstants.OUTBOX + "/";
   ...
   * {@inheritDoc}
   *
   * @see org.sakaiproject.kernel.api.messaging.ChatMessagingService#createMessage()
   */
  public ChatMessage createMessage() {
    ChatMessage m = injector.getInstance(ChatMessage.class);
    return m;
  }
}