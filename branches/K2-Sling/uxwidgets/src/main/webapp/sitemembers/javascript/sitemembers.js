...
sakai.sitemembers = function(tuid, placement, showSettings){
	...
	var displayPublications = function(user) {
		var json = {'publications' : []};
		if (user.academic) {
			json = {'publications' : user.academic};
		}
	...
	var displayAboutMe = function(user, what, template) {
		...
		if (user.aboutme) {
			...
			else if (what === "aboutme" && user.aboutme.aboutme) {
				json.aboutme = user.aboutme.aboutme;
			}
			else if (what === "personalinterests" && user.aboutme.personalinterests) {
				json.personalinterests = user.aboutme.personalinterests[0];
			}
			else if (what === "academicinterests" && user.aboutme.academicinterests) {
	...
	var checkIfContact = function(user) {
		for(var i = 0,j = contacts.length;i<j;i++) {
			if (user.userid === contacts[i].target) { return true; }
		}
		return false;
	};
	...
	var prepareSiteMembers = function(members) {
		...
		for (var i=0,j=members.length;i<j;i++) {
			...
			for (var di=0,dj=widgetSettings.data.length;di<dj;di++) {
				...
				if (toShow === "aboutme") { o.value = displayAboutMe(user, "aboutme", sitemembers_normal_data_aboutme); } 
				else if (toShow === "hobbies") {  o.value = displayAboutMe(user, "hobbies", sitemembers_normal_data_hobbies); }
				else if (toShow === "academicinterests") {  o.value = displayAboutMe(user, "academicinterests", sitemembers_normal_data_academicinterests); }
				else if (toShow === "personalinterests") {  o.value = displayAboutMe(user, "personalinterests", sitemembers_normal_data_personalinterests); }
				else if (toShow === "degrees") { o.value = displayDegrees(user); }
				else if (toShow === "talks") { o.value = displayTalks(user); }
				else if (toShow === "publications") { o.value = displayPublications(user); }
				else if (toShow === "profexperience") { o.value = displayProfExperience(user); }
	...
	$(sitemembers_normal_result_addtocontacts, rootel).live("click", function(e, ui) {
		var id = e.target.id.split('_');
		id = id[id.length - 1];
		sakai.addtocontacts.initialise(id, function() { renderAndAppendSiteMembers(allMembers) });
	});
