...
sakai.pollwow = function(tuid, placement, showSettings){
	...
	var checkDates = function(){
		...
		startdate = convertDate(json.pollwow.startdate, json.pollwow.starthour, json.pollwow.startmin);
		stopdate = convertDate(json.pollwow.stopdate, json.pollwow.stophour, json.pollwow.stopmin);
		if(startdate > today){
	...
	var processPoll = function(preview){
		if(preview){
			...
			for(var i = 0; i < json.pollwow.temp.votes.length; i++){
				json.pollwow.temp.processedVotes[i] = {};
				json.pollwow.temp.processedVotes[i].percentage = Math.round(json.pollwow.temp.votes[i] / totalvotescount * 100);
			}
			json.pollwow.temp.question = "How is the weather in England?";
		}else{
			totalvotes = json.pollwow.votes.length;
			totalvotescount = 0;
			for(var i = 0; i < json.pollwow.votes.length; i++){
				totalvotescount += json.pollwow.votes[i];
			}
			json.pollwow.processedVotes = [];
			
			if(totalvotescount == 0){
				for(var i = 0; i < json.pollwow.votes.length; i++){
					json.pollwow.processedVotes[i] = {};
					json.pollwow.processedVotes[i].percentage = 0;
				}
			}else{
				for(var i = 0; i < json.pollwow.votes.length; i++){
					json.pollwow.processedVotes[i] = {};
					json.pollwow.processedVotes[i].percentage = Math.round(json.pollwow.votes[i] / totalvotescount * 100);
				}
			}
			...
			if(preview){
				...
				switch(json.pollwow.poll_type){
					...
					case 1:
						...
						height = 20 + (json.pollwow.processedVotes.length*30);
						...
						for(var i = json.pollwow.processedVotes.length-1; i >=0 ; i--){
							json.pollwow.image += json.pollwow.processedVotes[i].percentage + "%|"
						}
						json.pollwow.image = json.pollwow.image.substring(0,json.pollwow.image.length - 1);
					...
					case 3:
						for(var i = 0; i < json.pollwow.processedVotes.length; i++){
							json.pollwow.options[i].percentage = json.pollwow.processedVotes[i].percentage + "%";
							json.pollwow.options[i].color = colors[i];
						}
					break;
				}
	...
	var saveOptions = function(){
		tempArray = [];
		for(i = 0; i < $("#pollwow_options", rootel).children().length-1; i++){ // -2 because we don't need the buttonbar
			tempValue = $("#pollwow_question_input"+i, rootel).val();
	...
	/**
	 * Remove option
	 * @param {String} id Id of the option that needs to be removed
	 */
	...
	var addCurrentToAllPollwow = function(){
		...
		jsonToString = sdata.JSON.stringify(jsonAll);
	...
	var savePollwowToDatabase = function(functionOnComplete, saveAll){
		...
		jsonToString = sdata.JSON.stringify(json);
		...
		if(me){
			if(!json.pollwow.users.contains(me.preferences.uuid)){
				if($('input[name=pollwow_question_view_options]:checked').length == 0){
					...
					json.pollwow.votes[parseInt($('input[name=pollwow_question_view_options]:checked').val())] +=1;
					...
					for(var i = 0; i < $('input[name=pollwow_question_view_options]:checked').length; i++){
						json.pollwow.votes[parseInt($("#" + $('input[name=pollwow_question_view_options]:checked')[i].id).val())] += 1;
					}
	...
	var addBinding = function(){
		...
		$("#pollwow_widget_insert", rootel).bind("click",function(e,ui){
			id = $(".pollwow_insert_existing_selected", rootel)[0].id;
			if(id != undefined){
				id = id.replace("pollwow_insert_existing_item", "");
				
				pollwowElement = getInsertElement(id);
				if(pollwowElement){
	...
	var saveInputValue = function(field){
		tempElement = $("#pollwow_" + field, rootel);
		/** Check if the type of the field is checkbox or text (or textarea) */
		if(tempElement.attr("type") == "checkbox") {
			(tempElement.attr("checked")) ? json.pollwow[field] = 1 : json.pollwow[field] = 0;
		}else{
