...
Timeplot.DefaultValueGeometry.prototype = {
    ...
    toScreen: function(value) {
        if (this._canvas && this._maxValue) {
            ...
            if (this._invertGraph) {
                return this._canvas.height - (this._canvas.height * (this._map.direct(v)) / this._mappedRange);
            } else {
    ...
    fromScreen: function(y) {
        if (this._canvas) {
            if (this._invertGraph) y = this._canvas.height - y;
    ...
    paint: function() {
        if (this._timeplot) {
            ...
            // paint grid
            if (this._gridColor && this._count > 0) {        
    ...
    _calculateGrid: function() {
        ...
        while (true) {
            var dy = this.toScreen(this._invertGraph ? this._maxValue - inc : this._minValue + inc);
            while (dy < this._gridSpacing) {
                inc += unit;
                dy = this.toScreen(this._invertGraph ? this._maxValue - inc : this._minValue + inc);
            }
        ...
        if (this._minValue >= 0) {
            while (this._invertGraph ? y > 0 : y < this._canvas.height) {
                if (y > 0) {
            ...
            while (this._invertGraph ? y < this._canvas.height : y > 0) {
...
// Inverse value geometry
Timeplot.InverseValueGeometry = function(params) {
    Timeplot.DefaultValueGeometry.apply(this, arguments);
    this._invertGraph = true;
}

Object.extend(Timeplot.InverseValueGeometry.prototype, Timeplot.DefaultValueGeometry.prototype);
...
Timeplot.DefaultTimeGeometry = function(params) {
    ...
    this._map = this._linMap;
    this._labeler = this._labeller ? this._labeller : this._unit.createLabeller(this._locale, this._timeZone);
            ...
            if (this._gridColor) {        
                ...
                for (var i = 0; i < this._grid.length; i++) {
                    ...
                    if (this._axisLabelsPlacement == "top") {
                        var div = this._timeplot.putText(this._id + "-" + i, this._reverseTime ? tick.label + " Ma" : tick.label,"timeplot-grid-label",{
                            left: x - 16,
                            top: 2,
                            visibility: "hidden"
                        });
                    ...
                    if (x - 12 + div.clientWidth < this._canvas.width + 10) {
                        div.style.visibility = "visible"; // avoid the labels that would overflow
                    }
		    this._labels.push(div);
                    // draw separator
...
// Reverse time geometry
Timeplot.ReverseTimeGeometry = function(params) {
    Timeplot.DefaultTimeGeometry.apply(this, arguments);
    this._reverseTime = true;
};

Object.extend(Timeplot.ReverseTimeGeometry.prototype, Timeplot.DefaultTimeGeometry.prototype);

Timeplot.ReverseTimeGeometry.prototype.toScreen = function(time) {
    if (this._canvas && this._latestDate) {
        var t = this._latestDate.getTime() - time;
	return this._canvas.width * this._map.direct(t) / this._mappedPeriod;
    } else {
        return -50;
    }
...
Timeplot.ReverseTimeGeometry.prototype.fromScreen = function(x) {
    if (this._canvas) {
        return this._latestDate.getTime() - this._map.inverse(this._mappedPeriod * x / this._canvas.width);
    } else {
        return 0;
    }
};
