...
casper.test.begin('Widget - About Content', function(test) {
    ...
    var verifyAboutLinkElements = function(user1, link) {
        ...
        test.assertEval(function(created) {
            return $('#aboutcontent-modal .modal-body #aboutcontent-metadata-container time').text() === require('oae.api.l10n').transformDateTime(created);
    ...
    casper.start(configUtil().tenantUI, function() {
        ...
        casper.then(function() {
            // Create a file
            contentUtil().createFile(null, null, [user2.id], function(_fileURL) {
                var fileURL = configUtil().tenantUI + _fileURL;
                var contentId = _fileURL.split('/');
                contentId = 'c:test:' + contentId[contentId.length - 1];
                contentUtil().updateContent(contentId, {'description': 'Test file description'}, function() {

                    // Create a link
                    contentUtil().createLink('http://www.oaeproject.org', null, [user2.id], function(link) {
                        contentUtil().updateContent(link.id, {'description': 'Test link description', 'displayName': 'Test link title'}, function() {

                            // Create a collaborative document
                            collabDocUtil().createCollabDoc(null, [user2.id], function(collabdoc) {
                                contentUtil().updateContent(collabdoc.id, {'description': 'Test collabdoc description', 'displayName': 'Test collabdoc title'}, function() {
                                    // Log out from user 1
                                    ...
                                    casper.thenOpen(fileURL, function() {
                                    ...
                                    casper.thenOpen(configUtil().tenantUI + link.profilePath, function() {
                                        ...
                                        casper.waitForSelector('#content-clip-container .oae-clip-content > button', function() {
                                            ...
                                            casper.then(function() {
                                                verifyAboutLinkElements(user1, link);
                                            });
                                        });
                                    });
                                    ...
                                    casper.thenOpen(configUtil().tenantUI + collabdoc.profilePath, function() {
                                        ...
                                        casper.waitForSelector('#content-clip-container .oae-clip-content > button', function() {
                                            casper.then(openAboutContent);
                                            casper.then(function() {
                                                verifyAboutCollabdocElements(user1, collabdoc);
                                            });
                                        });
                                    });
