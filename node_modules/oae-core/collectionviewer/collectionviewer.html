...
<div id="collectionviewer_grid_or_list_template"><!--
    {if items && items.length}
        ...
        {macro listContent(i, sakai)}
            ...
            <li id="${i.id}" class="fl-container fl-fix s3d-search-result-content s3d-search-result{if i_index === "0"} s3d-search-result-first{/if} s3d-draggable-container" data-scope="content" data-entityid="${i["_path"]}" data-canshare="${canShare}" {if sakai.api.Content.Collections.isCollection(i)} data-collection="true" {/if}>
                {if !editMode}
                ...
                {else}
                    <span class="s3d-search-result-user-functions fl-force-left collectionviewer_remove_actions">
                        <input class="collectionviewer_check s3d-draggable-select" id="collectionviewer_check_${i.id}" type="checkbox" data-entityid="${i.id}" data-entityname="${i['sakai:pooled-content-file-name']|safeOutput}" data-canshare="${canShare}"/>
                ...
                <div class="s3d-search-result-right">
                    ...
                    {if i.tagsProcessed && i.tagsProcessed.length}
                        <span class="s3d-aural-text">__MSG__TAGS_FOR__ ${i["sakai:pooled-content-file-name"]|safeOutput}</span>
        ...
        <ul {if grid} class="s3d-search-results-grid collectionviewer_list_grid_container"{else} class="collectionviewer_list_grid_container {if editMode && isEditor} collectionviewer_editable{/if}"{/if}>
            {for i in items}
                {if i['sakai:pooled-content-file-name']}
                    ${listContent(i, sakai)}
                {/if}
    ...
    {else}
        <div class="s3d-no-results-container">
            ...
            <p>__MSG__THIS_COLLECTION_IS_EMPTY__ {if isEditor} __MSG__WHY_NOT__ <button type="button" class="s3d-link-button sakai_add_content_overlay">__MSG__ADD_SOME_ITEMS__</button> __MSG__TO_THIS_COLLECTION__{/if}</p>
        </div>
    {/if}
--></div>
...
<div id="collectionviewer_carousel_template"><!--
    ...
    <div id="collectionviewer_carousel" class="jcarousel-skin-tango s3d-bold">
                    ...
                    {for result in page}
                        {if jQuery.isPlainObject(result)}
                            <li class="collectionviewer_carousel_item" data-arr-index="${result_index}" data-page-index="${page_index}" data-item-id="${result._path}" title="${result['sakai:pooled-content-file-name']|safeOutput}">
                                ...
                                <div class="collectionviewer_carousel_thumbnail_container s3d-draggable-container" data-canshare="${canShare}" data-scope="content" data-entityid="${result._path}" {if sakai.api.Content.Collections.isCollection(result)} data-collection="true" {/if}>
                                    {if thumbnail}
        ...
        {else}
            <div id="collectionviewer_no_content">__MSG__THIS_COLLECTION_IS_EMPTY__ {if !sakai.data.me.user.anon && isEditor} __MSG__WHY_NOT__ <button type="button" class="s3d-link-button sakai_add_content_overlay">__MSG__ADD_SOME_ITEMS__</button> __MSG__TO_THIS_COLLECTION__{/if}</div>
        {/if}
    </div>
--></div>
...
<div id="collectionviewer_list_item_template"><!--
    ...
    <ul>
        <li class="s3d-search-result s3d-draggable-container" data-canshare="${canShare}" data-scope="content" data-entityid="${data["_path"]}" {if sakai.api.Content.Collections.isCollection(data)} data-collection="true" {/if}>
