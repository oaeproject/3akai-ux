...
casper.test.begin('Widget - Revisions', function(test) {
    ...
    var openRevisionsModal = function() {
        casper.waitForSelector(
            ...
            function() {
                ...
                test.assertExists(
                    '.oae-trigger-revisions',
                    'Revisions trigger exists',
                );
                casper.click('.oae-trigger-revisions');
                casper.waitUntilVisible('#revisions-modal', function() {
                    test.assertVisible(
                        '#revisions-modal',
                        'Revisions pane is showing after trigger',
                    );
                    casper.click(
                        '#content-clip-container .oae-clip-content > button',
                    );
    ...
    var verifyContentRevisionsElements = function() {
        ...
        test.assertExists(
            'ul#revisions-list',
            'The list of content revisions is present',
        );
        casper.waitForSelector('ul#revisions-list li', function() {
            // Verify that there is at least one item in the revisions list
            test.assertExists(
                'ul#revisions-list li',
                'There is at least one content revision present',
            );
            // Verify current revision cannot be restored
            test.assertNotVisible(
                'ul#revisions-list li:first-child .revisions-list-actions-restore',
                'The current content revision cannot be restored',
            );
            // Verify other revisions can be restored
            test.assertExists(
                'ul#revisions-list li:first-child + li .revisions-list-actions-restore',
                'All other content revisions can be restored',
            );
            // Verify that the revision can be downloaded
            test.assertExists(
                'ul#revisions-list li .revisions-list-actions-download',
                'The download button for each content revision is present',
            );
            // Verify preview for revision is shown
            test.assertExists(
                '#revisions-preview-large',
                'The content revision preview container is present',
            );
            test.assertExists(
                '#revisions-preview-large img',
                'The content revision preview image is present',
            );
        });
    };
        ...
        casper.waitForSelector(
            ...
            function() {
                test.assertDoesntExist(
                    ...
                    'The content revision can be restored',
                );
    ...
    var verifyCollabDocRevisionsElements = function() {
        ...
        test.assertExists(
            'ul#revisions-list',
            'The list of collabdoc revisions is present',
        );
        casper.waitForSelector('ul#revisions-list li', function() {
            // Verify that there is at least one item in the revisions list
            test.assertExists(
                'ul#revisions-list li',
                'There is at least one collabdoc revision present',
            );
            // Verify current revision can be restored
            test.assertExists(
                'ul#revisions-list li:first-child .revisions-list-actions-restore',
                'The current collabdoc revision can be restored',
            );
            // Verify preview for revision is shown
            casper.waitForSelector('.revisions-etherpad-container', function() {
                test.assertExists(
                    '.revisions-etherpad-container',
                    'The collabdoc revision preview is present',
                );
            });
        });
    };
    ...
    var verifyRestoringCollabDocRevision = function() {
        ...
        casper.waitForSelector(
            ...
            function() {
                test.assertDoesntExist(
                    '#oae-notification-container .alert.alert-error',
                    'The collabdoc revision can be restored',
                );
                casper.click('#oae-notification-container .close');
            },
        );
            ...
            contentUtil.createFile(null, null, null, null, null, null, function(
                ...
                contentUtil.createCollabDoc(
                    ...
                    function(err, collabdocProfile) {
                        ...
                        casper.then(function() {
                            casper.echo(
                                '# Verify collabdoc revisions elements',
                                'INFO',
                            );
                            openRevisionsModal();
                        ...
                        casper.then(function() {
                            casper.echo(
                                '# Verify restoring collabdoc revision',
                                'INFO',
                            );
