    ...
    return function(uid, showSettings) {
            ...
            getClickedRevision(ev, function(clickedRevision) {
                ...
                oae.api.util
                    ...
                    .render(
                        $template,
                        data,
                        $('#revisions-preview-large', $rootel),
                    );
                oae.api.util
                    .template()
                    .render(
                        $template,
                        data,
                        $('.revisions-list-preview', $(ev.currentTarget)),
                    );
            });
        };
        ...
        var restoreRevision = function(ev) {
            ...
            getClickedRevision(ev, function(clickedRevision) {
                ...
                oae.api.content.restoreRevision(
                    contentProfile.id,
                    clickedRevision.revisionId,
                    function(err, data) {
                        if (!err) {
                            // Hide the modal
                            ...
                            oae.api.util.notification(
                                oae.api.i18n.translate(
                                    '__MSG__REVISION_RESTORED__',
                                    'revisions',
                                ),
                                oae.api.i18n.translate(
                                    '__MSG__REVISION_RESTORE_SUCCESS__',
                                    'revisions',
                                ),
                            );

                            // Fetch the updated content profile
                            oae.api.content.getContent(
                                contentProfile.id,
                                function(err, updatedContentProfile) {
                                    // Refresh the content profile
                                    $(document).trigger('oae.revisions.done', [
                                        data,
                            ...
                            oae.api.util.notification(
                                oae.api.i18n.translate(
                                    '__MSG__REVISION_NOT_RESTORED__',
                                    'revisions',
                                ),
                                oae.api.i18n.translate(
                                    '__MSG__REVISION_RESTORE_FAIL__',
                                    'revisions',
                                ),
                                'error',
                            );
        ...
        var setUpRevisionsModal = function() {
            ...
            $rootel.on(
                'click',
                '.revisions-list-actions-restore',
                restoreRevision,
            );
        };
