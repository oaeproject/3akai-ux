...
define(['jquery', 'oae/api/oae.core'], function($, oae) {
    ...
    return function(uid) {
        ...
        var handleEmptyResultList = function() {
            oae.api.util.renderTemplate($('#searchall_noresults_template'), null, $('#searchall_results_container'));
        };

        var handleResultList = function(data) {
            $('#searchall_numberFound').text(data.total);
            if (!data.total) {
                return handleEmptyResultList();
            }

            $('#searchpeople_numberFound').text(data.total);
            oae.api.util.renderTemplate('#searchall_template', {
                'data': data
            }, $('.oae-list', $rootel));
        };
        ...
        var search = function(query) {
            ...
            var url = '/api/search/general/all?limit=12';
            if (query) {
                url += '&q=' + query;
            }
