...
define(['jquery', 'oae.core', 'jquery.history'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        /**
         * Initialize a new infinite scroll container that fetches a content library.
         * This will detect when a search is happening and will change the endpoint
         * accordingly.
         */
        var getContentLibrary = function() {
            ...
            $('#contentlibrary-list-options-search-query', $rootel).val(query);

            // Set up the list actions
            var initialContent = null;
            if ((widgetData.canAdd || widgetData.canManage) && !query) {
                initialContent = oae.api.util.template().render($('#contentlibrary-list-actions-template', $rootel));
            }
         ...
         * is logged in, we render the list of available action based on whether or not the
         * user can manage this library.
         */
        var setUpListOptionsHeader = function() {
            // Determine which actions should be available to the user viewing the library
            var actions = [];
            if (!oae.data.me.anon) {
                // If the user is loggedin, they have the option to share the items
                actions.push({
                    'icon': 'icon-share',
                    'label': oae.api.i18n.translate('__MSG__SHARE__', 'contentlibrary'),
                ...
                if (widgetData.canManage) {
                    ...
                    actions.push({
                        'icon': 'icon-trash',
                        'label': oae.api.i18n.translate('__MSG__DELETE__', 'contentlibrary'),
            ...
            oae.api.util.template().render($('#contentlibrary-header-template', $rootel), {'actions': actions}, $('#contentlibrary-header', $rootel));
        };
         ...
         */
        var search = function() {
            var query = $('#contentlibrary-list-options-search-query', $(this)).val();
        ...
        var addBinding = function() {
            // Set up search event
            $rootel.on('submit', '#contentlibrary-list-options .form-search', search);

            // Listen to the event that indicates that new content has been added.
            // In that case, we reload the content library.
            $(window).on('done.addcontent.oae', getContentLibrary);

            // Listen to History.js state changes and listen to the event that indicates
            // that a piece of content has been deleted, so the library can be reloaded
            $(window).on('statechange', function() {
                // Only re-load the list when the widget is currently visible
            ...
            $(window).on('oae.deleteresources.done', function() {
                ...
                if ($rootel.is(':visible')) {
                    getContentLibrary();
                    // Make sure the Select All checkbox is unchecked
                    $('#contentlibrary-list-options-selectall', $rootel).prop('checked', false);
                }
            });
        };

        addBinding();
        setUpListOptionsHeader();
        getContentLibrary();
