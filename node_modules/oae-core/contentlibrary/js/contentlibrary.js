...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var handleEmptyResultList = function() {
            oae.api.util.template().render($('#contentlibrary-noresults-template'), null, $('.oae-list', $rootel));
        };
        ...
        var getContentLibrary = function() {
            ...
            if (query) {
                url = '/api/search/content-library/' + widgetData.principalId;
            }
        ...
        /**
         * Extract the search query from the current URL and load the initial content library list.
         * This will only be executed when the page is loaded.
         */
        var initContentLibrary = function() {
            // We parse the URL fragment that's inside of the current History.js hash.
            // We expect to find the search query in the `?q=<query>` part of the URL.
            var initialState = $.url(History.getState().hash);
            var query = initialState.param().q;
            // Include the query into the History.js state. This requires extending the existing
            // state as to not to lose any of the current state information (page, etc.).
            var newState = $.extend({}, History.getState().data, {
                'query': query,
                '_': Math.random()
            });
            ...
            // event, which will take care of the content library listing. The random number is added to
        ...
        var addBinding = function() {
            // Set up search event
            $rootel.on('submit', '#contentlibrary-search-form', search);
            ...
            $(window).on('statechange', function() {
                // Only re-load the content library list when the widget is currently visible
