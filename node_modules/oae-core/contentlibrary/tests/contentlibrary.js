...
casper.test.begin('Widget - Content library', function(test) {
    ...
    casper.start(configUtil().tenantUI, function() {
        ...
        casper.thenOpen(configUtil().tenantUI + '/me/library', function() {
            casper.then(function() {
                casper.echo('# Verify content library updates after upload', 'GREEN_BAR');
                casper.waitForSelector('li[data-id="library"]', verifyContentLibraryUpdatesAfterUpload);
            });

            casper.then(function() {
                casper.echo('# Verify content library elements present', 'GREEN_BAR');
                verifyContentLibraryElements();
            });

            casper.then(function() {
                casper.echo('# Verify content library view modes', 'GREEN_BAR');
                verifyContentLibraryViewMode();
            });

            casper.then(function() {
                casper.echo('# Verify content library delete', 'GREEN_BAR');
                verifyContentLibraryDelete();
            });

            casper.then(function() {
                casper.echo('# Verify content library share', 'GREEN_BAR');
                verifyContentLibraryShare();
            });

            casper.then(function() {
                casper.echo('# Verify content library search', 'GREEN_BAR');
