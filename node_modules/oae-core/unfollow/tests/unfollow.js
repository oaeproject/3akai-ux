casper.test.comment('Widget - Unfollow');
...
var verifyUnfollowingModal = function() {
    casper.waitForSelector('li.oae-list-actions + li .oae-list-grid-item input[type="checkbox"]', function() {
        ...
        casper.test.assertExists('#network-widget .oae-list-options button.oae-trigger-unfollow:not([disabled])', 'Verify that the unfollow button is enabled after user selection');
        casper.click('#network-widget .oae-list-options button.oae-trigger-unfollow');
        casper.waitUntilVisible('#unfollow-modal', function() {
            casper.test.assertVisible('#unfollow-modal', 'Verify that the unfollow pane is showing after trigger');
            casper.click('li.oae-list-actions + li .oae-list-grid-item input[type="checkbox"]');
        });
    });
};
...
var verifyUnfollowingElements = function() {
    ...
    casper.test.assertExists('#unfollow-modal #unfollow-selected-container li', 'Verify that the users that will be unfollowed are listed');
    // Verify there is a confirmation message shown in the UI
    casper.test.assertExists('#unfollow-modal #unfollow-list-container h4', 'Verify that a confirmation message is shown before unfollowing the users');
    // Verify the cancel button is present
    casper.test.assertExists('#unfollow-modal .modal-footer button[data-dismiss="modal"]', 'Verify that the cancel button is present');
    // Verify the unfollow button is present
    casper.test.assertExists('#unfollow-modal .modal-footer button#unfollow-unfollow', 'Verify that the \'Unfollow\' button is present');
};
...
var verifyUnfollowingUser = function() {
    ...
    casper.waitForSelector('#oae-notification-container .alert', function() {
        casper.test.assertDoesntExist('#oae-notification-container .alert.alert-error', 'Verify a user can be unfollowed');
        // Verify that the user is removed from the list when unfollowed
        casper.test.assertDoesntExist('#network-widget #network-following .oae-list li:first-child + li', 'Verify an unfollowed user is removed from the following list');
    });
};
