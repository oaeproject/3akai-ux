...
casper.test.begin('Widget - Unfollow', function(test) {
    ...
    var verifyUnfollowingModal = function() {
        casper.waitForSelector(
            ...
            function() {
                ...
                test.assertExists(
                    '#network-widget .oae-list-header-actions button.oae-trigger-unfollow:not([disabled])',
                    'Verify that the unfollow button is enabled after user selection',
                );
                casper.click(
                    '#network-widget .oae-list-header-actions button.oae-trigger-unfollow',
                );
                casper.waitUntilVisible('#unfollow-modal', function() {
                    test.assertVisible(
                        '#unfollow-modal',
                        'Verify that the unfollow pane is showing after trigger',
                    );
                    casper.click(
                        'li.oae-list-actions + li .oae-list-grid-item input[type="checkbox"]',
                    );
    ...
    var verifyUnfollowingElements = function() {
        ...
        test.assertExists(
            '#unfollow-modal #unfollow-selected-container li',
            'Verify that the users that will be unfollowed are listed',
        );
        // Verify there is a confirmation message shown in the UI
        test.assertExists(
            '#unfollow-modal #unfollow-list-container h4',
            'Verify that a confirmation message is shown before unfollowing the users',
        );
        // Verify the cancel button is present
        test.assertExists(
            '#unfollow-modal .modal-footer button[data-dismiss="modal"]',
            'Verify that the cancel button is present',
        );
        // Verify the unfollow button is present
        test.assertExists(
            '#unfollow-modal .modal-footer button#unfollow-unfollow',
            "Verify that the 'Unfollow' button is present",
        );
    };
        ...
        casper.waitForSelector(
            ...
            function() {
                test.assertDoesntExist(
                    ...
                    'Verify a user can be unfollowed',
                );
                // Verify that the user is removed from the list when unfollowed
                test.assertDoesntExist(
                    '#network-widget #network-following .oae-list li:first-child + li',
                    'Verify an unfollowed user is removed from the following list',
                );
            },
        );
    };
