...
<div class="notifications-widget hide">
    ...
    <div id="notifications-template"><!--
        {for activity in results}
            ...
            <li class="oae-listitem" data-id="${activity['oae:activityId']}" data-published="${activity.published}">
                <div class="pull-left">
                    ${renderThumbnail(actor)}
                </div>
                {if !previewObj['oae:collection'] && previewObj['oae:id'] !== oae.data.me.id}
                    <div class="pull-right hidden-xs hidden-sm notifications-thumbnail">
                        ${renderThumbnail(previewObj)}
                    </div>
                {/if}
                <div class="notifications-summary {if !previewObj['oae:collection']} notification-object {/if}">${renderActivitySummary(activity)}</div>
            </li>
        {/for}
    --></div>
</div>
