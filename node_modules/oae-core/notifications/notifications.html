...
<div class="notifications-widget hide">
    ...
    <div id="notifications-template"><!--
        {for activity in results}
            ...
            {var previewObj = activity.activity.target || activity.activity.object}
            {if previewObj['oae:id'] === oae.data.me.id}
                {var previewObj = activity.activity.object}
            {/if}

            <li data-id="${ac                {var hasTargetThumbnail = (!previewObj['oae:collection'] && previewObj['oae:id'] !== oae.data.me.id)}
tivity['oae:activityId']}" data-published="${activity.published}">
