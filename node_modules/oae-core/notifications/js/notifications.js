...
define(['jquery', 'underscore', 'oae.core'], function($, _, oae) {
    ...
    return function(uid) {
        ...
        var flagUnread = function() {
            ...
            $('#notifications-container .oae-listitem').removeClass('alert-info');
        ...
        /**
         * When the popover is closed, we assume that the user has seen all of its unread notifications and mark
         * all of them as read.
         */
        ...
        var getNotifications = function() {
            ...
            infinityScroll = $('#notifications-container .oae-list').infiniteScroll('/api/notifications', {
                'limit': 6
            }, '#notifications-template', {
                'scrollContainer': $('#notifications-container'),
...
=======
            oae.api.util.template().render($('#notifications-noresults-template', $rootel), null, $('#notifications-container .oae-list', $rootel));
>>>>>>> WIP on bringing native bootstrap popovers back
        };
        ...
        var setUpPushNotifications = function() {
            oae.api.push.subscribe(oae.data.me.id, 'notification', oae.data.me.signature, 'activitystreams', true, function(activity) {
                ...
                if ($('#notifications-container', $rootel).is(':visible')) {
                    markAsRead();
                }
            });
        };
        ...
        /**
         * Initializes the notifications popover
         */
        var setUpNotificationsPopover = function() {
            // Initialize the popover
            $('.oae-trigger-notifications').popover({
                'content': oae.api.util.template().render($('#notifications-popover-template', $rootel)),
                'container': '#' + uid + ' .notifications-widget',
                'html': true,
                'placement': 'bottom',
                'title': ''
            });

            // Remove the unread count when the popover is hidden
            $('.oae-trigger-notifications').on('hidden.bs.popover', removeUnreadCount);

            // If the popover is shown retrieve the notifications and mark them as read
            $('.oae-trigger-notifications').on('shown.bs.popover', function () {
                getNotifications();
                ...
                // popover on the current page
                markAsRead();
            });
        };

        setUpPushNotifications();
        setUpNotificationsPopover();
