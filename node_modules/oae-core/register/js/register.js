    ...
    return function(uid, showSettings) {
        ...
        var setUpReCaptcha = function() {
            if (recaptchaEnabled) {
                ...
                $rootel.on('keyup', '#recaptcha_response_field', hideRecaptchaError);
                // Add a consistent height to the bottom row containers
                $('#register-usercheck-container', $rootel).addClass('register-captcha-create-has-captcha');
            } else {
                $('#register-captcha-container', $rootel).hide();
            }
        };
        ...
        var showRecaptchaError = function() {
            ...
            $('#register-captcha-column', $rootel).addClass('has-error');
        ...
        var hideRecaptchaError = function() {
            ...
            $('#register-captcha-column', $rootel).removeClass('has-error');
        };
                ...
                if (err) {
                    ...
                    if (err.msg === 'Invalid reCaptcha token') {
                        ...
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__ACCOUNT_NOT_CREATED__', 'register'),
                            oae.api.i18n.translate('__MSG__AN_ERROR_OCCURRED_WHILE_CREATING_THE_ACCOUNT__', 'register'),
                            'error'
                        );
                    }
                    ...
                    oae.api.authentication.localLogin(values.username, values.password, loginOpts, function() {
                        // Relocate to the destination
                        window.location = redirectUrl || '/me';
                    });
                }
            });
        };
        ...
        var showRegisterForm = function() {
            // Render the email form pre-populated if this is being accepted from an invitation
            oae.api.util.template().render($('#email-form-group-template', $rootel), {
                'invitationInfo': invitationInfo
            }, $('#email-form-group-container', $rootel));
        ...
        var setUpTermsAndConditions = function() {
            ...
            if (termsAndConditionsEnabled) {
                oae.api.util.template().render($('#register-accept-template', $rootel), null, $('#register-accept-container', $rootel));
                $('#register-accept-terms-container', $rootel).show();
                ...
                $rootel.on('click', '#register-continue-signup', function() {
                    // Show the register form and hide the Terms and Conditions
                    showRegisterForm();
                    // As the Terms and Conditions have now been seen by the user, we check the Terms and Conditions checkbox
                    $('#register-accept-terms-and-conditions', $rootel).prop('checked', true);
                });
            }
        };
        ...
        var initRegister = function() {
            $(document).on('click', '.oae-trigger-register', function() {
                var registerData = $(this).data();

                redirectUrl = registerData.redirecturl;
                invitationInfo = {
