...
define(['jquery', 'underscore', 'oae.core', 'markdown'], function($, _, oae) {
    ...
    return function(uid, showSettings) {
        ...
        var setUpReCaptcha = function() {
            if (recaptchaEnabled) {
                $('#register-captcha', $rootel).addClass('span7');
                $('#register-accept-and-create-container', $rootel).addClass('span5');
                ...
                $('#register-accept-and-create-container', $rootel).addClass('span12');
            }
        };
        ...
        /*
         * Show a validation message in case a duplicate username was entered
         */
        var showDuplicateUserNameError = function() {
            // Set the aria attributes on the username input field
            $('#register-username', $rootel).attr({
                'aria-invalid': 'true',
                'aria-describedby': 'duplicate-username-error'
            });
            $('#register-username', $rootel).parent().parent().addClass('error');
            $('#duplicate_user-error', $rootel).show();
        };

        /*
         * Hide the duplicate username validation message
         */
        var hideDuplicateUserNameError = function() {
            $('#register-username', $rootel).removeAttr('aria-invalid aria-describedby');
            $('#register-username', $rootel).parent().parent().removeClass('error');
            $('#duplicate_user-error', $rootel).hide();
        };
        ...
        var createUser = function() {
            ...
            hideRecaptchaError();
            // Hie the duplicate username error
            hideDuplicateUserNameError();
            ...
            oae.api.user.createUser(values.username, values.password, displayName, additionalOptions, values.recaptchaChallenge, values.recaptchaResponse, function(err, createdUser) {
                if (err) {
                    if (err.msg === 'This login id already exists and is already associated to a user') {
        ...
        var initRegister = function() {
            $(document).on('click', '.oae-trigger-register', function() {
                ...
                $('#register-username', $rootel).on('keyup', function () {
                    // Hide the duplicate username error
                    hideDuplicateUserNameError();
                });
            });
        };
