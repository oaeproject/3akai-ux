    ...
    return function(uid, showSettings) {
        ...
        var createUser = function() {
            ...
            oae.api.user.createUser(values.username, values.password, displayName, additionalOptions, values.recaptchaChallenge, values.recaptchaResponse, function(err, createdUser) {
                ...
                // If the user entered an invalid reCaptcha token
                if (err === 'Invalid reCaptcha token') {
                    // Refresh reCaptcha
                    if (recaptchaEnabled) {
                        Recaptcha.reload();
                        showRecaptchaError();
                ...
                // If another error occurred; in this case a duplicate entry of a user
                } else if (err && err !== 'Invalid reCaptcha token') {
                    // Refresh reCaptcha
                    if (recaptchaEnabled) {
                        Recaptcha.reload();
                    }
