...
casper.test.begin('Widget - Register', function(test) {
    ...
    casper.start(configUtil().tenantUI, function(){
        ...
        casper.then(function() {
            casper.echo('# Verify register modal', 'GREEN_BAR');
            verifyOpenRegister();
        });

        // Verify that the register form is available
        casper.then(function() {
            casper.echo('# Verify register form elements', 'GREEN_BAR');
            verifyRegisterForm();
        });

        // Verify that all the errors are shown when there is wrong or no input
        casper.then(function() {
            casper.echo('# Verify register form validation', 'GREEN_BAR');
            verifyRegisterFormValidation(user1.username);
        });

        // Verify registering a user
        casper.then(function() {
            casper.echo('# Verify registering a user', 'GREEN_BAR');
