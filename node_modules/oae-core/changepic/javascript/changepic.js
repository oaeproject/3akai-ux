...
define(['jquery', 'underscore', 'oae.core', 'jquery.imgareaselect'], function($, _, oae) {
    ...
    return function(uid, showSettings) {
            ...
            if (allowSubmit) {
                ...
                $(picForm).ajaxForm({
                    'success': function(data) {
                        ...
                        if (type === 'user') {
                            oae.data.me.picture.large = data.picture.large;
                        }
                        doInit();
                    },
                    'error': function() {
        ...
        var doInit = function() {
            ...
            if (!principalData.picture.small) {
            ...
            if (principalData.picture.large) {
                // Set the unvisible image to the full blown image.
                $(pictureMeasurer).html('<img src="' + principalData.picture.large +'" id="picture_measurer_image" />');
                ...
                $(pictureMeasurerImage).on('load', function(ev) {
                    ...
                    // Set the images
                    $(fullPictureSpan).html('<img alt="' + $('#changepic_fullpicture_alt').html() + '" id="changepic_fullpicture_img" src="' + principalData.picture.large + '" />');
                    $(thumbnailSpan).html('<img alt="' + $('#thumbnail_alt').html() + '" id="thumbnail_img" src="' + principalData.picture.large + '" />');
        ...
        var cropPicture = function(userSelection) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    // Update the global me object.
                    if (type === 'user') {
                        oae.data.me.picture.small = data.picture.small;
                        oae.data.me.picture.medium = data.picture.medium;
                    }

                    // Change all the pictures on the page.
                    for (var i = 0; i < imagesToChange.length; i++) {
                        $(imagesToChange[i]).attr('src', data.picture.small);
                    }
                ...
                error: function(xhr, textStatus, thrownError) {
                    oae.api.util.notification(oae.api.i18n.translate('__MSG__AN_ERROR_HAS_OCCURRED__'), textStatus, 'error');
                }
            });
