...
casper.test.begin('Widget - Change picture', function(test) {
    ...
    casper.start(configUtil().tenantUI, function() {
        ...
        casper.then(function() {
            casper.echo('# Verify open change picture modal', 'GREEN_BAR');
            verifyOpenChangePicture();
        });

        // Verify picture upload
        casper.then(function() {
            casper.echo('# Verify picture upload', 'GREEN_BAR');
            verifyPictureUpload();
        });

        // Verify picture upload validation
        casper.then(function() {
            casper.echo('# Verify pictures over 10MB aren\'t accepted', 'GREEN_BAR');
