...
casper.test.begin('Widget - Change picture', function(test) {
    ...
    var openChangePicture = function() {
        casper.waitForSelector(
            ...
            function() {
                ...
                test.assertExists(
                    '.oae-trigger-changepic',
                    'Change picture trigger exists',
                );
                casper.click('.oae-trigger-changepic');
                casper.waitUntilVisible('#changepic-modal', function() {
                    test.assertVisible(
                        '#changepic-modal',
                        'Change picture pane is showing after trigger',
                    );
                    casper.click(
                        '#me-clip-container .oae-clip-content > button',
                    );
    ...
    var verifyPictureUpload = function() {
        ...
        test.assertExists(
            'form#changepic-form',
            'The change picture form is present',
        );
        // Verify the no-image placeholder is present
        test.assertSelectorHasText(
            '.changepic-dropzone-content small',
            'Drag a photo here to upload',
            'Some help is shown to users that have no profile picture yet',
        );
        // Upload an image
        casper.fill(
            'form#changepic-form',
        ...
        casper.waitForSelector('.jcrop-holder', function() {
            ...
            test.assertExists(
                '.jcrop-holder > img',
                'The image is showing in the cropping area',
            );
            // Verify the 'set picture' button is present
            test.assertExists(
                '#changepic-set',
                "The 'Set picture' button is present",
            );
            // Click the button
            ...
            casper.waitForSelector(
                ...
                function() {
                    test.assertDoesntExist(
                        ...
                        'Picture successfully submitted and stored',
                    );
    ...
    var verifyLargePictureValidation = function() {
        ...
        casper.waitForSelector(
            ...
            function() {
                test.assertExists(
                    '#oae-notification-container .alert.alert-error',
                    'Pictures over 10MB are rejected',
                );
                casper.click('#oae-notification-container .close');
            },
        );
