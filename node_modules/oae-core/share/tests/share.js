...
casper.test.begin('Widget - Share', function(test) {
    ...
    var openLibraryShare = function() {
        casper.waitForSelector(
            ...
            function() {
                ...
                casper.click(
                    '#contentlibrary-widget .oae-list-header-actions button.oae-trigger-share',
                );
                casper.waitUntilVisible('#share-modal', function() {
                    test.assertVisible(
                        '#share-modal',
                        'The share pane is showing after trigger',
                    );
                });
            },
        );
    ...
    var verifyLibraryShareElements = function() {
        casper.waitForSelector('#share-modal', function() {
            ...
            test.assertExists(
                'form#share-form ul.as-selections',
                'The share form autosuggest is present',
            );
            test.assertExists(
                'form#share-form button#share-save',
                'The share form submit button is present',
            );
            test.assertExists(
                'form#share-form button#share-save:disabled',
                'The share form submit button is disabled by default',
            );
            test.assertExists(
                'form#share-form button[data-dismiss="modal"]',
                'The share cancel button is present',
            );
        });
    };
    ...
    var verifySharing = function(user) {
        ...
        casper.thenEvaluate(function(user) {
            document.querySelector('#share-form .as-selections input').value =
                user.displayName;
        }, user);
        ...
        casper.waitForSelector('.as-list li', function() {
            ...
            test.assertExists(
                'form#share-form button[type="submit"]:not([disabled])',
                'The share form submit button is enabled after autosuggest selection',
            );
            ...
            casper.waitForSelector(
                ...
                function() {
                    test.assertDoesntExist(
                        ...
                        'The content item was successfully shared',
                    );
                    ...
                    casper.then(function() {
                        ...
                        casper.waitForSelector(
                            '#contentlibrary-widget .oae-list.oae-list-grid li.oae-list-actions + li',
                            function() {
                                // Verify there is an item that was shared with the user
                                test.assertExists(
                                    'li.oae-list-actions + li',
                                    "The shared item can be found in the second user's library",
                                );
                            },
                        );
                    });
