...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        // Whether or not the user can manage all content that's being shared
        var canManage = false;

        // Keep track of the available roles for the content
        var roles = null;
        ...
        var setUpShare = function() {
            $rootel.on('click', '#share-save', function() {
                ...
                /**
                 * Assign a role to the principals to share with
                 */
                var addShareRole = function() {
                    var role = $('#share-role', $rootel).val();
                ...
                if (resourceType === 'content') {
                    if (canManage) {
                        addShareRole();
                        shareFunction = oae.api.content.updateMembers;
                    } else {
                    ...
                    if (canManage) {
                        addShareRole();
                        shareFunction = oae.api.discussion.updateMembers;
                    } else {
                ...
                var done = 0;
                var share = function() {
                    shareFunction(resources[done], canManage ? members : principals, function(err) {
                        done++;
        ...
        var setUpAutoSuggest = function() {
            // Render the autosuggest input field
            oae.api.util.template().render($('#share-autosuggest-template', $rootel), {
                'canManage': canManage,
                'roles': roles
            }, $('#share-autosuggest-container', $rootel));
        ...
        var getContext = function(ev) {
            ...
            $(document).on('oae.context.send.share', function(ev, data) {
                ...
                if ($trigger.attr('data-id')) {
                    ...
                    // Get the available roles before setting up the autosuggest
                    $(document).on('oae.context.sendroles.share', function(ev, _roles) {
                        roles = _roles;
                        setUpAutoSuggest();
                    });
                    $(document).trigger('oae.context.getroles', 'share');
        ...
        var initShare = function() {
            $(document).on('click', '.oae-trigger-share', function(ev) {
                getContext(ev);
            ...
            $('#share-modal', $rootel).on('shown.bs.modal', focusAutoSuggest);
        };
