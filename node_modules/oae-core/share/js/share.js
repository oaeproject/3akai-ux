...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid) {
        ...
        // Variable that will keep track of the element that triggered the popover
        var $trigger = null;
        ...
        /**
         * Toggle the share popover
         */
        var toggleSharePopover = function() {
            // Emulate a click on the popover trigger
            $trigger.click();
        };
        ...
        var setUpShare = function() {
            $trigger.next().one('click', '#share-save', function() {
                // Disable the share button
                $('#share-save', $trigger.next()).prop('disabled', true);

                // Extract the resource ids and principal ids
                var resources = _.pluck(selectedResources, 'id');
                var principals = _.pluck(oae.api.util.autoSuggest().getSelection($trigger.next()), 'id');
            ...
            $trigger.next().one('click', '#share-cancel', toggleSharePopover);
        };
        ...
        var autoSuggestChanged = function() {
            $('#share-save', $trigger.next()).prop('disabled', oae.api.util.autoSuggest().getSelection($trigger.next()).length ? false : true);
        };
        ...
        /**
         * Initializes the share popover
         */
        var setUpSharePopover = function() {
            $(document).on('click', '.oae-trigger-share', function() {
                $trigger = $(this);
                // When the popover is shown retrieve the context and set up the autosuggest
                $trigger.one('shown.bs.popover', function () {
                    ...
                    $trigger[0].style.webkitTransform = 'scale(1)';
                    getContext();
                    setUpShare();
                });

                // If the popover is not initialized yet we go ahead and do that
                if (!$rootel.find('.popover').length) {
                    $trigger.popover({
                        'content': oae.api.util.template().render($('#share-popover-template', $rootel)),
                        'html': true,
                        'placement': 'bottom',
                        'title': ''
                    });
