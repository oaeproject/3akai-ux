    ...
    <div id="mycontent_list_template"><!--
            ...
            <div class="mycontent_item{if item_index % 2 == 1} s3d-highlight_area_background{/if}">
                <div class="mycontent_item_img fl-force-left">
                    <a href="/content#p=${item['_path']|safeURL}/{if item.filename}${item.filename|safeURL}{else}${item.fullname|safeURL}{/if}" title="${item['sakai:pooled-content-file-name']}">
                        ...
                        {else}
                            <img class="s3d-icon-32" src="${item.type_img_url}" alt="${item.type}" />
                        {/if}
                    </a>
                </div>
                <div class="mycontent_item_info">
                    <p class="mycontent_item_title">
                        <a class="mycontent_item_link s3d-widget-links s3d-bold" href="/content#p=${item['_path']|safeURL}/{if item.filename}${item.filename|safeURL}{else}${item.fullname|safeURL}{/if}">
                            ${item.name}
                        </a>
                    </p>
        ...
        {forelse}
            <div id="mycontent_no_content_wrap">
                <button class="s3d-button s3d-large-button add_content_button" type="button">__MSG__ADD_CONTENT__</button>
