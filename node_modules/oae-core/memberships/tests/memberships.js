...
casper.test.begin('Widget - Memberships', function(test) {
    ...
    var verifyMembershipsLeave = function() {
        ...
        test.assertExists(
            '#memberships-widget .oae-list-header-actions button.oae-trigger-leavegroup:disabled',
            'The leave group button is disabled by default',
        );
        ...
        casper.waitForSelector('li.oae-list-actions + li', function() {
            ...
            test.assertExists(
                '#memberships-widget .oae-list-header-actions button.oae-trigger-leavegroup:not([disabled])',
                'The leave group button is enabled after checking a membership item',
            );
            // Click the leave group button
            casper.click(
                '#memberships-widget .oae-list-header-actions button.oae-trigger-leavegroup',
            );
            // Verify that the leavegroup widget modal is shown
            casper.waitForSelector('#leavegroup-modal', function() {
                test.assertExists(
                    '#leavegroup-modal',
                    'The leavegroup modal can be triggered from within the list options',
                );
                // Close the modal
                ...
                test.assertExists(
                    '#memberships-widget .oae-list-header-actions button.oae-trigger-leavegroup:disabled',
                    'The leave group button is disabled again after unchecking all groups',
                );
            });
        });
    };
        ...
        casper.waitForSelector('.oae-list .alert-info', function() {
            ...
            casper.wait(configUtil.searchWaitTime, function() {
                ...
                casper.waitForSelector('.oae-list li', function() {
                    test.assertExists(
                        '.oae-list li',
                        "1 result is returned for '" + group.displayName + "'",
                    );
                    test.assertSelectorHasText(
                        '.oae-list li .oae-tile-metadata a',
                        group.displayName,
                        "The returned file has the title '" +
                            group.displayName +
                            "'",
                    );
                    // Reset the form
    ...
    var verifyMembershipsViewMode = function() {
        ...
        // Verify compact list
        casper.click(
            '#memberships-widget .oae-list-header-actions button[data-type="oae-list-compact"]',
        );
        test.assertExists(
            '.oae-list.oae-list-compact',
            'Memberships can be switched to compact view',
        );
        // Verify details list
        casper.click(
            '#memberships-widget .oae-list-header-actions button[data-type="oae-list-details"]',
        );
        test.assertExists(
            '.oae-list.oae-list-details',
            'Memberships can be switched to details view',
        );
        // Switch back to grid view
        casper.click(
            '#memberships-widget .oae-list-header-actions button[data-type="oae-list-grid"]',
        );
        test.assertExists(
            '.oae-list.oae-list-grid',
            'Memberships can be switched to grid view',
        );
    };
    ...
    var verifyMembershipsElements = function() {
        ...
        test.assertExists(
            '#memberships-widget .oae-list li:first-child',
            'Initial dummy list item is present',
        );
        // Verify dummy list item has create group button
        test.assertExists(
            '#memberships-widget .oae-list li:first-child .oae-trigger-creategroup',
            "The first list item has a 'Create group' trigger",
        );
        ...
        casper.waitForSelector('#creategroup-modal', function() {
            test.assertExists(
                '#creategroup-modal #creategroup-modal-title',
                'The creategroup widget can be triggered from the dummy list item',
            );
            casper.click('#creategroup-modal .close');
        });
        // Verify list options are there
        test.assertExists(
            '#memberships-widget .oae-list-header-actions',
            'The list options are present',
        );
        // Verify list options contain a checkbox that selects all
        test.assertExists(
            '#memberships-widget .oae-list-header-actions input[type="checkbox"]',
            "The 'Select all' checkbox is present in the list options",
        );
        // Verify list options contain a leave group button
        test.assertExists(
            '#memberships-widget .oae-list-header-actions .oae-trigger-leavegroup',
            'The leave group button is present in the list options',
        );
        ...
        test.assertExists(
            '#memberships-widget .oae-list-header-search .oae-list-header-search-query',
            ...
            groupUtil.createGroup(null, null, null, null, null, null, function(
                ...
                casper.then(function() {
                    ...
                    casper.waitForSelector(
                        '#memberships-widget .oae-list.oae-list-grid li',
                        verifyMembershipsElements,
                    );
                });
