...
casper.test.begin('Widget - Edit group', function(test) {
    ...
    var verifyEditGroupFormElements = function() {
        ...
        casper.waitForSelector('#editgroup-modal', function() {
            ...
            test.assertExists('form#editgroup-form #editgroup-joinable-container input[name="oae-joinable-group"]', 'The editgroup joinable options are present');
    ...
    casper.start(configUtil().tenantUI, function(){
        ...
        casper.then(function() {
            groupUtil().createGroup([], [], function(group) {
                ...
                casper.thenOpen(configUtil().tenantUI + group1.profilePath, function(){
                    // Verify that the group can be edited
                    casper.then(function() {
                        casper.echo('# Verify editgroup modal', 'GREEN_BAR');
                        verifyOpenEditGroup();
                    });

                    // Verify that the group form elements are present
                    casper.then(function() {
                        casper.echo('# Verify editgroup form elements', 'GREEN_BAR');
                        verifyEditGroupFormElements();
                    });

                    // Verify that the errors from the group form works
                    casper.then(function() {
                        casper.echo('# Verify editgroup form validation', 'GREEN_BAR');
                        verifyEditGroupFormValidation();
                    });

                    casper.then(function() {
                        casper.echo('# Verify group can be edited', 'GREEN_BAR');
                        verifyEditGroup();
                    });
                });
            });
