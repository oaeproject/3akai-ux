...
define(['require', 'jquery', 'oae.core'], function(require, $, oae) {
    return function(uid, showSettings, widgetData) {
        ...
        var renderDefault = function() {
            ...
            if (
                widgetData.previews &&
                widgetData.previews.status === 'pending'
            ) {
                oae.api.util
                    .template()
                    .render(
                        $('#filepreview-default-pending-template', $rootel),
                        { content: widgetData },
                        $('#filepreview-container', $rootel),
                    );
            } else {
                oae.api.util.template().render(
                    $('#filepreview-default-template', $rootel),
                    {
                        content: widgetData,
                        displayOptions: {
                            addLink: false,
        ...
        var renderImagePreview = function() {
            oae.api.util
                .template()
                .render(
                    $('#filepreview-image-template', $rootel),
                    { content: widgetData },
                    $('#filepreview-container', $rootel),
                );
        };
        ...
        var renderMediaCorePreview = function() {
            $.ajax({
                ...
                success: function(data) {
                    oae.api.util.template().render(
                        $('#filepreview-mediacore-template', $rootel),
                        {
                            isAudio:
                                widgetData.mime
                                    .substring(0, 6)
        ...
        var renderMarkdownPreview = function() {
            $.ajax({
                ...
                success: function(data) {
                    oae.api.util.template().render(
                        $('#filepreview-markdown-template', $rootel),
                        {
                            content: data,
                        },
                        $('#filepreview-container', $rootel),
        ...
        var setUpFilePreview = function() {
            ...
            if (widgetData.mime.substring(0, 6).toLowerCase() === 'image/') {
                ...
                widgetData.mime.toLowerCase() === 'text/x-markdown' &&
                widgetData.size <= MAX_TEXT_PREVIEW_SIZE
            ) {
