    ...
    var verifyFilePreview = function() {
        casper.waitForSelector('#filepreview-container', function() {
            test.assertExists(
                ...
                'The file preview container is present',
            );
            test.assertExists(
                '#filepreview-container #filepreview-image-container',
                'The image container is present',
            );
            test.assertExists(
                '#filepreview-container #filepreview-image-container img',
                'The image is shown on the content profile page',
            );
        });
    };
    ...
    var verifyCollabDocPreview = function(user, collabdocProfile) {
        casper.waitForSelector(
            ...
            function() {
                test.assertExists(
                    '#etherpad-container',
                    'The etherpad container is present',
                );
                test.assertExists(
                    '#etherpad-container #etherpad-edit-mode',
                    'The etherpad edit mode is present for other manager',
                );
                test.assertExists(
                    '#etherpad-container iframe#etherpad-editor',
                    'The etherpad iframe is present',
                );
                userUtil.doLogOut();
                ...
                casper.waitForSelector(
                    ...
                    function() {
                        test.assertExists(
                            '#etherpad-container',
                            'The etherpad container is present for viewer',
                        );
                        test.assertExists(
                            '#etherpad-container #etherpad-view-mode',
                            'The etherpad view mode is present for viewer',
                        );
                        test.assertDoesntExist(
                            '#etherpad-container iframe#etherpad-editor',
                            'The etherpad edit iframe is not present for viewer',
                        );
                    },
                );
            },
                    ...
                    function(err, content2Profile) {
                        contentUtil.createCollabDoc(
                            ...
                            function(err, collabdocProfile) {
                                ...
                                casper.then(function() {
                                    casper.echo(
                                        '# Verify collaborative document preview elements present',
                                        'INFO',
                                    );
                                    verifyCollabDocPreview(
                                        user2,
                                        collabdocProfile,
                                    );
                                });
