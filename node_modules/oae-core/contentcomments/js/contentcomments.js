...
define(['jquery', 'oae.core', 'jquery.timeago'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        var constructAndAppendComment = function(err, comment) {
            // Top level comment
            if (!comment.replyTo) {
                $('#contentcomments_content_container', $rootel).prepend(oae.api.util.template().render($('#contentcomments_comment_template'), {
                    'results': [comment],
            ...
            // Reply on an existing comment
                ...
                $('.contentcomments_comment_container[data-id="' + comment.replyTo + '"]', $rootel).after(oae.api.util.template().render($('#contentcomments_comment_template'), {
                    'results': [comment],
                    'canManage': contentProfile.isManager
                }));
            }
        ...
        var markCommentAsDeleted = function(comment) {
            $('.contentcomments_comment_container[data-id="' + comment.commentId + '"]').replaceWith(oae.api.util.template().render($('#contentcomments_comment_template'), {
                'results': [comment],
                'canManage': contentProfile.isManager
            }));
        };
