...
define(['jquery', 'oae.core', 'jquery.timeago'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        var constructAndAppendComment = function(err, comment) {
            // Top level comment
            if (!comment.replyTo) {
                $('#contentcomments_content_container li:first-child', $rootel).after(oae.api.util.renderTemplate('#contentcomments_comment_template', {
                    'results': [comment],
                    'canManage': contentProfile.isManager
                }));
            // Reply on an existing comment
            } else {
                $('li.media[data-id="' + comment.replyTo + '"]', $rootel).after(oae.api.util.renderTemplate('#contentcomments_comment_template', {
                    'results': [comment],
                    'canManage': contentProfile.isManager
                }));
            }
        ...
        var markCommentAsDeleted = function(comment) {
            $('li.media[data-id="' + comment.commentId + '"]').replaceWith(oae.api.util.renderTemplate('#contentcomments_comment_template', {
                'results': [comment],
                'canManage': contentProfile.isManager
            }));
        };
