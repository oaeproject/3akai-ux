...
define(['jquery', 'oae.core', 'jquery.timeago'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        var constructAndAppendComment = function(err, comment) {
            // Top level comment
            if (!comment.replyTo) {
                $('#contentcomments_content_container li:first-child', $rootel).after(oae.api.util.template().render('#contentcomments_comment_template', {
                    'results': [comment],
                    'canManage': contentProfile.isManager
                }));
            // Reply on an existing comment
            } else {
                $('li.media[data-id="' + comment.replyTo + '"]', $rootel).after(oae.api.util.template().render('#contentcomments_comment_template', {
                    'results': [comment],
                    'canManage': contentProfile.isManager
                }));
            }
        ...
        var markCommentAsDeleted = function(comment) {
            $('li.media[data-id="' + comment.commentId + '"]').replaceWith(oae.api.util.template().render('#contentcomments_comment_template', {
                'results': [comment],
                'canManage': contentProfile.isManager
            }));
        };
        ...
        var setUpReplyComment = function() {
            $rootel.on('click', '.contentcomments_reply_button', function() {
                ...
                $(this).siblings('.contentcomments_reply_container').toggle()
            });
            ...
            $rootel.on('click', '.contentcomments_post_reply', function() {
                ...
                $replyContainer = $($(this).parents('.contentcomments_reply_container'));
                $replyContainer.siblings('.contentcomments_reply_button').toggleClass('active');
