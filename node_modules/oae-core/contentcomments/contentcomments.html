...
<div id="contentcomments_comment_template"><!--
    {for comment in results}
        <li class="contentcomments_comment_container {if comment.level === 1} contentcomments_reply_container {elseif comment.level > 1} contentcomments_reply_container_deep {/if} oae-push" data-id="${comment.commentId}" data-key="${comment.created}">
            ...
            {else}
                <img class="oae-force-left contentcomments_entity_image" alt="${comment.createdBy.displayName}" src="{if comment.createdBy.mediumPicture}${comment.createdBy.mediumPicture}{else}/ui/img/entities/user_icon_medium.png{/if}">
                <div class="contentcomments_comment_box">
                    ...
                    <div class="oae-bold contentcomments_comment_title">${comment.createdBy.displayName} <span>${jQuery.timeago(new Date(parseInt(created.substring(0, created.length - 1))))}</span></div>
                    ${oae.api.util.security().safeUserInput(comment.body).replace(/\n/g, '<br/>')}
                </div>

                <div class="contentcomments_reply_container oae-hidden">
                    <img class="oae-force-left contentcomments_entity_image" src="{if oae.data.me.mediumPicture}${oae.data.me.mediumPicture}{else}/ui/img/entities/user_icon_medium.png{/if}">
                    <div class="contentcomments_post_comment_body_container">
