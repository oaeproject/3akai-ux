...
<div id="contentcomments_comment_template"><!--
    {for comment in results}
        <li class="contentcomments_comment_container {if comment.level === 1} contentcomments_reply_container {elseif comment.level > 1} contentcomments_reply_container_deep {/if} oae-push" data-id="${comment.commentId}" data-key="${comment.created}">
            ...
            {else}
                <img class="oae-force-left contentcomments_entity_image" alt="${comment.createdBy.displayName|safeUserInput}" src="{if comment.createdBy.picture.medium}${comment.createdBy.picture.medium}{else}/ui/img/entities/user_icon_medium.png{/if}">
                <div class="contentcomments_comment_box">
                    ...
                    <div class="oae-bold contentcomments_comment_title">${comment.createdBy.displayName|safeUserInput} <span>${$.timeago(new Date(parseInt(created.substring(0, created.length - 1))))}</span></div>
