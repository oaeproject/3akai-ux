casper.test.comment('Widget - Create discussion');
...
var verifyOpenCreateDiscussion = function() {
    casper.waitForSelector('.oae-clip-secondary .oae-clip-content > button', function() {
        ...
        casper.test.assertExists('.oae-trigger-creatediscussion', 'create discussion trigger exists');
        casper.click('.oae-trigger-creatediscussion');
        casper.wait(1000, function() {
            casper.test.assertVisible('#creatediscussion-modal', 'create discussion pane is showing after trigger');
            casper.click('.oae-clip-secondary .oae-clip-content > button');
        });
    });
};
...
var verifyCreatediscussion = function(user2Id) {
    ...
    casper.test.assertExists('form#creatediscussion-form', 'The create discussion form is present');
    casper.test.assertExists('#creatediscussion-name', 'The discussion name field is present');
    casper.test.assertExists('#creatediscussion-topic', 'The discussion topic field is present');
    // Fill the form
    ...
    casper.test.assertExists('#creatediscussion-change-permissions', 'The \'change permissions\' button is present');
    ...
    casper.test.assertExists('#creatediscussion-permissions-container #setpermissions-container input[type="radio"]', 'The \'change permissions\' radio button group is present');
    casper.test.assertExists('#creatediscussion-permissions-container .as-selections input', 'The \'share\' input field is present');
    ...
    casper.thenEvaluate(function(user2Id) {
        document.querySelector('#creatediscussion-permissions-container .as-selections input').value = user2Id;
    }, user2Id);
    ...
    casper.waitForSelector('.as-list li', function() {
        ...
        casper.test.assertExists('#creatediscussion-create', 'The \'Create discussion\' button is present');
        ...
        casper.wait(1000, function() {
            casper.test.assertVisible('#discussion-clip-container', 'Discussion profile is shown after creation of discussion');
            casper.test.assertSelectorHasText('#discussion-clip-container h1', 'Testing tools', 'Title matches \'Testing tools\'');
        });
    });
};
...
var verifyCreatediscussionValidation = function() {
    ...
    casper.test.assertExists('#creatediscussion-topic-error', 'Successfully validated empty topic');
    ...
    casper.click('#creatediscussion-create');
    // Verify that an error label is shown
    casper.test.assertExists('#creatediscussion-name-error', 'Successfully validated empty title');
};
