    ...
    return function(uid) {
        ...
        var setUpAddToFolder = function() {
            $('#addtofolder-form', $rootel).on('submit', function() {
                ...
                var selectedFolderProfilePath = $selectedFolder.attr(
                    'data-profilePath',
                );
                var selectedFolderDisplayName = $selectedFolder
                    .closest('.oae-listitem')
                    .find('h3')
                    .text();

                // Add  the selected content items to the selected folder
                oae.api.folder.addToFolder(
                    selectedFolderId,
                    _.pluck(selectedItems, 'id'),
                    function(err) {
                        ...
                        var notificationTitle = oae.api.util
                            ...
                            .render(
                                $(
                                    '#addtofolder-notification-title-template',
                                    $rootel,
                                ),
                                data,
                            );
                        var notificationBody = oae.api.util
                            ...
                            .render(
                                $(
                                    '#addtofolder-notification-body-template',
                                    $rootel,
                                ),
                                data,
                            );
        ...
        var setUpFolders = function(contextId) {
            ...
            var $visibleList = $('.tab-pane.active', $rootel);
            foldersInfinityScroll = $(
                '.oae-list.addtofolder-folder-list',
                $visibleList,
            ).infiniteScroll(
                url,
                {
                ...
                '#addtofolder-folders-template',
                {
                    ...
                    emptyListProcessor: function() {
                        oae.api.util.template().render(
                            $(
                                '#addtofolder-folders-noresults-template',
                                $rootel,
                            ),
                            {
                                context: contextId,
        ...
        var setUpGroups = function() {
            ...
            // Set up the infinite scroll instance
            groupsInfinityScroll = $(
                '#addtofolder-mygroups-list',
                $rootel,
            ).infiniteScroll(
                url,
                {
                ...
                {
                    scrollContainer: $(
                        '#addtofolder-mygroups-list-container',
                        $rootel,
                    ),
                    postProcessor: function(data) {
                        data.displayOptions = {
                    ...
                    emptyListProcessor: function() {
                        oae.api.util
                            ...
                            .render(
                                $(
                                    '#addtofolder-groups-noresults-template',
                                    $rootel,
                                ),
                                null,
                                $('#addtofolder-mygroups-list', $rootel),
                            );
                    },
                },
            );
        ...
        var setUpGroupFolders = function(groupId) {
            ...
            oae.api.group.getGroup(groupId, function(err, group) {
                oae.api.util.template().render(
                    $('#addtofolder-group-back-template', $rootel),
                    {
                        group: group,
                        displayOptions: {
                            addLink: false,
        ...
        var getContext = function() {
            ...
            $(document).on('oae.context.send.addtofolder', function(ev, data) {
                ...
                if ($trigger.attr('data-id')) {
                    ...
                    $(document).on(
                        'oae.list.sendSelection.addtofolder',
                        function(ev, data) {
        ...
        var finishGetContext = function() {
            if (
                contextProfile.resourceType === 'group' &&
                contextProfile.isManager
            ) {
        ...
        var addBinding = function() {
            ...
            $('#addtofolder-group-back-container', $rootel).on(
                'click',
                function() {
                    setUpGroups();
                    // Avoid default click behavior
