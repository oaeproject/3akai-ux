<!-- CSS -->
<link rel="stylesheet" type="text/css" href="/node_modules/oae-core/recentmessages/css/recentmessages.css" />
...
<!-- CONTENT -->
            ...
            {for message in results}
                <li {if message_index % 2} class="recentmessages_alterrow"{/if}>
                    <span class="recentmessages_sendername" title="${sakai.api.User.getDisplayName(message.userFrom[0])}">${message.dotteduserFrom} {if message.userFrom.length > 1} __MSG__AND_OTHERS__...{/if}:</span>
                        <a href="/me#l=messages{if message["sakai:category"]==="invitation"}/invitations{else}/${message["sakai:messagebox"]}{/if}&amp;message=${message.id}" class="oae-regular-links {if message["sakai:read"]==="false" || message["sakai:read"]===false} oae-bold{/if}">
    ...
    <div class="oae-widget-footer">
        <a href="/me#l=messages/inbox" class="oae-regular-links oae-action-seeall">__MSG__VIEW_AND_MANAGE_ALL_MY_MESSAGES__</a>
    </div>
</div>

<!-- JAVASCRIPT -->
<script type="text/javascript" src="/node_modules/oae-core/recentmessages/javascript/recentmessages.js"></script>
