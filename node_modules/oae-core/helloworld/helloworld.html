<!-- CSS -->
<link rel="stylesheet" type="text/css" href="/node_modules/oae-core/helloworld/css/helloworld.css" />

<!-- WIDGET CONTENT CONTAINER -->
<div class="helloworld_widget">
    <!-- MAIN VIEW -->
    <div id="helloworld_main" style="display:none;">
        <p>__MSG__HELLO_WORLD__, <span id="helloworld_username"></span></p>
    </div>
    <!-- SETTINGS VIEW -->
    <div id="helloworld_settings" style="display:none;">
        <form id="helloworld_settings_form" action="#">
            <p>__MSG__COLOR_TO_USE__ 
                <select id="helloworld_color">
                    <option value="#000000">__MSG__BLACK__</option>
                    <option value="#FF0000">__MSG__RED__</option>
                    <option value="#00FF00">__MSG__GREEN__</option>
                    <option value="#0000FF">__MSG__BLUE__</option>
                </select>
            </p>
            <div class="oae-force-right">
                <button type="button" class="oae-button oae-link-button" id="helloworld_cancel_settings">__MSG__CANCEL__</button>
                <button type="submit" class="oae-button oae-overlay-button">__MSG__SAVE__</button>
            </div>
            <hr class="oae-hidden oae-push"/>
        </form>
    </div>
</div>

<!-- JAVASCRIPT -->
<script type="text/javascript" src="/node_modules/oae-core/helloworld/javascript/helloworld.js"></script>
