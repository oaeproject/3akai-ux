...
<div class="s3d-dialog s3d-dialog-container" id="deletecontent_dialog">
                    ...
                    {for profile in profiles}
                        <li class="s3d-outer-shadow-container">
                            {if profile.userid}
                                ...
                                <a title="__MSG__VIEW_USERS_PROFILE__" href="/~${profile.userid|safeURL}" class="s3d-bold" target="_blank">${displayName}</a>
                            {elseif profile.collectionid}
                                {var displayName = profile["sakai:group-title"]}
                                <img alt="${displayName|safeOutput}" class="s3d-icon-32 fl-force-left" src="/dev/images/mimetypes/collection.png">
                                {if profile.showLink}
                                    <a title="__MSG__VIEW_USERS_PROFILE__" href="/content#p=${profile.collectionid}/${displayName|safeURL}" class="s3d-bold" target="_blank">
                                {else}
                                    <span class="s3d-bold">
                                {/if}
                            ...
                            {else}
                                ...
                                {if profile.showLink}
                                    <a title="__MSG__VIEW_USERS_PROFILE__" href="/~${profile.groupid|safeURL}" class="s3d-bold" target="_blank">
                                {else}
                                    <span class="s3d-bold">
                                {/if}
                                    ${displayName|safeOutput}
    ...
    <div id="deletecontent_button_container" class="fl-force-right">
        ...
        <button type="button" class="s3d-button s3d-overlay-button" id="deletecontent_action_removefromlibrary">__MSG__REMOVE_FROM_LIBRARY__</button>
        <button type="button" class="s3d-button s3d-overlay-button" id="deletecontent_action_removefromcollection">__MSG__REMOVE_FROM_COLLECTION__</button>
        <button type="button" class="s3d-button s3d-overlay-action-button" id="deletecontent_action_removefromsystem">__MSG__REMOVE_FROM_SYSTEM__</button>
        ...
        <button type="button" class="s3d-button s3d-overlay-button" id="deletecontent_action_apply">__MSG__APPLY__</button>
        ...
        <button type="button" class="s3d-button s3d-overlay-button" id="deletecontent_action_removefromcollection_only">__MSG__REMOVE_FROM_COLLECTION_ONLY__</button>
