    ...
    {macro renderView(view)}
        ...
        <div class="visible-${view}">
            {for block in blocks}
                ...
                {if totalWidth >= 12 || parseInt(block_tenantlandingpage, 10) === blocks.length - 1}
                    </div>
                    ${totalWidth = 0|eat}
                {/if}
            {/for}
