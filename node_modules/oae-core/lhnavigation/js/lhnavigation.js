...
define(['jquery', 'oae.core', 'jquery.history'], function($, oae) {
    ...
    return function(uid) {
        ...
        // Whether or not the navigation will be shown to the user
        var showNav = false;
         ...
         *
         * @param  {Object}     lhNavPages          The navigation structure for pages
         * @param  {Object}     [lhNavActions]      The navigation structure for actions
        ...
        /**
         * Mechanism used for widgets to pass in a navigation structure. We register a trigger event
         ...
         * data.
         *
         * A left hand navigation contains 2 different sections.
         * The first section is a list of action buttons that replace clips when the viewport width is too small to render them. These actions can also be grouped into collapsible sections.
         * An example array required to initiate these action buttons is the following:
         *
         * [{
         ...
         *
         * Notes:
         *     - `icon` is the FontAwesome icon class that preceeds the title of the navigation item. @see http://fontawesome.io/3.2.1/
         *     - `title` is what is shown as the title of the action button.
         *     - `class` is used to define the widget trigger. It can be extended to apply any class to the list item.
         *     - `children` is an optional Array of lhNavActions that will be rendered as children of the list item it belongs to. Each child item has the same properties as top level items. All items that have children will be rendered as a collapsible item.
         *
         ...
         *
         * Notes:
         *     - `id` is the page alias that will be used in the url (e.g. /me/dashboard).
         *     - `title` is what is shown as the title of the navigation item.
         *     - `icon` is the FontAwesome icon class that preceeds the title of the navigation item. @see http://fontawesome.io/3.2.1/
         *     - `layout` defines the structure of the page that is associated to the item.
         *         - `width` defines the page width. This leverages Bootstrap's grid system (@see http://getbootstrap.com/css/#grid)
         *         - `widgets` defines an Array of widgets to be loaded on the page
         ...
         */
        $(window).on('oae.trigger.lhnavigation', function(ev, lhNavPages, lhNavActions, baseUrl, hasNav) {
            showNav = hasNav || false;
            lhNavigationPages = lhNavPages;
            setUpNavigation(lhNavPages, lhNavActions, baseUrl);
        });
