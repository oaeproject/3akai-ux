...
define(['jquery', 'oae.core', 'jquery.history'], function($, oae) {
    ...
    return function(uid) {
        ...
        // Holds the base url of the page that is given when the lhnav is initialized when the
        // lhnavigation is triggered
        var baseUrl = null;
        ...
        var renderPage = function() {
            ...
            if ($cachedPage.length > 0) {
                ...
                // TODO: We are temporarily showing the navigation toggle on the content and discussion profile
                // pages here because the secondary clip items collapse into it on narrow screens. In the future
                // those secondary clips will collapse into the main clip instead. In this. case this lhnav toggle
                // can be completely removed from the lhnavigation.html template because in all other views, the
                // lhnavigation toggle is provided by the `listOptionsHeader` macro in list.html
                var showNavToggle = (baseUrl.indexOf('/content') === 0 || baseUrl.indexOf('/discussion') === 0);
         ...
         */
        $(window).on('oae.trigger.lhnavigation', function(ev, _lhNavigationPages, _lhNavigationActions, _baseUrl) {
            baseUrl = _baseUrl;
