...
define(['jquery', 'oae.core', 'jquery.history'], function($, oae) {
    ...
    return function(uid) {
         ...
         *     - lhNavPages is an Array of `page links` that link to a new page, showing specified widgets
         *     - lhNavActions is an Array of `action buttons` that replace clips when the viewport width is too small to render them
         *
         * Example lhNavPages Array:
         *     - `id` is the alias that will be used in the url (e.g. /me/dashboard).
         *     - `title` is what is shown as the title of the navigation item.
         *     - `icon` is the icon that preceeds the title of the navigation item.
         *     - `layout` defines the structure of the page that is associated to the link.
         *         - `width` will define what width the page has. This leverages Bootstrap's grid system (@see http://getbootstrap.com/css/#grid)
         *         - `widgets` defines an Array of widgets to be loaded on the page
         *             - `id` is the ID of the widget to be loaded
         *             - `settings` is a settings object that will be passed into the widget as `widget data`
         *
         * [{
         *     'id': 'dashboard',
