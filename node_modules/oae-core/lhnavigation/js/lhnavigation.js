...
define(['jquery', 'oae.core', 'jquery.history'], function($, oae) {
    ...
    return function(uid) {
        ...
        var setUpNavigation = function(lhNavPages, lhNavActions, baseUrl) {
            ...
            var queryString = loadedUrl.attr('query');
            var queryStringObj = loadedUrl.param();
            var path = loadedUrl.attr('path');
            ...
            // Take the _suid (state unique id) off the query string to ensure we get a unique state
            // that will trigger the statechange event, retaining the remainder of the query string.
            // We also take off the empty string key, since History.js appends the _suid in the
            // following manner: /?&_suid=<unique state id>, we need to make sure we don't just
            // append an unnecessary empty query string parameter
            if (queryString) {
                path += '?' + queryString;
            }

            // Store any search query onto the data object
            if (queryStringObj.q) {
                data.query = queryStringObj.q;
            }
