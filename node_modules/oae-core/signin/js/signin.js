...
define(['jquery', 'oae.core', 'moment', 'async', 'underscore', 'select2'], function($, oae, moment, async, _, select2) {
    ...
    return function(uid, showSettings, widgetData) {
         ...
         */
        let finishLogin = function(err) {
            if (err) {
                let signInMessage = oae.api.i18n.translate('__MSG__SIGN_IN__');
                ...
                let existingCookie = _.findWhere(loggedInTenancies, { alias: currentTenant.alias });
         ...
         */
        let doLocalSignIn = function() {
            // Hide the login button and show a logging in message
            let signingInMessage = oae.api.i18n.translate('__MSG__SIGNING_IN__');
            $('#signin-local-form button', $rootel).html(signingInMessage);
            $('#signin-local-form *', $rootel).prop('disable', 'true');

            // Get the entered username and password
            let username = $.trim($('#signin-local-username', $rootel).val());
            let password = $.trim($('#signin-local-password', $rootel).val());

            let ldapEnabled = authStrategyInfo[oae.api.authentication.STRATEGY_LDAP];
            let localEnabled = authStrategyInfo[oae.api.authentication.STRATEGY_LOCAL];
         ...
         */
        let initLocalSignInFormValidation = function() {
            let validateOpts = {
                'messages': {
                    'username': {
        ...
        let _getTenantLogo = function (tenantAlias, callback) {
            $.ajax({
                'url': `${protocol}://${tenantAlias}.oae.com/api/ui/logo`,
         ...
         * form
         */
        let renderSignInOptions = function() {
            ...
            let now = moment();
            let cookieName = 'loggedin_tenancies';
            let accessDeniedToContent = location.attr('relative').indexOf('accessdenied') !== -1;
            ...
            async.map(loggedInTenancies, (eachTenancy, callback) => {
                ...
                if (accessDeniedToContent) {
                    if(loggedInTenancies && loggedInTenancies.length > 0) {
                        ...
                        $rootElement = $('body');
                        oae.api.util.template().render($('#select-other-tenancy-template', $rootElement), {
                            'loggedInTenancies': loggedInTenancies,
                            'currentTenant': currentTenant
                        }, $('#login-to-other-tenancies-container', $rootElement));
                    }
