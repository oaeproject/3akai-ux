            ...
            {for item in items}
                ...
                {else}
                    ...
                    {else}
                        ...
                        <div class="${embedClass} ${layout} {if embedmethod === 'thumbnail'} s3d-draggable-container {/if}" data-canshare="${canShare}" data-scope="content" data-entityid="${item.fullresult._path}" {if sakai.api.Content.Collections.isCollection(item.fullresult)} data-collection="true" {/if}>
                            ...
                            {/if}
                            <div class="embedcontent_details">
                                {if name}<span><a href="${item.contentProfileLink}" class="s3d-action embedcontent_include_name">${item.name}</a></span>{/if}
                                {if details}<span class="embedcontent_include_details">${item.description|safeOutput}</span>{/if}
                                {if download}
                                    <span>
                                    {if item._mimeType === 'x-sakai/document'}
                                        <a href="${item.downloadLink}" class="s3d-action embedcontent_include_download" target="_blank">__MSG__VIEW_DOCUMENT__</a>
                                    {elseif sakai.api.Content.Collections.isCollection(item)}
                                        <a href="${item.downloadLink}" class="s3d-action embedcontent_include_download" target="_blank">__MSG__VIEW_COLLECTION__</a>
                                    {elseif item._mimeType === 'x-sakai/link'}
                                        <a href="${item.downloadLink}" class="s3d-action embedcontent_include_download" target="_blank">__MSG__OPEN_LINK__</a>
                                    {else}
                                        <a href="${item.downloadLink}" class="s3d-action embedcontent_include_download" target="_blank">__MSG__DOWNLOAD__</a>
                                    {/if}
                                    </span>
                                {/if}
                            </div>
...
<div id="embedcontent_settings" style="display:none">
    <div class="fl-widget-content s3d-widget-content">
        <div id="embedcontent_settings_container">
            ...
            <ul class="s3d-tabs">
                <li><button id="embedcontent_tab_choose" class="s3d-link-button s3d-tabs-active">__MSG__CHOOSE_CONTENT__</button></li>
                <li><button id="embedcontent_tab_display" class="s3d-link-button">__MSG__DISPLAY_SETTINGS__</button></li>
            </ul>
            <div class="s3d-tabs-content-container">
                <div id="embedcontent_tab_choose_content" class="tab_content_active">
                    ...
                    <div class="embedcontent_footer">
                        <div class="fl-force-right">
                            ...
                            <button type="button" class="s3d-link-button s3d-bold embedcontent_dont_add">__MSG__CANCEL__</button>
                            <button type="button" id="embedcontent_just_add" class="s3d-button s3d-overlay-action-button" disabled="disabled">__MSG__ADD_SELECTED_CONTENT__</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div id="embedcontent_tab_display_content" style="display:none">
                        ...
                        <div class="embedcontent_footer">
                            <div class="fl-force-right">
                                ...
                                <button type="button" class="s3d-bold s3d-link-button embedcontent_dont_add">__MSG__CANCEL__</button>
                                <button type="button" id="embedcontent_button_add_selected_content" class="s3d-button s3d-overlay-action-button" disabled="disabled">__MSG__ADD_SELECTED_CONTENT__</button>
                            </div>
                        </div>
                    </form>
                </div>
