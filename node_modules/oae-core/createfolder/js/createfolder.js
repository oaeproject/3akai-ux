    ...
    return function (uid) {
        ...
        var setUpSetPermissions = function() {
            ...
            $(document).on('oae.setpermissions.changed.' + setPermissionsId, function(ev, data) {
                ...
                // TODO: Make the current context an editor instead of a manager.
                // As currently only managers can add items to a folder, we make them managers
                if (contextData.id !== oae.data.me.id) {
                    managers.push(contextData.id);
                }

                members = _.without(data.viewers, contextData.id);
