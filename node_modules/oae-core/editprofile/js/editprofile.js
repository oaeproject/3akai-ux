...
define(['jquery', 'oae.core'], function ($, oae) {
    ...
    return function (uid) {
        ...
        var setUpEditProfile = function() {
         ...
         var disableModification = false;
           var EnableModification = true;
           var authenticationStrategy = oae.data.me.authenticationStrategy;
           if ((authenticationStrategy === 'cas')| (authenticationStrategy === 'shibboleth')){
            if (oae.api.config.getValue('oae-authentication', 'shibboleth', 'enabled')){
                if (oae.api.config.getValue('oae-authentication', 'shibboleth', 'disableProfile')){
                    var disableModification = true;
                    var EnableModification = false;
                }
            }
            if (oae.api.config.getValue('oae-authentication', 'cas', 'enabled')){
                if (oae.api.config.getValue('oae-authentication', 'cas', 'disableProfile')){
                    var disableModification = true;
                    var EnableModification = false;
                }
            }
            ...
            oae.api.util.template().render($('#editprofile-template', $rootel), {
                'isEnabled':EnableModification,
                'isDisabled':disableModification
            }, $('.modal-body', $rootel));
        ...
        var editProfile = function() {
            ...
            oae.api.user.updateUser(params, function (err, data) {
                if (!err) {
                    // If the update succeeded, trigger the `oae.editprofile.done` event,
