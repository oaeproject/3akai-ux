...
define(['jquery', 'oae.core'], function ($, oae) {
    ...
    return function (uid) {
        ...
        // Holds an email address that is pending verification from the user, if applicable
        var unverifiedEmail = null;

        /**
         * Render the edit profile form and initialize its validation
         */
        var setUpEditProfile = function() {
        ...
        var editProfile = function() {
            ...
            var isEmailChange = (oae.data.me.email !== newEmail);
            oae.api.user.updateUser(params, function (err, data) {
                if (!err) {
                    // Notify the rest of the UI that the profile has been updated
                    $(document).trigger('oae.editprofile.done', data);

                    if (!isEmailChange) {
                        // If the update succeeded and didn't have an email change, trigger the
                        // `oae.editprofile.done` event, show a success notification and close the
                        // modal
                        ...
                        oae.api.util.template().render($('#editprofile-email-template', $rootel), {
                            'unverifiedEmail': newEmail
                        }, $('#editprofile-email-container', $rootel));

                        // Switch to the next view to instruct the user how to proceed with their
                        // new email
                        $('#editprofile-panel-main').hide();
                        $('#editprofile-panel-email').show();
                    }
                } else {
        ...
        var setUpReset = function() {
            $('#editprofile-modal', $rootel).on('shown.bs.modal hidden.bs.modal', function (evt) {
                // Reset the form
        ...
        /**
         * Initialize the edit profile modal dialog
         */
        var setUpEditProfileMainPanel = function() {
            $(document).on('click', '.oae-trigger-editprofile', function() {
            ...
            $rootel.on('change', '#editprofile-panel-main .oae-large-options-container input[type="radio"]', function() {
            ...
            $('#editprofile-form', $rootel).on(oae.api.util.getFormChangeEventNames(), function() {
                $('#editprofile-panel-main button[type="submit"]', $rootel).prop('disabled', false);
            });
            ...
            $rootel.on('click', '#editprofile-email-verification button', function() {
                // Disable all actions in the modal
                $('#editprofile-form *', $rootel).prop('disabled', true);
                oae.api.user.resendEmailToken(function(err) {
                    if (!err) {
                        // If the token resent successfully show a notification
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__PROFILE_VERIFICATION_EMAIL_SENT__', 'editprofile'),
                            oae.api.i18n.translate('__MSG__PROFILE_A_VERIFICATION_EMAIL_HAS_BEEN_SENT_TO_UNVERIFIED_EMAIL__', 'editprofile', {
                                'unverifiedEmail': unverifiedEmail
                            }));
                    } else {
                        // If the token failed to resend, show a notification
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__PROFILE_VERIFICATION_EMAIL_FAILED__', 'editprofile'),
                            oae.api.i18n.translate('__MSG__PROFILE_A_VERIFICATION_EMAIL_FAILED_TO_BE_SENT_TO_UNVERIFIED_EMAIL__', 'editprofile', {
                                'unverifiedEmail': unverifiedEmail
                            }),
                            'error');
                    }
            ...
            $rootel.on('click', '#editprofile-email-verification .editprofile-email-verification-cancel a', function() {
                oae.api.user.deletePendingEmailVerification(function(err) {
                    if (!err) {
                        ...
                        // If the token failed to resend, show a notification
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__PROFILE_CANCEL_EMAIL_VERIFICATION_FAILED__', 'editprofile'),
                            oae.api.i18n.translate('__MSG__PROFILE_AN_ERROR_OCCURRED_WHILE_CANCELLING_THE_EMAIL_VERIFICATION__', 'editprofile'),
                            'error');
                    }
                });
            });
        ...
        var setUpEditProfileEmailPanel = function() {
            $rootel.on('click', '#editprofile-panel-email button', function() {
                $('#editprofile-modal', $rootel).modal('hide');
            });
        };

        setUpReset();
        setUpEditProfileMainPanel();
        setUpEditProfileEmailPanel();
    };
});
