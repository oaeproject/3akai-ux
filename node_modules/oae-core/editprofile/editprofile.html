<!-- CSS -->
<link rel="stylesheet" type="text/css" href="css/editprofile.css" />

<!-- MODAL -->
<div id="editprofile-modal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="editprofile-modal-title" aria-hidden="true">
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&#215;</button>
        <h3 id="editprofile-modal-title">__MSG__EDIT_DETAILS__</h3>
    </div>
    <form id="editprofile-form">
        <div class="modal-body"><!-- --></div>
        <div class="modal-footer">
            <button type="button" class="btn btn-link pull-left" data-dismiss="modal">__MSG__CANCEL__</button>
            <button type="submit" class="btn btn-primary pull-right">__MSG__EDIT_DETAILS__</button>
        </div>
    </form>
</div>

<div id="editprofile-template"><!--
    <div class="control-group">
        <label for="editprofile-name"><h4>__MSG__DISPLAY_NAME__</h4></label>
        <div class="controls">
            <input type="text" id="editprofile-name" class="required" name="editprofile-name" placeholder="__MSG__ENTER_A_DISPLAY_NAME__" value="${userProfile.displayName|encodeForHTMLAttribute}"/>
        </div>
    </div>
    <div class="control-group">
        <label for="editprofile-email"><h4>__MSG__EMAIL__</h4></label>
        <div class="controls">
            <input type="text" id="editprofile-email" class="required email" name="editprofile-email" placeholder="__MSG__ENTER_AN_EMAIL_ADDRESS__" value="${userProfile.email|encodeForHTMLAttribute}"/>
        </div>
    </div>
    <h4>__MSG__PROFILE_VISIBILITY__</h4>
    {var tenant = oae.api.util.security().encodeForHTML(userProfile.tenant.displayName)}
    <div class="oae-large-options-container row-fluid">
        <label for="oae-visibility-private" class="span4 text-center">
            <div class="oae-large-options well pull-left text-center {if userProfile.visibility === 'private'} checked{/if}">
                <input type="radio" id="oae-visibility-private" value="private" name="oae-visibility-group" class="pull-left" {if userProfile.visibility === 'private'} checked="checked"{/if} tabindex="0"/>
                <i class="icon-oae-private large text-center"></i>
                <span>__MSG__PRIVATE__</span>
            </div>
            <i class="icon-ok hide"></i>
            <small>__MSG__PROFILE_PRIVATE_DESCRIPTION__</small>
        </label>
        <label for="oae-visibility-loggedin" class="span4 text-center">
            <div class="oae-large-options well pull-left text-center {if userProfile.visibility === 'loggedin'} checked{/if}">
                <input type="radio" id="oae-visibility-loggedin" value="loggedin" name="oae-visibility-group" class="pull-left" {if userProfile.visibility === 'loggedin'} checked="checked"{/if} tabindex="0"/>
                <i class="icon-oae-loggedin large text-center"></i>
                <span>${tenant}</span>
            </div>
            <i class="icon-ok hide"></i>
            <small>__MSG__PROFILE_LOGGEDIN_DESCRIPTION__</small>
        </label>
        <label for="oae-visibility-public" class="span4 text-center">
            <div class="oae-large-options well pull-left text-center {if userProfile.visibility === 'public'} checked{/if}">
                <input type="radio" id="oae-visibility-public" value="public" name="oae-visibility-group" class="pull-left" {if userProfile.visibility === 'public'} checked="checked"{/if} tabindex="0"/>
                <i class="icon-oae-public large text-center"></i>
                <span>__MSG__PUBLIC__</span>
            </div>
            <i class="icon-ok hide"></i>
            <small>__MSG__PROFILE_PUBLIC_DESCRIPTION__</small>
        </label>
    </div>
--></div>

<div id="editprofile-notification-template"><!--
    {if err}
        __MSG__PROFILE_DETAILS_EDIT_FAIL__
    {else}
        __MSG__PROFILE_DETAILS_EDIT_SUCCESS__
    {/if}
--></div>

<!-- JAVASCRIPT -->
<script type="text/javascript" src="js/editprofile.js"></script>
