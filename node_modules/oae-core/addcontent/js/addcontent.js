...
define(['jquery', 'underscore', 'oae/api/oae.core', 'jquery-plugins/jquery.fileupload', 'jquery-plugins/jquery.iframe-transport'], function($, _, oae) {
    ...
    return function(uid, showSettings) {
        ...
        console.log('INITIALIZING ADD CONTENT');


        var init = function() {
            console.log('IN INITIALIZE');
        }
        $(document).on('click', '.oae-trigger-addcontent', init);
        ...
        var addcontentExistingItemsListContainerList = '#addcontent_existingitems_list_container_list';
        var addcontentAddLinkTemplate = '#addcontent_add_link_template';
        var addcontentAddExistingTemplate = '#addcontent_add_existing_template';
        ...
        var addcontentExistingItemsTemplate = 'addcontent_existingitems_template';
        ...
        var $addcontentExistingItemsSearch = $('.addcontent_existingitems_search');
        ...
        var addcontentExistingItemsListContainerListItemIcon = '.addcontent_existingitems_list_container_list_item_icon';
        var addcontentExistingItemsListContainerActionsSort = '#addcontent_existingitems_list_container_actions_sort';
        var addcontentExistingItemsListContainerCheckboxes = '#addcontent_existingitems_list_container input[type=\'checkbox\']';
        ...
        var addcontentAddExistingSearchButton = '#addcontent_add_existing_template .oae-search-button';
        ...
        var addcontentContainerNewItemAddToListExistingContentposition = 'addcontent_container_newitem_add_to_list_existingcontentposition';
        ...
        var addcontentExistingItemsListContainerDisabledListItem = 'addcontent_existingitems_list_container_disabled_list_item';
        ...
        var $addcontentAddLinkForm = $('#addcontent_add_link_form');
        var addcontentExistingContentForm = '#addcontent_existing_content_form';
        var addcontentAddDocumentForm = '#addcontent_add_document_form';
        var addcontentExistingCheckAll = '#addcontent_existingitems_list_container_actions_checkall';
        ...
        var greyOutExistingInLibrary = function() {
            currentSelectedLibrary = $(addcontentSaveTo).val();
            renderQueue();
        ...
        var removeItemToAdd = function() {
            ...
            switch (obj.type) {
                ...
                case 'existing':
                    ...
                    $existing.parent().removeClass(addcontentExistingItemsListContainerDisabledListItem);
                    break;
            }
        ...
        var constructItemToAdd = function() {
            ...
            switch ($thisForm.attr('id')) {
                ...
                case 'addcontent_existing_content_form':
                    var contentToAdd = [];
                    $.each($thisForm.find('.addcontent_existingitems_select_checkbox:checked'), function(index, item) {
                        if (!$(item).is(':disabled')) {
                            var viewers = [];
                            ...
                            var contentObj = {
                                ...
                                'type': 'existing',
                                'css_class': $(item).next().children(addcontentExistingItemsListContainerListItemIcon)[0].id
                            };
                            contentToAdd.push(contentObj);
                            $(item).attr('disabled', 'disabled');
                            $(item).parent().addClass(addcontentExistingItemsListContainerDisabledListItem);
                        }
                    });
       ...
       var fileDropped = function(file) {
        ...
        var uncheckCheckboxes = function() {
            ...
            $(addcontentExistingItemsListContainerCheckboxes).removeAttr('checked');
            // Uncheck the check all checkbox
            $(addcontentExistingCheckAll).removeAttr('checked');
        };
        ...
        var checkFieldValidToAdd = function() {
            if ($(this).attr('type') === 'text') {
                ...
                if ($(addcontentExistingContentForm + ' input[type=checkbox]:checked:enabled').length) {
                    enableAddToQueue();
                } else {
                    disableAddToQueue();
                }
        ...
        var searchAndRenderExistingContent = function($container, q, pagenum) {
            pagenum = pagenum || 1;
            var searchURL = '';
            var sortOrder = $(addcontentExistingItemsListContainerActionsSort + ' option:selected').attr('data-sort-order');
            var sortOn = $(addcontentExistingItemsListContainerActionsSort + ' option:selected').attr('data-sort-on');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $container.html(sakai.api.Util.TemplateRenderer(addcontentExistingItemsTemplate, {'data': data, 'query':q, 'sakai':sakai, 'queue':existingIDs, 'context':currentExistingContext}));
                    // Disable the add button
                    disableAddToQueue();
                    var numberOfPages = Math.ceil(data.total / 10);
                    $('#addcontent_existingitems_paging').pager({
                        pagenumber: pagenum,
                        pagecount: numberOfPages,
                        buttonClickCallback: searchPaging
                    });
                    if (numberOfPages > 1) {
                        $('#addcontent_existingitems_paging').show();
                    } else {
                        $('#addcontent_existingitems_paging').hide();
                    }
                },
                error: function(err) {
        ...
        var renderExistingContent = function(q, pagenum) {
            ...
            switch(currentExistingContext) {
                case 'everything':
                    showSelectedItem($(addcontentAddExistingTemplate));
                    searchAndRenderExistingContent($(addcontentExistingItemsListContainerList), q, pagenum);
                    break;
                case 'my_library':
                    showSelectedItem($(addcontentAddExistingTemplate));
                    searchAndRenderExistingContent($(addcontentExistingItemsListContainerList), q, pagenum);
        ...
        var checkUncheckAll = function() {
            if ($(addcontentExistingCheckAll).is(':checked')) {
                $('.addcontent_existingitems_select_checkbox:enabled', $(addcontentExistingItemsListContainerList)).attr('checked', 'checked');
            } else {
                $('.addcontent_existingitems_select_checkbox:enabled', $(addcontentExistingItemsListContainerList)).removeAttr('checked');
            }
        ...
        var prepareContentSearch = function(pagenum) {
            var query = $.trim($addcontentExistingItemsSearch.val());
        ...
        var searchExistingContent = function(ev) {
            if (ev.keyCode === 13 || ev.currentTarget.id === 'addcontent_existing_search_button') {
        ...
        var navigateMenu = function() {
            ...
            $addcontentContainerNewItemAddToList.removeClass(addcontentContainerNewItemAddToListExistingContentposition);
            ...
            switch ($(this)[0].id) {
                ...
                default: // No ID found on class -> subnav present
                    switch ($(this).children('ul').children(addcontentContainerLHChoiceSelectedSubitem)[0].id) {
                        case 'addcontent_existing_content_everything':
                            currentExistingContext = 'everything';
                            renderExistingContent($addcontentExistingItemsSearch.val());
                            $addcontentContainerNewItemAddToList.addClass(addcontentContainerNewItemAddToListExistingContentposition);
                            break;
                        case 'addcontent_existing_content_my_library':
        ...
        var removeBinding = function() {
            ...
            $(addcontentExistingItemsListContainerActionsSort).off('change');
            $addcontentContainer.off('click', '#addcontent_existingitems_paging .sakai_pager button');
        ...
        var addBinding = function() {
            ...
            $addcontentExistingItemsSearch.keydown(searchExistingContent);
            $(addcontentAddExistingSearchButton).click(searchExistingContent);
            $(addcontentExistingContentForm).on('click', 'input', checkFieldValidToAdd);
            $(addcontentExistingCheckAll).on('change', checkUncheckAll);
            $(addcontentExistingItemsListContainerActionsSort).on('change', function() {searchPaging(1);});
            $addcontentContainer.on('change', addcontentSaveTo, greyOutExistingInLibrary)
            $addcontentContainer.on('click', '#addcontent_existingitems_paging .sakai_pager button', function() {
                return false;
            });
