...
define(['jquery', 'underscore', 'oae.core', 'jquery-plugins/jquery.fileupload', 'jquery-plugins/jquery.iframe-transport'], function($, _, oae) {
    ...
    return function(uid, showSettings) {
        ...
        var addContentToList = function() {
            ...
            if ($('#addcontent_upload_content_container').is(':visible')) {
                contentObj = {
                    'name': $('#addcontent_upload_content_title').val(),
        ...
        var setUpAddContent = function() {
            $(document).on('click', '#addcontent_done_add_collected', function() {
                ...
                $.each(contentToAdd, function(index, item) {
                    if (item.type === 'file') {
                        oae.api.content.createFile(item.name, item.description, item.visibility, $('#addcontent_file_upload'), item.file, [], [libraryToAdd], finishedAdd);
                    } else if (item.type === 'link') {
                        oae.api.content.createLink(item.name, item.description, item.visibility, item.link, [], [libraryToAdd], finishedAdd);
                    } else if (item.type === 'sakaidoc') {
                        oae.api.content.createSakaiDoc(item.name, item.description, item.visibility, [], [libraryToAdd], finishedAdd);
                    }
                });
            });
        };
