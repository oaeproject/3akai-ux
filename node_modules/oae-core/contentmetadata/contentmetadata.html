<!-- CSS -->
<link rel="stylesheet" type="text/css" href="/node_modules/oae-core/contentmetadata/css/contentmetadata.css" />

<div class="oae-widget-container" id="contentmetadata_container" role="complementary" aria-labelledby="contentmetadata_aria_label_details">
    <h2 class="oae-contentpage-title metadata" id="contentmetadata_aria_label_details">__MSG__DETAILS__</h2>
    ...
    <div class="oae-widget-footer">
        <button type="button" id="contentmetadata_show_more" class="oae-button oae-link-button oae-action">
    ...
    <span class="contentmetadata_second_column">
        {if $.trim(data.description)}
            {if data.isManager || data.isEditor}
                <div class="contentmetadata_description_textarea contentmetadata_edit_input">${data.description|safeOutput}</div>
            {else}
                ${data.description|safeOutput}
            {/if}
        {else}
            {if data.isManager || data.isEditor}
        ...
        {else}
            ...
            {if tags && tags.length > 0}
                <span class="contentmetadata_second_column">
                    {for tag in tags}
                        {if tag_index > 0},{/if}
                        <a href="${tag.link}" class="oae-action" title="${tag.linkTitle}">${tag.value}</a>
                    {/for}
                </span>
            {else}
                {if data.isManager || data.isEditor}
        ...
        {if $.trim(data.link)}
            {if data.isManager || data.isEditor}
                <div class="contentmetadata_url_textarea contentmetadata_edit_input">
                    <a class="oae-action" target="_blank" href="${data.link}">
                        ${data.link}
                    </a>
                </div>
            {else}
                <a class="oae-action" target="_blank" href="${data.link}">
                    ${data.link|safeOutput}
                </a>
            {/if}
        {else}
            {if data.isManager || data.isEditor}
...
<div id="contentmetadata_copyright_template" style="display:none;"><!--
    <span class="contentmetadata_first_column">__MSG__COPYRIGHT_COLON__</span>
    <span class="contentmetadata_second_column">
        {for copyrightLabel in sakai.config.Permissions.Copyright.types}
            {if data.copyright === copyrightLabel_index}
...
<div id="contentmetadata_details_template" style="display:none;"><!--
        ...
        {if sakai.api.User.getDisplayName(creator)}
            <div class="contentmetadata_content_row">
                ...
                <span>
                    <a href="${creator.homePath}" class="oae-regular-links" title="__MSG__VIEW__ ${sakai.api.User.getDisplayName(creator)}'s __MSG__PROFILE__">
        ...
        <div class="contentmetadata_content_row">
            ...
            <span>${sakai.api.l10n.transformDate(sakai.api.l10n.fromEpoch(data.created), sakai.data.me)}</span>
        </div>
        <div class="contentmetadata_content_row">
            <span class="contentmetadata_first_column">__MSG__LAST_MODIFIED__ </span>
            <span>${sakai.api.l10n.transformDate(sakai.api.l10n.fromEpoch(data.lastModified), sakai.data.me)}</span>
        </div>
--></div>

<!-- JAVASCRIPT -->
<script type="text/javascript" src="/node_modules/oae-core/contentmetadata/javascript/contentmetadata.js"></script>