...
casper.test.begin('Widget - Content Shared', function(test) {
    ...
    var verifyContentSharedElements = function(user1, user2) {
        ...
        casper.waitForText(user1.displayName, function() {
            ...
            casper.waitForText(user2.displayName, function() {
    ...
    casper.start(configUtil().tenantUI, function() {
        ...
        casper.then(function() {
            userUtil().doLogIn(users[0].username, configUtil().defaultUserPassword);
        });

        var contentURL = null;
        casper.then(function() {
            casper.echo('Verify open content shared modal', 'INFO');
            contentUtil().createFile(null, null, [users[1].id, users[2].id, users[3].id, users[4].id, users[5].id, users[6].id, users[7].id, users[8].id], function(_contentURL) {
                ...
                casper.then(function() {
                    userUtil().doLogIn(users[1].username, configUtil().defaultUserPassword);
                });
