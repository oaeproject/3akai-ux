...
<div id="topnavigation_left_loggedin_template"><!--
    <ul role="menubar">
        <li>
            <a href="#" title="Dashboard" id="topnav-home" role="menuitem">
...
<div id="topnavigation_left_anonymous_template"><!--
    <a href="#" title="__MSG__SIGN_UP__" id="topnav-links">__MSG__SIGN_UP__</a>
--></div>
...
<div id="topnavigation_right_anonymous_template"><!--
    ...
    <a href="#" title="Sign in" class="oae-force-right" id="topnav-links">__MSG__SIGN_IN__</a>
    <form id="topnav-search-form" class="oae-force-right">
        <input type="text" id="topnav-search-input" class="oae-force-right"/>
    </form>

    <div class="oae-popup-white topnav-signin oae-form-field-wrapper">
        <div id="topnavigation_user_options_login_fields" class="{if hasExternalAuth && hasLocalAuth}topnavigation-local-and-external {/if}">
            <div id="topnav_3rd_party_login_container" {if !hasExternalAuth} class="oae-hidden" {/if}>
            ...
            <div id="topnav_local_login_container" {if !hasLocalAuth} class="oae-hidden" {/if}>
                <form id="topnavigation_user_options_login_form" novalidate="novalidate">
                    <label class="oae-input-label-above" for="topnavigation_login_username">Username</label>
                    <input type="text" class="topnavigation-login-container-textfield oae-input required" name="topnavigation_login_username" id="topnavigation_login_username" maxlength="255">
                    <label class="oae-input-label-above" for="topnavigation_login_password">Password</label>
                    <input type="password" class="topnavigation-login-container-textfield oae-input required" name="topnavigation_login_password" id="topnavigation_login_password" maxlength="255">
                    <hr class="oae-split-line">
                    <button id="topnavigation_user_options_login_button_login" class="oae-button oae-overlay-button oae-force-right" type="submit">Sign in</button>
                    <span class="oae-hidden" id="topnavigation_user_options_login_button_signing_in" class="oae-force-right">__MSG__SIGNING_IN__...</span>
...
<div id="search_template" style="display:none"><!--
    <a href="/search#l=all&q=${query}" class="topnavigation-search-results-section-header">__MSG__SEARCH__</a>
    <ul role="menu">
       {for r in data.results}
            <li>
                <div class="topnav_content_icon_container fl-force-left">
                    {if r.resourceType === 'user'}
                        <span class="topnav_person_icon oae-force-left"></span>
                    {elseif r.resourceType === 'group'}
                        <span class="topnav_content_icon oae-visibility-${r.visibility} oae-force-left"></span>
                    {elseif r.resourceType === 'content'}
                        <span class="topnav_content_icon oae-visibility-${r.visibility} oae-force-left"></span>
                    {/if}
                </div>
                {if r.resourceType === 'user'}
                    <a role="menuitem" tabindex="-1" href="/user/${r.id}" title="${r.displayName|safeUserInput}" class="oae-bold oae-threedots">${r.displayName|safeUserInput}</a>
                {elseif r.resourceType === 'group'}
                    <a role="menuitem" tabindex="-1" href="/group/${r.id}" title="${r.displayName|safeUserInput}" class="oae-bold oae-threedots">${r.displayName|safeUserInput}</a>
                {elseif r.resourceType === 'content'}
                    <a role="menuitem" tabindex="-1" href="/content/${r.id}" title="${r.displayName|safeUserInput}" class="oae-bold oae-threedots">${r.displayName|safeUserInput}</a>
                {/if}
            </li>
...
<div id="search_bottom_template" style="display:none;"><!--
    {if data.total}
        <a href="/search#l=all&q=${query}" id="topnavigation_search_see_all" class="oae-button oae-overlay-button">
            __MSG__SEE_ALL__
        </a>
    {/if}
