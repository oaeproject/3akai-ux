<link rel="stylesheet" type="text/css" href="/node_modules/oae-core/topnavigation/css/topnavigation.css"/>
...
<div id="topnavigation_container" role="navigation" aria-label="__MSG__TOPNAV_ARIA_LABEL__">
    ...
    <div class="topnavigation_right">
        <div class="topnavigation_search oae-search-container" role="search" aria-label="__MSG__TOPNAV_SEARCH_ARIA_LABEL__">
...
<div id="navigation_template" style="display:none"><!--
        ...
        {if type === "messages"}
            <div id="topnavigation_user_messages_container" class="topnavigation_counts_container">
                ...
                <button class="topnavigation_menuitem_counts_container" id="topnavigation_messages_container" title="__MSG__LATEST_MESSAGE__">
                    <div id="topnavigation_user_inbox_icon" class="topnavigation_menuitem_icon"><span class="oae-aural-text">__MSG__LATEST_MESSAGE__</span></div>
                    <div id="topnavigation_user_inbox_messages" class="topnavigation_menuitem_counts oae-bold"></div>
                </button>
                <div class="oae-dropdown-menu" style="display:none;"></div>
            </div>
        {elseif type === "collections"}
            <div id="topnavigation_user_collections_container" class="topnavigation_counts_container">
                ...
                <button class="topnavigation_menuitem_counts_container inserter_toggle" title="__MSG__MY_COLLECTIONS__">
                    <div id="topnavigation_user_collections_icon" class="topnavigation_menuitem_icon"><span class="oae-aural-text">__MSG__MY_COLLECTIONS__</span></div>
                    <div id="topnavigation_user_collections_total" class="topnavigation_menuitem_counts oae-bold">${"" + (sakai.api.Content.Collections.getMyCollectionsCount() + 1)}</div>
                </button>
            </div>
        {/if}
    {/macro}
    ...
    {for i in links}
        <li class="oae-bold oae-dropdown-menu{if i.subnav} hassubnav{/if}">
...
<div id="topnavigation_messages_dropdown_template"><!--
    ...
    <div class="oae-dropdown-container">
        {if dataPresent}
            {var category = data.results[0].category === "invitation" ? "invitations" : "inbox"}
            <a href="/me#l=messages/${category}" class="oae-button oae-overlay-button" id="topnavigation_message_showall">__MSG__SEE_ALL__</a>
            <div class="oae-dropdown-menu-content-container topnavigation_message">
                ...
                <div id="topnavigation_message_sender" class="oae-bold">${data.results[0].from.name} <span>(${data.results[0].timeago})</span></div>
                <div id="topnavigation_message_subject" class="oae-bold">${sakai.api.Util.applyThreeDots(data.results[0].subject, 115)}</div>
                <div id="topnavigation_message_body">${sakai.api.Util.applyThreeDots(data.results[0].body_nolinebreaks, 255, null, null, true)}</div>
                <a href="/me#l=messages/${category}&message=${data.results[0].id}" class="oae-button oae-overlay-button" id="topnavigation_message_readfull">__MSG__READ_FULL_MESSAGE__</a>
                <hr class="oae-hidden oae-push"/>
            {else}
                <div class="oae-dropdown-menu-content-container no_messages">
                    <div class="oae-no-results-container">
                        <img src="/node_modules/oae-core/topnavigation/images/topnav_messages_demo.png" alt="__MSG__MESSAGES_DEMO__" id="message_video_preview"/>
                        <div class="oae-no-results-icon oae-no-results-people"></div>
                        <p>__MSG__YOUR_INBOX_IS_EMPTY__. <a class="oae-regular-links" href="/me#l=messages/inbox&newmessage=true">__MSG__SEND_A_MESSAGE__</a> __MSG__OR__ <a class="oae-regular-links" href="/search#l=people">__MSG__FIND_PEOPLE_LC__</a> __MSG__TO_EXPAND_YOUR_NETWORK__.</p>
                    </div>
            {/if}
        </div>
    </div>
...
<div id="topnavigation_user_template"><!--
    {macro externalAuthMenu(auth)}
        <ul role="menu" aria-hidden="true">
            {for i in auth.Authentication.external}
                <li>
                    <a tabindex="-1" role="menuitem" href="${i.url}" class="oae-bold topnavigation_external_login_link">${i.label}</a>
                </li>
            {/for}
            {if auth.internalAndExternal}
                <li>
                    <button tabindex="-1" role="menuitem" class="oae-bold oae-link-button" id="topnavigation_toggle_internal_login">__MSG__INTERNAL_LOGIN__</button>
                </li>
            {/if}
        </ul>
    {/macro}
    ...
    {macro internalAuthMenu(auth)}
        <form id="topnavigation_user_options_login_form" method="POST" {if auth.internalAndExternal}style="display:none"{/if}>
            <div id="topnavigation_user_options_login_fields_container" class="oae-dropdown-menu-content-container oae-form-field-wrapper">
                ...
                <input type="text" maxlength="255" id="topnavigation_user_options_login_fields_username" name="topnav_login_username" class="login-container-textfield oae-input" />
                <label for="topnavigation_user_options_login_fields_password" class="oae-input-label-above">__MSG__PASSWORD__</label>
                <input type="password" maxlength="255" id="topnavigation_user_options_login_fields_password" name="topnav_login_password" class="login-container-textfield oae-input" />
                ...
                {if auth.Authentication.allowInternalAccountCreation}
                    ...
                    <a class="oae-regular-links topnavigation_sign_up_link" href="/register">__MSG__SIGN_UP__</a>
                {/if}
    ...
    {if anon}
        <div class="oae-dropdown-menu{if auth.internalAndExternal} topnavigation_dual_login{/if}" id="topnavigation_user_options_login_wrapper">
    ...
    {else}
        <ul id="topnavigation_user_options_list" role="menu">
            <li class="oae-bold oae-dropdown-menu hassubnav">
                <a role="menuitem" aria-haspopup="true" href="/me#" id="topnavigation_user_options_name" class="oae-bold"></a>
                <div class="topnavigation_user_dropdown oae-dropdown-container" style="display:none;">
                    <ul role="menu" aria-hidden="true">
...
<!-- JAVASCRIPT -->
<script type="text/javascript" src="/node_modules/oae-core/topnavigation/javascript/topnavigation.js"></script>