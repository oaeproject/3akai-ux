...
define(['jquery', 'oae/api/oae.core'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        var initLeftMenu = function() {
            if (oae.data.me.anon) {
                oae.api.util.renderTemplate('#topnavigation-left-anonymous-template', null, $('#topnav-left'));
            } else {
                oae.api.util.renderTemplate('#topnavigation-left-loggedin-template', null, $('#topnav-left'));
            }
        };
        ...
        var initRightMenu = function() {
            if (oae.data.me.anon) {
                oae.api.util.renderTemplate('#topnavigation-right-anonymous-template', null, $('#topnav-right'));
            } else {
                oae.api.util.renderTemplate('#topnavigation-right-loggedin-template', null, $('#topnav-right'));
            }
        };
        ...
        var setUpLogout = function() {
            $rootel.on('click', '#topnav-signout', function() {
        ...
        var setUpSearch = function() {
            ...
            var doSearch = function() {
                $.ajax({
                    'url': '/api/search/general/all?limit=9&q=' + lastSearchVal,
                    'success': function(data) {
                        $('#topnavigation-search-results-container').html(oae.api.util.renderTemplate('#search-template', {
                            'data': data,
                            'query': $('#topnav-search-input').val()
                        }));
                        $('#topnavigation-search-results-bottom-container').html(oae.api.util.renderTemplate('#search-bottom-template', {
                            'data': data,
                            'query': $('#topnav-search-input').val()
                        }));
            ...
            var handleEnterKeyInSearch = function() {
                if ($('#topnavigation-search-results-container').find('li.selected').length) {
                    document.location = $('#topnavigation-search-results-container').find('li.selected a').attr('href');
                } else {
                    document.location = '/search#l=all&q=' + $.trim($('#topnav-search-input').val());
        ...
        var setUpLogin = function() {
            $rootel.on('click', '#topnav-signin-link', function() {
                $(this).toggleClass('selected');
                $('.topnav-signin').toggle();
                $('#topnavigation-login-username').focus();
            });
        ...
        var setUpLoginFocus = function() {
            ...
            $('#topnav-right', $rootel).focusin(function() {
                // Make sure the Sign in button is still active
                $('.oae-menu-dropdown', $('#topnav-right')).addClass('topnavigation-force-submenu-display-title');
                // Make sure the Login dropdown remains visible
                $('#topnavigation-user-options-login-fields').addClass('topnavigation-force-submenu-display');
            });
            // We hide the login dialog when we click outside of the dropdown
            $('html').on('click', function(ev) {
                if ($(ev.target).parents('#topnav-right').length === 0) {
                    // Make sure the Sign in button is inactive
                    $('.oae-menu-dropdown', $('#topnav-right')).removeClass('topnavigation-force-submenu-display-title');
                    // Make sure the Login dropdown becomes invisible
                    $('#topnavigation-user-options-login-fields').removeClass('topnavigation-force-submenu-display');
                }
            });
        };
        ...
        var addLoginValidation = function() {
            var $loginForm = $('#topnavigation-user-options-login-form', $rootel);
            var validateOpts = {
                'messages': {
                    'topnavigation-login-username': {
                    ...
                    'topnavigation-login-password': oae.api.i18n.translate('__MSG__PLEASE_ENTER_YOUR_PASSWORD__')
                },
                'methods': {
        ...
        var doLogin = function() {
            ...
            $('#topnavigation-user-options-login-button-login', $rootel).show();
            $('#topnavigation-user-options-login-button-signing-in', $rootel).hide();
            // Sign the user in
            oae.api.authentication.login($('#topnavigation-login-username', $rootel).val(), $('#topnavigation-login-password', $rootel).val(), function(err) {
                if (err) {
                    ...
                    $('#topnavigation-login-username', $rootel).addClass('topnavigation_failed_attempt');
                    $('#topnavigation-user-options-login-form', $rootel).valid();
                    $('#topnavigation-login-username', $rootel).removeClass('topnavigation_failed_attempt');
                    // Clear the password field
                    $('#topnavigation-login-password', $rootel).val('');
                    // Focus into the username field
                    $('#topnavigation-login-username', $rootel).focus();
                } else {
