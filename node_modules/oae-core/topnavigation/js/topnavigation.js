...
define(['jquery', 'oae.core', 'activityadapter'], function($, oae, ActivityAdapter) {
    ...
    return function(uid, showSettings) {
        ...
        /**
         * Use the current authentication strategies to determine some important characteristics
         * about how to offer a user their authentication method
         *
         * @return {Object}     authStrategyInfo                                Authentication strategy information
         * @return {Object}     authStrategyInfo.enabledStrategies              All the enabled strategies
         ...
         */
        var getAuthStrategyInfo = function() {
            var enabledStrategies = oae.api.authentication.getEnabledStrategies();
            var enabledStrategyNames = _.keys(enabledStrategies);

            // Classify all our known authentication strategies
            var localAuthStrategies = ['local', 'ldap'];
            var externalAuthStrategies = ['cas', 'facebook', 'google', 'googleApps', 'shibboleth', 'twitter'];
            var institutionalAuthStrategies = ['cas', 'googleApps', 'shibboleth'];

            var hasLocalAuth = (!_.chain(enabledStrategyNames).intersection(localAuthStrategies).isEmpty().value());
            var hasExternalAuth = (!_.chain(enabledStrategyNames).intersection(externalAuthStrategies).isEmpty().value());
            var hasInstitutionalAuth = (!_.chain(enabledStrategyNames).intersection(institutionalAuthStrategies).isEmpty().value());
        ...
        var initRightMenu = function() {
            if (oae.data.me.anon) {
                $('#topnavigation-right', $rootel).append(oae.api.util.template().render($('#topnavigation-right-anonymous-template', $rootel), {
                    'authStrategyInfo': getAuthStrategyInfo(),
        ...
        var setUpLogin = function() {
            ...
            // If the UI should suggest a login action, invoke the configured user action for login
         ...
         *
         * @return {String}     The URL the user should be sent to upon successful login
         */
        var getLoginRedirectURL = function() {
            var currentLocation = window.location.pathname;

            // $.parseurl will mysteriously run decodeURI on everything it gets, which messes up
            // proper URI decoding of redirect parameters. Offset this by first encoding it
            var redirectUrl = $.url(encodeURI(window.location.toString())).param('url');
        ...
        /**
         * Parse the invitation information out of the login redirect URL, if any
         *
         * @return {Object}     An object with keys `token` and `email`, referring to any invitation data that was held in the redirect url
         */
        var getInvitationInfo = function() {
            var redirectUrlStr = getLoginRedirectURL();

            // Do an `encodeURI` to offset the fact the `$.url` will run a `decodeURI` before doing
            // any parsing and break url-encoded query string parameters in the redirect url
            var parsedRedirectUrl = $.url(encodeURI(redirectUrlStr));
            var invitationInfo = {
                'email': parsedRedirectUrl.param('invitationEmail'),
                'token': parsedRedirectUrl.param('invitationToken')
            };
        ...
        /**
         * Determine if the UI should suggest a login action to the user
         *
         * @return {Boolean}    Truethy indicates the UI should suggest a login action. Falsey otherwise
         */
        var shouldSuggestLogin = function() {
        ...
        /**
         * Attempt to log the user in with the provided username and password onto the current tenant using either
        ...
        var doLogin = function() {
            ...
            var invitationInfo = getInvitationInfo();
            var loginOpts = {};
            if (invitationInfo.token) {
                loginOpts.invitationToken = invitationInfo.token;
            }

            // Both LDAP and local are enabled. We try LDAP first, and try local next if LDAP has failed
            if (ldapEnabled && localEnabled) {
                oae.api.authentication.LDAPLogin(username, password, loginOpts, function(err) {
                    if (err) {
                        oae.api.authentication.localLogin(username, password, loginOpts, finishLogin);
                    } else {
                        finishLogin();
                    }
                });
            // Only LDAP is enabled
                ...
                oae.api.authentication.LDAPLogin(username, password, loginOpts, finishLogin);
