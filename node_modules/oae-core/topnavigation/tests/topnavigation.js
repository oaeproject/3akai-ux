...
casper.test.begin('Widget - Top navigation', function(test) {
    ...
    var verifyLoggedInTopNavigation = function() {
        casper.waitForSelector('#topnavigation-left', function() {
            ...
            test.assertExists('#topnavigation-left a[href="/me"]', 'The \'Home\' button is present');
    ...
    casper.start(configUtil().tenantUI, function() {
        ...
        casper.then(function() {
            casper.echo('# Verify top navigation elemens as an anonymous user', 'GREEN_BAR');
            verifyAnonymousTopNavigation();
        });

        // Log in
        casper.then(function() {
            casper.echo('# Verify logging in through the top navigation', 'GREEN_BAR');
            verifyLogin(user1.username);
        });

        // Verify the top navigation elements as a logged in user
        casper.then(function() {
            casper.echo('# Verify top navigation elemens as a logged in user', 'GREEN_BAR');
            verifyLoggedInTopNavigation();
        });

        // Verify the top navigation search
        casper.then(function() {
            casper.echo('# Verify top navigation search', 'GREEN_BAR');
