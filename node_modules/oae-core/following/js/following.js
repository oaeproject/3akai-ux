...
define(['jquery', 'oae.core', 'jquery.history'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
         ...
         */
        var search = function() {
            var query = $('#following-search-query', $(this)).val();
            ...
            var url = $.url(History.getState().cleanUrl).attr('path') + '?q=' + oae.api.util.security().encodeForURL(query);
         ...
         * available action based on whether or not the user can manage this library.
         */
        var setUpListOptions = function() {
            if (oae.data.me.anon) {
                $('.oae-list-options-toggle', $rootel).prop('disabled', true);
            } else {
                oae.api.util.template().render($('#following-list-options-actions-template', $rootel), {
        ...
        /**
         * Get the users that follow me and render an infinitescroll list
         */
        var getFollowers = function() {
            ...
            $('#following-search-query', $rootel).val(query);

            var url = '/api/following/' + widgetData.principalId + '/followers';
            if (query) {
                url = '/api/search/followers/' + widgetData.principalId;
            }

            // Set up the infinite scroll for the content library
            followersInfinityScroll = $('#following-followers .oae-list', $rootel).infiniteScroll(url, {
                'limit': 12,
                'q': query
            }, '#followers-template', {
                ...
                'emptyListProcessor': function() {
                    oae.api.util.template().render($('#followers-noresults-template', $rootel), {
                        'query': query
                    }, $('#following-followers .oae-list', $rootel));
                }
            });
        };

        /**
         * Get the users I'm following and render an infinitescroll list
         */
        var getFollowing = function() {
            ...
            var url = '/api/following/' + widgetData.principalId + '/following';
            if (query) {
                url = '/api/search/following/' + widgetData.principalId;
            }

            // Set up the infinite scroll for the content library
            followingInfinityScroll = $('#following-following .oae-list', $rootel).infiniteScroll(url, {
                'limit': 12,
                'q': query
            }, '#following-template', {
                'postProcessor': function(data) {
                ...
                'emptyListProcessor': function() {
                    oae.api.util.template().render($('#following-noresults-template', $rootel), {
                        'query': query
                    }, $('#following-following .oae-list', $rootel));
                }
            });
        };

        /**
         * Add binding to various elements in the following widget
         */
        var addBinding = function() {
            // Set up search event
            $rootel.on('submit', '#following-search-form', search);

            // Listen to History.js state changes and listen to the event that indicates
            // that a search has been done
            $(window).on('statechange', function() {
                // Reload both lists with the search results
            ...
            $('a[data-toggle="tab"]', $rootel).on('shown', function(ev) {
                if ($(ev.target).attr('data-type') === 'followers') {
                    $('.oae-list-selectall', $rootel).prop('disabled', true);
                    $('.oae-trigger-unfollow', $rootel).prop('disabled', true);
                } else {
                    $('.oae-list-selectall', $rootel).prop('disabled', false);
                    if ($('.oae-list input[type="checkbox"]:checked', $rootel).length) {
                        $('.oae-trigger-unfollow', $rootel).prop('disabled', false);
                    }
                }
            });
