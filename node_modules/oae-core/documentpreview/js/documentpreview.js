...
define(['jquery', 'underscore', 'oae.core', 'lazyload'], function($, _, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        // Whether or not any pages are being preloaded for a requested page number
        var isPreloading = false;

        // Variable that keeps track of the width of the document viewer. This is used to
        // appropriately center the pages horizontally
        var contentWidth = 0;
        ...
        /**
         * When a specific page in the document is requested, all pages leading
         * up to that page that haven't been loaded yet are loaded before loading
         * the requested page.
         *
        ...
        var preloadPages = function(pageNumber, callback) {
            ...
            /**
             * Recursive function that will preload a page and will subsequently
             * load the next page to preload until all pages have been preloaded
             */
            var preloadPage = function() {
                // Find the first page in the overall set of pages to preload that hasn't been loaded yet
                var pageToLoad = _.find(pages, function(page) {
                    return page.loaded === false && (page.pageNumber < pageNumber);
                });
                if (pageToLoad) {
                    loadPage(pageToLoad, preloadPage);
                } else {
                    isPreloading = false;
        ...
        /**
         * Position all pages in the document so they are horizontally centred in the document
         * viewer. This is done by adjusting their width based on the width of the document viewer
         * and the current zoom level. When pages are wider than the available space in the viewer,
         * they will be aligned to the left of the viewer.
         */
        var positionAllPages = function() {
            // Keep track of the current width of the document viewer
            contentWidth = $content.width();
            // Adjust the width of all pages to horizontally center them in the viewer
        ...
        var loadPagesForInfiniteScrolling = function() {
            // If all pages have been loading, there's nothing more to do
        ...
        var scrollContent = function() {
            ...
            // Update the toolbar when the viewer has move on to a different page
        ...
        var detectVisiblePage = function() {
            ...
            var visiblePage = _.find(pages, function(page) {
                return page.$el && (page.$el.position().top + page.height * state.zoomLevel) >= 0;
            });
            return visiblePage;
        };
        ...
        var scrollToPage = function(page) {
            ...
            loadPage(page, function() {
                ...
                $content.scrollTop($content.scrollTop() + page.$el.position().top - 5);
        ...
        var pageInput = function() {
            ...
            if (!(_.isNaN(pageNumber)) && pageNumber >= 1 && pageNumber <= widgetData.previews.pageCount) {
                ...
                // Load all preceding pages that haven't been loaded yet
        ...
        /**
         * Zoom a page according to the document zoom level
         *
         * @param  {Object}         page                Page object representing the page that needs to be zoomed to the document zoom level
         */
        var zoomPage = function(page) {
            if (page.loaded) {
                // In order to zoom the page to the document zoom level, the `transform: scale` CSS property is used.
                // In order to make sure that the page is still horizontally centered, its width is adjusted based on
                // the width of the overall viewer. The height of the page is also changed according to the zooming scale,
                // so we maintain a consistent space between different pages
                page.$el.css({
        ...
        var changeZoom = function(zoom) {
            ...
            var currentZoom = state.zoomLevel;
        ...
        var loadStyleSheets = function() {
            // The stylesheets are loaded before the pages are loaded to avoid strange display issues
            // if any of the pages would be loaded before the stylesheets are loaded
        ...
        var addBinding = function() {
            ...
            // Page positioning
            $(window).on('resize', _.debounce(positionAllPages, 100));
        };
