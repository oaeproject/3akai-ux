...
define(['jquery', 'underscore', 'oae.core', 'lazyload', 'annotator', '/shared/oae/js/annotator-plugins/sidebar.js'], function($, _, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var $content = $('#documentpreview-content-container', $rootel);
        var $zoomIn = $('#documentpreview-zoom-in', $rootel);
        var $zoomOut = $('#documentpreview-zoom-out', $rootel);
        var $annotatorToggle = $('#documentpreview-toggle-annotator', $rootel);
        ...
        var loadPage = function(page, callback, _isRetryAttempt) {
            ...
            $.ajax({
                ...
                'success': function(response) {
                    ...
                    // Apply the annotator to the page
                    page.$el
                        .annotator()
                        .annotator('addPlugin', 'Store', {
                            'prefix': '/api/content/' + widgetData.id + '/annotations/' + widgetData.latestRevisionId,
                            'urls': {
                                'create': '',
                                'read': '?pageNumber=' + page.pageNumber,
                                'update': '/:id',
                                'destroy': '/:id'
                            },
                            annotationData: {
                        ...
                        .annotator('addPlugin', 'Sidebar', {
                            'container': $('#documentpreview-sidebar'),
                            'contentProfile': widgetData,
                            'editTemplate': $('#documentpreview-sidebar-edit-template', $rootel),
                            'userProfile': oae.data.me,
                            'viewTemplate': $('#documentpreview-sidebar-view-template', $rootel)
                        });
         ...
         */
        var resetPageLoadProgress = function() {
            $('.progress .progress-bar', $rootel).css('width', '0%').attr('aria-valuenow', 0);
            $('.progress .progress-bar .sr-only', $rootel).text('0%');
        ...
        var updatePageLoadProgress = function(progress) {
            $('.progress .progress-bar', $rootel).css('width', progress + '%').attr('aria-valuenow', progress);
            $('.progress .progress-bar .sr-only', $rootel).text(progress + '%');
        };
        ...
        var loadPages = function(pageNumber) {
            ...
            var loadNextPage = function(err) {
                ...
                updatePageLoadProgress(Math.round(((lastPageLoaded - startPageForBatch) / (pageNumber - startPageForBatch)) * 100));

                // Wait for the stack to clear before starting to load the next page. This is needed to enable
                // Chrome to animate the progress bar properly
                _.defer(function() {
        ...
        var changeZoom = function(zoom) {
            ...
            // Let other widgets know that the document has been zoomed
            $(document).trigger('oae.documentpreview.zoom', {'zoom': zoom});
        ...
        var setUpDocumentPreview = function() {
            ...
            // Enable the annotator button when the viewer has permissions to make
            // annotations
            if (widgetData.isManager || widgetData.isViewer) {
                $annotatorToggle.show();
            }
            updateToolbar();
