...
define(['jquery', 'underscore', 'oae.core'], function($, _, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        // How many ms to give browser to refresh DOM after insertion as
        // a fallback to animation end events
        var DOMREFRESHDELAY = 4000;
            ...
            if (!page.deferred) {
                ...
                page.deferred = $.ajax({
                    ...
                    'success': function(response) {
                        ...
                        // Trigger the animation so we can tell when the
                        // page is ready for positioning
                        page.$el.addClass('documentpreview-inserting');

                        // Set a last ditch timer in case the animation is
        ...
        var zoomPage = function(page) {
            if (page.$outer && page.$inner) {
                ...
                if (state.zoomLevel !== 100) {
                    page.$el.css('height', (page.$inner.outerHeight(true) * scale) + 'px');
                } else {
                    page.$el.css('height', '');
                }
         ...
         */
        $content.on('animationend oanimationend MSAnimationEnd webkitAnimationEnd', function(event) {
            event = event.originalEvent;
            if (event.animationName == 'documentPreviewPageInserted') {
                var pageNumber = $(event.target).attr('data-page-number');
                var page = pages[pageNumber-1];
                $(event.target).removeClass('documentpreview-inserting');
                alignPage(page);
                page.$el.css('opacity', '');
            }
