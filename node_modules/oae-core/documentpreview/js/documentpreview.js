...
define(['jquery', 'underscore', 'oae.core', 'lazyload'], function($, _, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        /**
         * Render the HTML content for a page
         *
         * @param  {Object}         page                Page object representing the page that will be loaded
         * @param  {String}         html                HTML content for the page
         */
        var renderPageHtml = function(page, html) {
            // Add the HTML content (overwriting the loading indicator)
            page.$html = $('.documentpreview-content-html-page', page.$el).html(html);
        ...
        /**
         * Render the text content for a page (for screen readers)
         *
         * @param  {Object}         page                Page object representing the page that will be loaded
         * @param  {String}         text                Text content for the page
         */
        var renderPageText = function(page, text) {
            // Add the text content
            page.$text = $('.documentpreview-content-text-page', page.$el)
            $('textarea', page.$text).text(text);
        };
        ...
        var loadPage = function(page, callback, _isRetryAttempt) {
            ...
            // Variables to track progress of HTML and text content loads
            var htmlDeferred = new $.Deferred();
            var textDeferred = new $.Deferred();
            ...
            // If it hasn't been loaded yet, request the page's HTML content
            if (!page.$html) {
                $.ajax({
                    ...
                    'success': function(response) {
                        // Render the HTML content for the page
                        renderPageHtml(page, response);
                        // Indicate that HTML content has been loaded
                        htmlDeferred.resolve();
                    },
                    'error': function(jqXHR, textStatus) {
                        // Indicate that HTML content load has failed
                        htmlDeferred.reject();
                ...
                // If HTML content is already present, indicate that it's there
                htmlDeferred.resolve();
            }

            // If it hasn't been loaded yet, request the page's text content
            if (!page.$text) {
                $.ajax({
                    ...
                    'dataType': 'text',
                    'success': function(response) {
                        // Render the text content for the page
                        renderPageText(page, response);

                        // Indicate that text content has been loaded
                        textDeferred.resolve();
                    },
                    'error': function(jqXHR, textStatus) {
                        // Indicate that text content load has failed
                        textDeferred.reject();
                        // Handle the specific load error
                        handlePageLoadError(page, callback, _isRetryAttempt, jqXHR);
                    }
                ...
                // If text content is already present, indicate that it's there
                textDeferred.resolve();
            }

            // When both HTML and text versions are complete, finish up
            $.when(htmlDeferred, textDeferred).done(function() {
