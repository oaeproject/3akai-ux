...
define(['jquery', 'underscore', 'oae.core', 'lazyload'], function($, _, oae) {
    return function(uid, showSettings, widgetData) {
        ...
        var ZOOMLEVELS = [0.33, 0.5, 0.75, 0.9, 1, 1.1, 1.25, 1.5, 2, 5];
        ...
        var loadPage = function(page, callback, _isRetryAttempt) {
            ...
            $spacer.before(
                oae.api.util
                    .template()
                    .render(
                        $('#documentpreview-content-page-template', $rootel),
                        {
            ...
            page.$el = $(
                '.documentpreview-content-page[data-page-number="' +
                    page.pageNumber +
                    '"]',
                $rootel,
            );
            ...
            $.ajax({
                url: constructDocumentPreviewURL(
                    'page.' + page.pageNumber + '.html',
                ),
                crossDomain: true,
                dataType: 'text',
        ...
        var loadPages = function(pageNumber) {
            ...
            var loadNextPage = function(err) {
                ...
                if (lastPageLoaded < pageNumber) {
                    ...
                    var spaceBelowRequestedPage =
                        $content.prop('scrollHeight') -
                        requestedPage.$el.position().top -
                        $content.height();
                    if (
                        lastPageLoaded < widgetData.previews.pageCount &&
                        spaceBelowRequestedPage < $content.height()
                    ) {
                        loadPage(pages[lastPageLoaded], loadNextPage);
                    } else {
                        stopLoadingPages();
        ...
        var updateToolbar = function() {
            ...
            $nextPage.prop(
                'disabled',
                state.pageNumber >= widgetData.previews.pageCount,
            );
        ...
        var zoomPages = function(pageNumber) {
            ...
            var widestPage = _.max(
                $('.documentpreview-content-page', $content),
                function(el) {
            ...
            $spacer.css({
                width:
                    _.isObject(widestPage) &&
                    $(widestPage).width() < widestPage.scrollWidth
                        ? state.zoomLevel * widestPage.scrollWidth +
        ...
        var loadStyleSheets = function() {
            ...
            LazyLoad.css(
                [constructDocumentPreviewURL('combined.css')],
                function() {
                    // Load the first page
                    loadPage(pages[0]);
                },
        ...
        var setUpDocumentPreview = function() {
            ...
            $('#documentpreview-page-count', $rootel).text(
                widgetData.previews.pageCount,
            );
