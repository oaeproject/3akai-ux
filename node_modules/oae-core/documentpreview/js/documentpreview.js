...
define(['jquery', 'underscore', 'oae.core', 'lazyload'], function($, _, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var scrollContent = function() {
            ...
            if (visiblePage) {
                // Detect when the viewer has moved on to a different page
                if (visiblePage.pageNumber !== state.pageNumber) {
                    state.pageNumber = visiblePage.pageNumber;
                }
                // Update toolbar for new page number and position
                updateToolbar();
            }
         ...
         * @param  {Object}     [options.disabled]    Disable all toolbar controls
         */
        var updateToolbar = function(options) {
            // Should all controls be disabled regardless of state
            var disabled = options && options.disabled;

            // Enable/disable the zooming and page navigations controls
            var zoomIndex = _.indexOf(ZOOMLEVELS, state.zoomLevel);
            $zoomOut.prop('disabled', disabled || (zoomIndex <= 0));
            $zoomIn.prop('disabled', disabled || (zoomIndex >= (ZOOMLEVELS.length - 1)));
            $prevPage.prop('disabled', disabled || (state.pageNumber === 1 && $content.scrollTop() === 0));
            $nextPage.prop('disabled', disabled || (state.pageNumber >= widgetData.previews.pageCount));
            $pageNumber.prop('disabled', !!disabled); // `!!` to handle undefined
        ...
        var changeZoom = function(zoom) {
            ...
            // Restore the scroll position; very top of document is a special
            // case because margin adjustment of prior pages doesn't apply
            if ($content.scrollTop() > 0) {
                var newTop = currentTop * (state.zoomLevel / currentZoom);
        ...
        var fullScreenChanged = function() {
            ...
            $fullScreen.find('i').toggleClass('icon-resize-small').toggleClass('icon-resize-full');
        };
