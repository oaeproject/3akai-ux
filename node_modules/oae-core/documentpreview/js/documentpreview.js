...
define(['jquery', 'underscore', 'oae.core'], function($, _, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        // How many pages to prefetch on initial loading
        var PREFETCHPAGECOUNT = 3;

        // How many ms to give browser to refresh DOM after insertion
        var DOMREFRESHDELAY = 250;
            ...
            if (!page.deferred) {
                ...
                page.deferred = $.ajax({
                    ...
                    'success': function(response) {
                        ...
                        // Don't show content while we get the page set up
                        ...
                        // Give the browser a chance to update the DOM before
                        // we get the properties of the inserted page
                        var interval = setInterval(function() {

                            // What size does the browser think the page is now?
                            var height = page.$inner.outerHeight(true);
                            var width = page.$inner.outerWidth(true);

                            // Has the browser settled on a size?
                            if ((page.naturalHeight === height) && (page.naturalWidth === width)) {

                                // Dimensions are stable, cancel the interval timer
                                clearInterval(interval);

                                // Show the page
                                zoomPage(page);

                                // Restore opacity now that the page is in place
                                page.$el.css('opacity', '1.0');
                            } else {

                                // Dimensions not yet stable, record current
                                // value and continue waiting for stability
        ...
        var alignPage = function(page) {
            ...
            if (page.$inner) {
                ...
                var contentWidth = page.naturalWidth * state.zoomLevel / 100;
                var left;
        ...
        $content.on('scroll', _.debounce(function() {
             ...
             */
            var viewingPage = _.chain(pages).filter(function(page) {
                return page.$el && page.$el.position().top <= 0;
            }).last().value();
            var pageNumber = viewingPage ? viewingPage.pageNumber : 1;
            ...
            /**
             * Check how close we are to the bottom of the current
             * page so, if we're getting close, we can load in
             * the next one
             */
            if (pageNumber < pages.length) {
                var pixelsRemaing = $content.prop('scrollHeight') -
                                    $content.height() -
                                    $content.scrollTop();
        ...
        /**
         * Preload a few additional pages. This will not only speed
         * things up for the user, it will also prevent a problem
         * where a single page doesn't take up the full content
         * container, which would otherwise prevent the user from
         * scrolling and automatically triggering the loading of
         * further content.
         */
        _.each(_.range(1, Math.min(PREFETCHPAGECOUNT, widgetData.previews.pageCount)), function(index) {
