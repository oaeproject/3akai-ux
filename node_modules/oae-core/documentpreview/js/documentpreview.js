...
define(['jquery', 'underscore', 'oae.core', 'lazyload'], function($, _, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var $progressbar = $('#documentpreview-progressbar', $rootel);
        ...
        /**
         * Prepare markup for a page to be loaded
         *
         * @param  {Object}         page                Page object representing the page that will be loaded
         */
        var prepareForPageLoading = function(page) {
            ...
            $spacer.before(oae.api.util.template().render($('#documentpreview-content-page-template', $rootel), {
                ...
                'totalPages': widgetData.previews.pageCount
            }));
            ...
            // Initially the height is just the height of the loading
            // message, but that's still critical for scaling and
            // scrolling correctly
        ...
        /**
         * Load and render the text content for a page (for screen readers)
         *
         * @param  {Object}         page                Page object representing the page that will be loaded
         */
        var loadPageText = function(page) {
            $.ajax({
                'url': constructDocumentPreviewURL('page.' + page.pageNumber + '.txt'),
                'crossDomain': true,
                'dataType': 'text',
                'success': function(response) {
                    page.$text = $('.documentpreview-content-text-page', page.$el);
                    $('textarea', page.$text).text(response);
                }
                // Don't handle error responses. The only recoverable response is a
                // signature expiry and loading the HTML content will catch that
            });
        };

        /**
         * Load and render the HTML content for a page
         *
         * @param  {Object}         page                Page object representing the page that will be loaded
         * @param  {Function}       callback            Standard callback function
         */
        var loadPageHtml = function(page, callback) {
            ...
            $.ajax({
                ...
                'dataType': 'text',
                'success': function(response) {
                    // Add the HTML content (overwriting the loading indicator)
                    page.$html = $('.documentpreview-content-html-page', page.$el).html(response);
                ...
                'error': function(jqXHR) {
                    // Return error information to callback
                    callback(jqXHR);
                }
            });
        ...
        /**
         * Handle page loading errors to recover from signature expiry
         *
         * @param  {Object}         page                Page object representing the page that will be loaded
         * @param  {Function}       callback            Standard callback function
         * @param  {Boolean}        _isRetryAttempt    `true` if we attempted to load this page previously
         * @param  {Object}         jqXHR               jQuery XHR from the failed load attempt
         */
        var handlePageLoadError = function(page, callback, _isRetryAttempt, jqXHR) {
            ...
            if (jqXHR.status === 401 && !_isRetryAttempt) {
                refreshSignature(function(err) {
                    if (err) {
                        callback(err);
                    }

                    // Try loading the page again
                    loadPage(page, callback, true);
                });
            }
        ...
        var loadPage = function(page, callback, _isRetryAttempt) {
            ...
            // Don't reload the page if all of its components have already been loaded
            if (page.$el && page.$html && page.$text) {
                return callback();
            }
            ...
            // Set up the HTML markup for the page
            prepareForPageLoading(page);

            // Kick off the request for the page's text content (for screen readers).
            // Load the text first so we can rely on the HTML load to catch signature
            // expiry and initiate a recovery
            if (!page.$text) {
                loadPageText(page);
            }
            ...
            if (!page.$html) {
                loadPageHtml(page, function(jqXHR) {
                    // Returned object indicates an error
                    if (jqXHR) {
                        // Handle the specific load error
                        handlePageLoadError(page, callback, _isRetryAttempt, jqXHR);
                    } else {
        ...
        /**
         * Enable the progress bar that indicates pages loading
         */
        var enablePageLoadProgress = function() {
            $progressbar.attr('aria-hidden', 'false').addClass('active');
        };

        /**
         * Update the progress bar that indicates pages loading
         *
         * @param  {Number}   progress   Number between 0 and 100 indicating the page load progress
         */
        var updatePageLoadProgress = function(progress) {
            progress = Math.round(progress) + '%';
            $progressbar.css('width', progress).attr('aria-valuenow', progress);
        };

        /**
         * Disable the progress bar that indicates pages loading
         */
        var disablePageLoadProgress = function() {
            $progressbar.attr('aria-hidden', 'true').removeClass('active');
        };
        ...
        var loadPages = function(pageNumber) {
            ...
            // Remember starting point for progress bar
            var startPageForBatch = lastPageLoaded;
            
            // Reset and enable the progress bar
            updatePageLoadProgress(0);
            enablePageLoadProgress();
            ...
            var loadNextPage = function(err) {
                ...
                updatePageLoadProgress(((lastPageLoaded - startPageForBatch) / (pageNumber - startPageForBatch)) * 100);
