...
define(['jquery', 'underscore', 'oae.core'], function($, _, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var DOMLAYOUTDELAY = 50;

        // Maximum time to keep checking
        var DOMLAYOUTCHECKMAX = 5000;

        // How close to page top before page is "in view"
        var PAGEVIEWCUSION = 1;

        // Extra margin, e.g., for left-aligned pages
        var MINPAGEMARGIN = 3;
                ...
                page.deferred = $.ajax({
                    ...
                    'success': function(response) {
                        ...
                        if (state.zoomLevel === 100) {
                             ...
                             * hide the content so it doesn't look weird.
                             */
                            page.$el.addClass('documentpreview-content-page-positioning');

                            var layoutCheckTime = 0;
                            var checkInterval = setInterval(function() {

                                /**
                                 * Only do this for the configured limit or until we
                                 * detect that the browser has finally figured out the
                                 * dimensions of the page.
                                 */

                                if ((layoutCheckTime > DOMLAYOUTCHECKMAX) ||
                                    (page.$el.innerWidth() !== page.$inner.outerWidth(true))) {

                                    clearInterval(checkInterval);

                                    zoomPage(page);
                                    page.$el.removeClass('documentpreview-content-page-positioning');
                                    page.loaded.resolve();
        ...
        var alignPage = function(page) {
            if (page.$inner) {
                if (state.zoomLevel === 100) {
                     ...
                     * we'll have to manually calculate.
                     */
                    var scale = state.zoomLevel / 100;
                    var containerWidth = isFullscreenActive() ? $(window).width() : $content.width();
                    var contentWidth = page.$inner.outerWidth(true) * scale;
                    var left = null;
                    var width = null;

                    if (contentWidth < containerWidth) {
                        left = Math.floor((100 / state.zoomLevel) * (containerWidth - contentWidth) / 2);
                        width = Math.ceil(page.$inner.outerWidth(true) / Math.max(scale,1));
                    } else {
                        left = MINPAGEMARGIN; // add some space (e.g. for border)
                        width = page.$inner.outerWidth(true) + 2 * MINPAGEMARGIN;
                    }
                    page.$inner.css('left', left + 'px');
                    page.$el.css('width', width + 'px');
                }
            }
        };
        ...
        _.each(_.range(widgetData.previews.pageCount), function(index) {
            pages.push({
                'pageNumber': index + 1,  // Document page numbers start at 1
                'loaded': $.Deferred()
            });
        });
