<!-- CSS -->
<link rel="stylesheet" type="text/css" href="/node_modules/oae-core/dashboard/css/dashboard.css" />
...
<div id="dashboard_container">
    ...
    <div id="change_layout_dialog" class="s3d-dialog s3d-dialog-container">
        ...
        <div class="dialog_buttons">
            ...
            <button type="button" class="s3d-button s3d-overlay-action-button" id="select-layout-finished">__MSG__SAVE__</button>
        </div>
    </div>
                ...
                {for i in items}
                    {if i.deletable === false}
                        <li id="row_compulsary_${i.id}" class="compulsary" style="display:none">
                                ...
                                ${sakai.api.Widgets.getWidgetDescription(i.id)}
                                </div>
                        </li>
                    {else}
                        {if i.alreadyIn}
                        ...
                        {/if}
                            ...
                            <button type="button" id="btn_add_${i.id}" class="s3d-button s3d-overlay-action-button add-button add goodies_add_button">__MSG__ADD__</button>
                            </div>
                                <div class="dialog_add_goodies_title">${i.title}</div>
                                <div class="dialog_add_goodies_description">
                                ${sakai.api.Widgets.getWidgetDescription(i.id)}
                        ...
                        {/if}
                            <div class="remove_add_column">
                                <button type="button" id="btn_remove_${i.id}" class="s3d-button s3d-overlay-action-button remove goodies_remove_button">__MSG__REMOVE__</button>
                            </div>
                ...
                {for p in c.portlets}
                    <div class="s3d-widget s3d-widget-container widget1 ${p.id}" id="${p.id}_${p.uid}">
                        <div {if p.display=="none"} class="fl-widget-titlebar widget1-head hiddenwidget" {else} class="fl-widget-titlebar widget1-head" {/if} id="draghandle_${p.id}_${p.uid}">
                            <h2 class="s3d-contentpage-title">
                                <button type="button" id="${p.id}_${p.uid}_settings" class="fl-force-right fl-icon settings s3d-link-button s3d-action" title="__MSG__OPTIONS__">
                                    <span class="s3d-aural-text">__MSG__OPTIONS__</span>
                                    <span class="dashboard_widget_arrow" style="display:none">&#9660;</span>
                                </button>
                                ${sakai.api.Widgets.getWidgetTitle(p.id)}
                            </h2>
                        </div>
                    {if p.display == "none"}
...
<!-- JAVASCRIPT -->
<script type="text/javascript" src="/node_modules/oae-core/dashboard/javascript/dashboard.js" />
