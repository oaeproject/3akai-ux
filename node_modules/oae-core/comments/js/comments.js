    ...
    return function(uid, showSettings) {
        ...
        var showErrorNotification = function(isReply) {
            var notificationTitle = oae.api.util
                .template()
                .render($('#comment-notification-title-template', $rootel), {
                    isReply: isReply,
                });
            var notificationBody = oae.api.util
                .template()
                .render($('#comment-notification-body-template', $rootel), {
                    isReply: isReply,
                });
        ...
        var renderComment = function(comment) {
            ...
            if (!comment.replyTo) {
                ...
                $('#comments-container li.media:first-child', $rootel).after(
                    oae.api.util
                        .template()
                        .render($('#comments-comment-template', $rootel), {
                            results: [comment],
                            canManage: contextProfile.isManager,
                        }),
                );
                // Apply timeago to the reply timestamp
                oae.api.l10n.timeAgo(
                    $('#comments-container li:first-child + li', $rootel),
                );
                ...
                oae.api.l10n.timeAgo(
                    $(
                        'li.media[data-id="' + comment.replyTo + '"] + li',
                        $rootel,
                    ),
                );
            }
        ...
        var deleteComment = function(ev, data) {
            ...
            if (data.softDeleted) {
                $(
                    'li.media[data-id="' + data.softDeleted.created + '"]',
                    $rootel,
                ).replaceWith(
                    oae.api.util
                        .template()
        ...
        var createComment = function(form) {
            ...
            oae.api.comment.createComment(
                contextProfile.id,
                contextProfile.resourceType,
                comment,
                null,
        ...
        var setUpValidation = function() {
            oae.api.util
                ...
                .validate(
                    $(
                        '.comments-new-comment-form[novalidate!="novalidate"]',
                        $rootel,
                    ),
                    {
            ...
            $(
                '.comments-new-reply-form[novalidate!="novalidate"]',
                $rootel,
        ...
        var setUpReplyComment = function() {
            $rootel.on('click', '.comments-reply-button', function() {
                var $replyContainer = $(this)
                    .parent()
                    .siblings('.comments-reply-container');
        ...
        var renderCreateNewComment = function() {
            $('#comments-container', $rootel).prepend(
                oae.api.util
                    .template()
                    .render($('#comments-new-comment-template'), $rootel),
            );
                ...
                function(activities) {
                    ...
                    if (
                        ...
                        var notificationBody = oae.api.util
                            ...
                            .render(
                                $(
                                    '#comments-new-comment-notifications-template',
                                    $rootel,
                                ),
                                {
                                    actorURL: oae.api.util.profilePath(
        ...
        var setUpInfiniteScroll = function() {
            ...
            infinityScroll = $('#comments-container', $rootel).infiniteScroll(
                url,
                null,
                $('#comments-comment-template', $rootel),
                {
        ...
        var setUpComments = function() {
            ...
            $(document).on('oae.context.send.comments', function(
                ev,
                contextData,
            ) {
