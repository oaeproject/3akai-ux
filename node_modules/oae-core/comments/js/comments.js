...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        // Variable used to keep track of current c
         ...
         */
         var createComment = function(form) {
            ...
            oae.api.comments.createComment(contextProfile.id, contextProfile.resourceType, comment, null, renderComment);
            // Reset the form
            $form[0].reset();
        ...
        var createReply = function(form) {
            ...
            var comment = $.trim($form.find('textarea').val());
            // Post the comment and re-render the results
            oae.api.content.createComment(contextProfile.id, contextProfile.resourceType, comment, replyTo, renderComment);
            // Reset the form
            $form[0].reset();
        ...
        var setUpDeleteComment = function() {
            $rootel.on('click', '.comments-delete-button', function() {
                ...
                oae.api.comments.deleteComment(contextProfile.id, contextProfile.resourceType, commentId, function(err, softDeleted) {
                    if (err) {
        ...
        var setUpInfiniteScroll = function() {
            ...
            // TODO: Just use the resourceType once discussions is sorted out
            var url = null;
            if (contextProfile.resourceType === 'content') {
                url = '/api/content/' + contextProfile.id + '/comments'
            } else if (contextProfile.resourceType === 'discussion') {
                url = '/api/discussions/' + contextProfile.id + '/messages';
            }
