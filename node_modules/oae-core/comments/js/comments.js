    ...
    return function(uid, showSettings) {
         ...
         */
         var createComment = function(form) {
            ...
            oae.api.comment.createComment(contextProfile.id, contextProfile.resourceType, comment, null, renderComment);
            // Reset the form
            $form[0].reset();
        ...
        var createReply = function(form) {
            ...
            var comment = $.trim($form.find('textarea').val());
            // Post the comment and re-render the results
            oae.api.comment.createComment(contextProfile.id, contextProfile.resourceType, comment, replyTo, renderComment);
            // Reset the form
            $form[0].reset();
        ...
        var setUpReplyComment = function() {
            $rootel.on('click', '.comments-reply-button', function() {
                ...
                // Render the reply container
                oae.api.util.template().render($('#comments-reply-template', $rootel), {
                    'created': $(this).attr('data-id')
                }, $('.media-body', $replyContainer));
                // Set up validation on the form
                setUpValidation();
