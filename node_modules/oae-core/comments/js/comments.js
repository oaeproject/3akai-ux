...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        /**
         * Deletes a comment based on the data-id attribute set on the delete button. An item that has been
         * properly deleted, because it had no replies, will be removed from the list. A deleted item that
         * does have replies will remain available as `This comment has been deleted`.
         */
        var deleteComment = function() {
            var $deleteButton = $('#deletecomment-delete', $rootel);
            var commentId = $deleteButton.attr('data-id');
            ...
            oae.api.comment.deleteComment(contextProfile.id, contextProfile.resourceType, commentId, function(err, softDeleted) {
                ...
                // Show a notification if there was an error deleting the comment
                if (err) {
                    oae.api.util.notification(
                        oae.api.i18n.translate('__MSG__COMMENT_NOT_DELETED__', 'comments'),
                        oae.api.i18n.translate('__MSG__COMMENT_DELETE_FAIL__', 'comments'),
                        'error');
                    return;
                }
        ...
        /**
         * Initializes the delete comment modal dialog
         */
        var setUpDeleteCommentModal = function() {
            $rootel.on('click', '.comments-delete-button', function() {
                // Set the clicked comment id as a data attribute on the modal delete button
                $('#deletecomment-delete').attr('data-id', $(this).attr('data-id'));
            ...
            $rootel.on('click', '#deletecomment-delete', function() {
                deleteComment();
            });
        };
