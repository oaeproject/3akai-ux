<!-- CSS -->
<link rel="stylesheet" type="text/css" href="/node_modules/oae-core/comments/css/comments.css" />

<!-- MAIN VIEW -->
<div id="comments_mainContainer" class="fl-widget-content s3d-widget-content" style="display:none;">
    <div id="comments_titlebar">
        <span class="comments_titleLeft">__MSG__COMMENTS:__</span><span class="comments_titleRight">__MSG__DISPLAYING__ <span id="comments_numCommentsDisplayed"></span> __MSG__OUT_OF__ <span id="comments_numComments" >0</span id="comments_commentscomment"> __MSG__COMMENTS_LC__</span>
    </div>
    ...
    <div class="commentsTable">
        {if comments && comments.length > 0}
            {for i in comments}
                ...
                <div class="comments_commentMessage s3d-regular-light-links">
                    <div class="comments_commentLinks">
                        <input type="hidden" id="comments_path_${i['sakai:id']}" value="/p/${i['_path']}" />
                        <ul>
                            {if i.canEdit && i.canEdit === true && i["sakai:deleted"] !== true}
                                <li><button type="button" class="comments_edit s3d-button s3d-link-button s3d-action" id="comments_edit_${i['sakai:id']}">__MSG__EDIT__</button></li>
                            {/if}
                            ...
                            {if i.canDelete && i.canDelete === true}
                                {if i["sakai:deleted"] && i["sakai:deleted"] === true}
                                    <button type="button" class="comments_undelete s3d-button s3d-link-button s3d-action" id="comments_undelete${i['sakai:id']}">__MSG__UNDELETE__</button></li>
                                {else}
                                    <button type="button" class="comments_delete s3d-button s3d-link-button s3d-action" id="comments_delete${i['sakai:id']}">__MSG__DELETE__</button></li>
                                {/if}
                            {/if}
                        </ul>
                    </div>
                    ...
                    {if i["sakai:deleted"] && i["sakai:deleted"] === true}
                        <p class="comments_infoDeleted">__MSG__COMMENT_HAS_BEEN_DELETED__
                        {if i.canDelete && i.canDelete === true}
                            <br />__MSG__YOU_CAN_RETURN__
                        {/if}
                        </p>
                    {/if}
            ...
            {/for}
        {else}
            __MSG__THERE_ARE_NO_COMMENTS__
        {/if}
    </div>
    --></div>
    <div class="fl-force-right">
        <button type="button" class="s3d-button s3d-overlay-button comments_comment" id="comments_comment_Bottom" style="display:none">__MSG__ADD_COMMENT__</button>
    </div>
    <div class="comments_nextButtons fl-push">
...
<!-- SETTINGS VIEW -->
<div id="comments_settings" class="s3d-widget-content" style="display:none;">
    <form class="comments_form">
        <div class="comments_settingsPart">
            <ul>
                <li>
                    <label for="comments_PerPageID" class="video_source_rbtLabel">__MSG__DISPLAY__ <input type="text" id="comments_txtPage" value="10" />
        ...
        <div class="fl-force-right">
            <button type="button" class="s3d-bold s3d-link-button" id="comments_cancel">__MSG__DONT_SAVE__</button>
            <button type="button" class="s3d-button s3d-overlay-action-button" id="comments_submit">__MSG__SAVE__</button>
        </div>
    </form>
</div>

<!-- JAVASCRIPT -->
<script type="text/javascript" src="/node_modules/oae-core/comments/javascript/comments.js"></script>
