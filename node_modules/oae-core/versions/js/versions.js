...
define(['jquery', 'oae.core', 'storyjs-embed'], function($, oae) {
    ...
    return function(uid) {
        ...
        var reset = function() {
            // Reset the timeline
            $('#versions-timeline', $rootel).empty();
        ...
        var parseVersionsIntoTimeline = function(revisions) {
            ...
            var FirstCreationDate = new Date(parseInt(revisions[0].created));
            ...
            $(revisions).each(function(index, version) {
                ...
                timelineData.timeline.date.push({
                    ...
                    'headline': oae.api.i18n.translate('__MSG__BY__') + ' ' + oae.api.util.security().encodeForHTML(version.createdBy.displayName),
                    'text': dateButtons,
                    'asset': {
                        'media': '<img src=' + version.link + '/>', // TODO: Adjust when https://github.com/sakaiproject/Hilary/issues/456 is merged.
                    }
                });
            });
        ...
        var restoreRevision = function() {
            oae.api.content.restoreRevision(contentContext.id, $(this).attr('data-revisionid'), function(error) {
                if (!error) {
                    // Hide the modal
                    $('#versions-modal', $rootel).modal('hide');
        ...
        var getRevisions = function() {
            oae.api.content.getRevisions(contentContext.id, function(error, data) {
                if (!error) {
        ...
        var setUpVersionsModal = function() {
            ...
            $rootel.on('click', '.versions-restore', restoreRevision);
        };
