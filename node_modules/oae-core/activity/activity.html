...
<div id="activity-items-template"><!--
    {macro renderSummary(activity)}
        ...
        {var object1Tenant = 'Cambridge University'}
        ...
        <div class="activity-summary-container clearfix">
            ...
            <div class="activity-summary oae-threedots">
                ...
                {elseif activityType === 'group-add-member'}
                    ${renderGroupAddMemberSummary(activity, actor1, actor1URL, object1, object1URL, target1, target1URL, target2, target2URL, targetCount)}
            ...
            <small>${oae.api.l10n.timeAgo(activity.published)}</small>
        </div>
    {/macro}
    ...
    {macro renderContentRevisionSummary(activity, actor1, actor1URL, actor2, actor2URL, actorCount, object1, object1URL)}
        {if activity.object['oae:resourceSubType'] === 'collabdoc'}
            {if !actorCount}
                __MSG__CONTENT_REVISION_COLLABDOC_1__
            {elseif actorCount === 2}
                __MSG__CONTENT_REVISION_COLLABDOC_2__
            {else}
                __MSG__CONTENT_REVISION_COLLABDOC_2+__
            {/if}
        {elseif activity.object['oae:resourceSubType'] === 'file'}
            {if !actorCount}
                __MSG__CONTENT_REVISION_FILE_1__
            {elseif actorCount === 2}
                __MSG__CONTENT_REVISION_FILE_2__
            {else}
                __MSG__CONTENT_REVISION_FILE_2+__
            {/if}
        {elseif activity.object['oae:resourceSubType'] === 'link'}
            {if !actorCount}
                __MSG__CONTENT_REVISION_LINK_1__
            {elseif actorCount === 2}
                __MSG__CONTENT_REVISION_LINK_2__
            {else}
                __MSG__CONTENT_REVISION_LINK_2+__
            {/if}
        {/if}
    {/macro}
    ...
    {macro renderContentShareSummary(activity, actor1, actor1URL, object1, object1URL, target1, target1URL, target2, target2URL, targetCount)}
        {if activity.object['oae:resourceSubType'] === 'collabdoc'}
            {if !targetCount}
                {if activity.target['oae:id'] === oae.data.me.id}
                    __MSG__CONTENT_SHARE_COLLABDOC_YOU__
                {else}
        ...
        {elseif activity.object['oae:resourceSubType'] === 'file'}
            {if !targetCount}
               {if activity.target['oae:id'] === oae.data.me.id}
                    __MSG__CONTENT_SHARE_FILE_YOU__
                {else}
        ...
        {elseif activity.object['oae:resourceSubType'] === 'link'}
            {if !targetCount}
                {if activity.target['oae:id'] === oae.data.me.id}
                    __MSG__CONTENT_SHARE_LINK_YOU__
                {else}
    ...
    {macro renderContentUpdateSummary(activity, actor1, actor1URL, actor2, actor2URL, actorCount, object1, object1URL)}
        {if activity.object['oae:resourceSubType'] === 'collabdoc'}
            {if !actorCount}
                __MSG__CONTENT_UPDATE_COLLABDOC_1__
            {elseif actorCount === 2}
                __MSG__CONTENT_UPDATE_COLLABDOC_2__
            {else}
                __MSG__CONTENT_UPDATE_COLLABDOC_2+__
            {/if}
        {elseif activity.object['oae:resourceSubType'] === 'file'}
            {if !actorCount}
                __MSG__CONTENT_UPDATE_FILE_1__
            {elseif actorCount === 2}
                __MSG__CONTENT_UPDATE_FILE_2__
            {else}
                __MSG__CONTENT_UPDATE_FILE_2+__
            {/if}
        {elseif activity.object['oae:resourceSubType'] === 'link'}
            {if !actorCount}
                __MSG__CONTENT_UPDATE_LINK_1__
            {elseif actorCount === 2}
                __MSG__CONTENT_UPDATE_LINK_2__
            {else}
                __MSG__CONTENT_UPDATE_LINK_2+__
            {/if}
        {/if}
    {/macro}
    ...
    {macro renderContentUpdateVisibilitySummary(activity, actor1, actor1URL, object1, object1URL, object1Tenant)}
        {if activity.object['oae:resourceSubType'] === 'collabdoc'}
            {if activity.object['oae:visibility'] === 'public'}
                __MSG__CONTENT_VISIBILITY_COLLABDOC_PUBLIC__
            {elseif activity.object['oae:visibility'] === 'loggedin'}
                __MSG__CONTENT_VISIBILITY_COLLABDOC_LOGGEDIN__
            {else}
                __MSG__CONTENT_VISIBILITY_COLLABDOC_PRIVATE__
            {/if}
        {elseif activity.object['oae:resourceSubType'] === 'file'}
            {if activity.object['oae:visibility'] === 'public'}
                __MSG__CONTENT_VISIBILITY_FILE_PUBLIC__
            {elseif activity.object['oae:visibility'] === 'loggedin'}
                __MSG__CONTENT_VISIBILITY_FILE_LOGGEDIN__
            {else}
                __MSG__CONTENT_VISIBILITY_FILE_PRIVATE__
            {/if}
        {elseif activity.object['oae:resourceSubType'] === 'link'}
            {if activity.object['oae:visibility'] === 'public'}
                __MSG__CONTENT_VISIBILITY_LINK_PUBLIC__
            {elseif activity.object['oae:visibility'] === 'loggedin'}
                __MSG__CONTENT_VISIBILITY_LINK_LOGGEDIN__
            {else}
                __MSG__CONTENT_VISIBILITY_LINK_PRIVATE__
            {/if}
        {/if}
    {/macro}

    {macro renderGroupAddMemberSummary(activity, actor1, actor1URL, object1, object1URL, target1, target1URL, target2, target2URL, targetCount)}
        {if !targetCount}
            {if activity.target['oae:id'] === oae.data.me.id}
                __MSG__GROUP_ADD_MEMBER_YOU__
            {else}
                __MSG__GROUP_ADD_MEMBER_1__
            {/if}
        {elseif targetCount === 2}
            __MSG__GROUP_ADD_MEMBER_2__
        {else}
            __MSG__GROUP_ADD_MEMBER_2+__
        {/if}
    {/macro}

    {macro renderGroupCreateSummary(activity, actor1, actor1URL, object1, object1URL, object2, object2URL, objectCount)}
        {if !objectCount}
            __MSG__GROUP_CREATE_1__
        {elseif objectCount === 2}
            __MSG__GROUP_CREATE_2__
        {else}
            __MSG__GROUP_CREATE_2+__
        {/if}
    {/macro}

    {macro renderGroupUpdateSummary(activity, actor1, actor1URL, actor2, actor2URL, actorCount, object1, object1URL)}
        {if !actorCount}
            __MSG__GROUP_UPDATE_1__
        {elseif actorCount === 2}
            __MSG__GROUP_UPDATE_2__
        {else}
            __MSG__GROUP_UPDATE_2+__
        {/if}
    {/macro}

    {macro renderGroupUpdateVisibilitySummary(activity, actor1, actor1URL, object1, object1URL, object1Tenant)}
        {if activity.object['oae:visibility'] === 'public'}
            __MSG__GROUP_VISIBILITY_PUBLIC__
        {elseif activity.object['oae:visibility'] === 'loggedin'}
            __MSG__GROUP_VISIBILITY_LOGGEDIN__
        {else}
            __MSG__GROUP_VISIBILITY_PRIVATE__
        {/if}
    {/macro}
                ...
                <ul class="media-list">
                        ...
                        <li class="media activity-level-${comment['oae:level']}">
                            ...
                            <div class="media-body">
                                <h4 class="media-heading">
                                    ...
                                    {/if}
                                    <small>
                                        ${oae.api.l10n.timeAgo(parseInt(created.substring(0, created.length - 1)))}
                                    </small>
                                </h4>
                    ...
                    {if !activity.object.hasAllComments}
                        <li class="media text-center">
                            <a href="${content.url}">__MSG__SHOW_ALL__...</a>
                        </li>
                    {/if}
                </ul>
            </div>
