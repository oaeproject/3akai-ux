...
<div id="activity-items-template"><!--
    {macro renderSummary(activity)}
        <div class="activity-summary-container clearfix">
            <div class="pull-left activity-entity-thumbnail">
                ${renderThumbnail(activity.primaryThumbnail)}
            </div>
            <div class="activity-summary">
                ${oae.api.i18n.translate('__MSG__' + activity.summary.i18nKey + '__', 'activity', activity.summary.i18nArguments)}
                <small class="muted"><time datetime="${activity.created}""></time></small>
            </div>
        </div>
    {/macro}
    ...
    {macro renderPreview(activity)}
        <div class="activity-preview-container">
            {if activity.thumbnails.length === 1 && activity.thumbnails[0]['oae:wideImage']}
                <ul class="row oae-list oae-list-grid activity-wide">
                    ${listItem(activity.thumbnails[0], null, false, activity.thumbnails[0]['oae:wideImage'].url, null, 'col-xs-12 col-lg-6')}
                </ul>
            {else}
                <ul class="row oae-list oae-list-grid" data-items="${activity.thumbnails.length}">
                    {for item in activity.thumbnails}
    ...
    {macro renderComments(activity)}
        {if activity.comments}
            <div class="activity-comment-container clearfix">
                <ul class="media-list">
                    {eval}
                        activity.comments = activity.comments.splice(0, 2);
                    {/eval}
                    ...
                    {/for}
                    {if !activity.comments.hasAllComments}
