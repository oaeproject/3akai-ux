...
define(['jquery', 'oae/api/oae.core', 'jquery-plugins/jquery.timeago'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        /**
         * Renders the activities in the view
         * @param {Object}   data    The activity stream for the current user/group
         */
        var renderActivity = function(data) {
            $('#activity_items_container', $rootel).html(oae.api.util.renderTemplate('#activity_items_template', {'data': data}));
        };
                ...
                if (comments[i]['oae:replyTo']) {
                    if (tree.length) {
                        ...
                        for (var ii = 0; ii < tree.length; ii++) {
                            // If the looped, already in the tree, comment oae:id matches the replyTo oae:id of the comment to add to the tree
        ...
        var parseActivity = function(data) {
            for (var i = 0; i < data.items.length; i++) {
                if (data.items[i]['oae:activityType'] === 'content-comment') {
                    if (data.items[i].object['oae:collection']) {
                        data.items[i].object['oae:collection'].sort(sortComments);
                        data.items[i].object['oae:collection'] = data.items[i].object['oae:collection'].splice(0, 2);
                        data.items[i].object['oae:collection'] = constructCommentTree(data.items[i].object['oae:collection']);
                    }
                }
            }
            renderActivity(data);
        ...
        var getActivity = function() {
            $.ajax({
                'url': '/api/activity/' + widgetData.principalId + '?limit=10',
