...
<!-- MODAL -->
<div id="deleteresources-modal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="deleteresources-modal-title" aria-hidden="true">
...
<div id="deleteresources-overview-template"><!--
    ...
    <ul class="oae-list oae-list-compact">
        {for resource in resources}
            ${listItem(resource, null, false)}
        {/for}
    </ul>

    {if canManage}
        <div class="row-fluid">
            <div class="span6 well text-center">
                <div>
                    <button type="button" class="btn" id="deleteresources-manage-delete-library">
                        <i class="icon-unlink pull-left"></i> __MSG__DELETE_FROM_LIBRARY__
                    </button>
                </div>
                <small>__MSG__DELETE_FROM_LIBRARY_DESCRIPTION__</small>
            </div>
            <div class="span6 well text-center">
                <div>
                    <button type="button" class="btn btn-primary" id="deleteresources-manage-delete-system">
                        <i class="icon-trash pull-left"></i> __MSG__DELETE_FROM_SYSTEM__
                    </button>
                </div>
                <small>__MSG__DELETE_FROM_SYSTEM_DESCRIPTION__</small>
            </div>
        </div>
    {else}
        <div class="well text-center">
            <div>
                <button type="button" class="btn" id="deleteresources-view-delete-library">
                    <i class="icon-unlink pull-left" /> __MSG__DELETE_FROM_LIBRARY__
                </button>
            </div>
            <small>__MSG__DELETE_FROM_LIBRARY_DESCRIPTION__</small>
        </div>
...
<div id="deleteresources-notification-body-template"><!--
    ...
    {else}
        {if errCount === 1}
            {if resourceType === 'content'}
                {if resourceSubType === 'collabdoc'}
                    {if !hardDeleteManage} __MSG__DOCUMENT_AT_LEAST_ONE_MANAGER__{else} __MSG__DOCUMENT_DELETE_FAIL__{/if}
                {elseif resourceSubType === 'file'}
                    {if !hardDeleteManage} __MSG__CONTENT_AT_LEAST_ONE_MANAGER__{else} __MSG__FILE_DELETE_FAIL__{/if}
                {elseif resourceSubType === 'link'}
                    {if !hardDeleteManage} __MSG__LINK_AT_LEAST_ONE_MANAGER__{else} __MSG__LINK_DELETE_FAIL__{/if}
                {/if}
            {elseif resourceType === 'discussion'}
                {if !hardDeleteManage} __MSG__DISCUSSION_AT_LEAST_ONE_MANAGER__{else} __MSG__DISCUSSION_DELETE_FAIL__{/if}
            {/if}
        {else}
            {if resourceType === 'content'}
                {if !hardDeleteManage} __MSG__CONTENT_AT_LEAST_ONE_MANAGER__{else} __MSG__CONTENT_DELETE_FAIL__{/if}
            {elseif resourceType === 'discussion'}
                {if !hardDeleteManage} __MSG__DISCUSSION_AT_LEAST_ONE_MANAGER__{else} __MSG__DISCUSSIONS_DELETE_FAIL__{/if}
            {/if}
        {/if}
    {/if}
--></div>
