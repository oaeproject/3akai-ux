...
define(['jquery', 'oae.core'], function ($, oae) {
    ...
    return function (uid) {
        ...
        var startRemoval = function() {
            ...
            $('#deleteresources-view-list', $rootel).hide();
            $('#deleteresources-view-progress', $rootel).show();
            ...
            // Synchronously remove/delete all resources.
            if (resourcesToRemoveFromLibrary.length !== 0) {
                removeResourceFromLibrary(resourcesToRemoveFromLibrary.pop(), resourceCallback);
            } else if (resourcesToDelete.length !== 0) {
                deleteResource(resourcesToDelete.pop(), resourceCallback);
        ...
        var renderListView = function(resourcesToRender, showRemoveAndDelete, notificationMessage) {
            ...
            $('#deleteresources-view-progress', $rootel).hide();
            $('#deleteresources-view-list', $rootel).show();
        };
        ...
        var gatherProfiles = function() {
            setTitle('__MSG__GATHERING_SELECTED_CONTENT__');
         ...
         *
         * @param  {Number}     done        How many items have been processed
         * @param  {Number}     todo        How many items that should be processed when everything is completed.
         * @param  {String}     i18nKey     The key to use for the message that should be displayed under the bar.
         */
        var updateProgress = function(done, todo, i18nKey) {
            var progress = (done / todo) * 100;
            $('.bar', $rootel).css('width', progress + '%');
            var variables = {
                'done': done,
                'todo': todo
            };
            var text = oae.api.i18n.translate(i18nKey, 'deleteresources', variables);
            $('#deleteresources-progress-msg', $rootel).html(text);
        };
         ...
         */
        var setTitle = function(i18nKey) {
            var text = oae.api.i18n.translate(i18nKey, 'deleteresources');
        ...
        var setUpDeleteResourcesModal = function() {
            $(document).on('click', '.oae-delete-resources', function() {
                // Reset our widget.
                resetWidget();
