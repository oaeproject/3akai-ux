    ...
    return function(uid) {
        ...
        var setModalTitle = function(view) {
            oae.api.util.template().render(
                $('#deleteresources-modal-title-template', $rootel),
                {
        ...
        var deleteResources = function(hardDeleteManage) {
            ...
            var count =
                selectedResourcesManage.length + selectedResourcesMember.length;
            ...
            var deletedResource = function(err) {
                ...
                if (selectedResourcesMember.length !== 0) {
                    deleteResourceFromLibrary(
                        selectedResourcesMember.pop(),
                        deletedResource,
                    );
                } else if (selectedResourcesManage.length !== 0) {
                    if (hardDeleteManage) {
                        deleteResourceFromSystem(
                            selectedResourcesManage.pop(),
                            deletedResource,
                        );
                    } else {
                        deleteResourceFromLibrary(
        ...
        var deleteResourceFromLibrary = function(resource, callback) {
            if (contextProfile.resourceType === 'folder') {
                oae.api.folder.deleteContentFromFolder(
                    contextProfile.id,
                    resource.id,
                    callback,
                );
            } else if (resource.resourceType === 'content') {
                oae.api.content.deleteContentFromLibrary(
                    contextProfile.id,
                    resource.id,
                    callback,
                );
            } else if (resource.resourceType === 'folder') {
                oae.api.folder.deleteFolderFromLibrary(
                    contextProfile.id,
                    resource.id,
                    callback,
                );
            } else if (resource.resourceType === 'discussion') {
                oae.api.discussion.deleteDiscussionFromLibrary(
                    contextProfile.id,
                    resource.id,
                    callback,
                );
            } else if (resource.resourceType === 'meeting-jitsi') {
                oae.api.meetingJitsi.deleteMeetingFromLibrary(
                    contextProfile.id,
                    resource.id,
                    callback,
                );
        ...
        var finishDeleteResource = function(count, errCount) {
            ...
            var notificationTitle = oae.api.util
                .template()
                .render(
                    $('#deleteresources-notification-title-template', $rootel),
                    data,
                );
            var notificationBody = oae.api.util
                .template()
                .render(
                    $('#deleteresources-notification-body-template', $rootel),
                    data,
                );
        ...
        var renderOverview = function(resources, canManage) {
            oae.api.util.template().render(
                $('#deleteresources-overview-template', $rootel),
                {
                    contextProfile: contextProfile,
                    canManage: canManage,
                    hasManageAndMember:
                        selectedResourcesManage.length > 0 &&
                        selectedResourcesMember.length > 0,
                    resources: resources,
                    resourceType: resourceType,
                },
        ...
        var gatherResourceProfiles = function() {
            ...
            var getResourceProfile = function() {
                resourceProfileFunction(selectedResources[done].id, function(
                    err,
                    resourceProfile,
                ) {
                    if (err) {
                        ...
                        if (
                            resourceProfile.isManager ||
                            resourceProfile.canManage
                        ) {
        ...
        var setUpDeleteResourcesModal = function() {
            ...
            $(document).on('oae.context.send.deleteresources', function(
                ev,
                context,
            ) {
            ...
            $(document).on('oae.list.sendSelection.deleteresources', function(
                ev,
                selected,
            ) {
            ...
            $rootel.on(
                'click',
                '#deleteresources-view-delete-library',
                showResourcesManage,
            );
            ...
            $rootel.on(
                'click',
                '#deleteresources-manage-delete-library',
                function() {
            ...
            $rootel.on(
                'click',
                '#deleteresources-manage-delete-system',
                function() {
