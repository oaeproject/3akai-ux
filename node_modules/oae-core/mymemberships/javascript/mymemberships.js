...
require(['jquery', 'sakai/sakai.api.core'], function($, sakai) {
    ...
    sakai_global.mymemberships = function(tuid, showSettings) {
        ...
        var $mymemberships_show_grid = $('.oae-listview-grid', $rootel);
        var $mymemberships_show_list = $('.oae-listview-list', $rootel);
        var $mymemberships_nosearchresults = $('#mymemberships_nosearchresults');
        var $mymemberships_result_count = $('.oae-search-result-count', $rootel);
        ...
        var render = function(groups) {
            if (groups.length === 0) {
                ...
                if (currentQuery && groupData.length) {
                    ...
                    $mymemberships_result_count.children('.oae-search-result-count-label').text(resultLabel);
                    $mymemberships_result_count.children('.oae-search-result-count-count').text(groupData.length);
                } else {
        ...
        var handleHashChange = function() {
            ...
            if (mymemberships.listStyle === 'list') {
                $('#mymemberships_items', $rootel).removeClass('oae-search-results-grid');
                $mymemberships_show_list.addClass('selected');
                $mymemberships_show_list.children().addClass('selected');
            } else {
                $('#mymemberships_items', $rootel).addClass('oae-search-results-grid');
        ...
        var addBinding = function() {
            ...
            $rootel.on('click', '.oae-actions-delete', function() {
                if ($(this).hasClass('mymemberhips_disabled_leave')) {
                    ...
                    var msg = sakai.api.i18n.getValueForKey('ARE_YOU_SURE_YOU_WANT_TO_LEAVE', 'mymemberships').replace('${groupname}', '<span class="oae-bold">' + $(this).data('sakai-entityname') + '</span>');
