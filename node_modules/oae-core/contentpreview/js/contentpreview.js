...
define(['jquery', 'oae.core', '/node_modules/oae-core/contentpreview/lib/document-viewer/assets/viewer.js'], function($, oae, DV) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var oembedProviders = {
            ...
            'livestream': RegExp('livestream.com/.+'),
        ...
        var initPreview = function() {
            ...
            if (widgetData.resourceSubType === 'link') {
                $.each(oembedProviders, function(provider, expression) {
                    // Test the link with the expression
                    if (expression.test(widgetData.link)) {
                        oae.api.widget.insertWidget('oembed', null, $('#contentpreview_preview', $rootel), null, widgetData);
                        isEmbedded = true;
                    }
                });
                // If the link isn't embedded yet load the link in a iframe
                if (!isEmbedded) {
                    renderLinkPreview();
                }
