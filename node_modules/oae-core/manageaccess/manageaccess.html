            ...
            <form role="form">
                ...
                <div class="modal-body">
                    <div id="manageaccess-overview">
                        ...
                        <h4>
                            <button type="button" id="manageaccess-share-add-more" class="btn btn-sm btn-link pull-right"><i class="icon-plus-sign"></i> __MSG__ADD_MORE__</button>
                            <span id="manageaccess-overview-shared-title"></span>
                        </h4>
                        <div id="manageaccess-overview-shared" class="well">
                            <ul id="manageaccess-overview-selected" class="oae-list oae-list-compact"></ul>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    <div id="manageaccess-overview-footer">
                        ...
                        <button type="button" id="manageaccess-overview-save" class="btn btn-primary pull-right" disabled="true">__MSG__SAVE__</button>
                    </div>
                    <div id="manageaccess-visibility-footer" class="hide">
                        <button type="button" class="manageaccess-cancel btn btn-link pull-left">__MSG__CANCEL__</button>
                        <button type="button" id="manageaccess-visibility-save" class="btn btn-primary pull-right">__MSG__UPDATE__</button>
                    </div>
                    <div id="manageaccess-share-footer" class="hide">
                        <button type="button" class="manageaccess-cancel btn btn-link pull-left">__MSG__CANCEL__</button>
                        <button type="button" id="manageaccess-share-update" class="btn btn-primary pull-right" disabled="true">__MSG__ADD__</button>
                    </div>
                </div>
            </form>
        </div>
...
<div id="manageaccess-members-template"><!--
    {for member in results}
        <li data-id="${member.profile.id}">
            <div class="oae-list-compact-item">
                <div class="oae-listitem oae-listitem-compact clearfix">
                    <div class="oae-listitem-actions pull-right">
                        {var memberName = oae.api.util.security().encodeForHTMLAttribute(member.profile.displayName)}
                        <button type="button" title="__MSG__REMOVE_MEMBER__" class="close" data-id="${member.profile.id}">&#215;</button>
                        <div class="form-group pull-right">
                            <label for="manageaccess-${member.profile.id}-role" class="oae-aural-text">__MSG__SELECT_ROLE_FOR_USER__</label>
                            <select id="manageaccess-${member.profile.id}-role" class="form-control" data-id="${member.profile.id}">
                                {for role in roles}
                                    <option value="${role_index}" {if member.role === role_index}selected="true"{/if}>${role}</option>
                                {/for}
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        <h3 class="oae-threedots">${member.profile.displayName|encodeForHTML}</h3>
                    </div>
                </div>
            </div>
        </li>
...
<div id="manageaccess-visibility-template"><!--
    ...
    <div class="oae-large-options-container row">
        <label for="oae-visibility-private" class="col-md-4 text-center">
        ...
        <label for="oae-visibility-loggedin" class="col-md-4 text-center">
        ...
        <label for="oae-visibility-public" class="col-md-4 text-center">
...
<div id="manageaccess-share-template"><!--
    ...
    <div class="form-group clearfix">
        <select id="manageaccess-share-role" class="pull-right form-control">
            {for role in roles}
                <option value="${role_index}">${role}</option>
            {/for}
        </select>
        <h4>
