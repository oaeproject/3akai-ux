...
<div id="manageaccess-overview-selected-template"><!--
    {macro manageaccessListItemActions()}
        ...
        {if member.resourceType === 'email' && member.beenInvited}
            <button type="button" title="__MSG__RESEND_INVITATION__" class="btn manageaccess-invitation-resend" data-email="${member.id|encodeForHTMLAttribute}">__MSG__RESEND__</button>
        {/if}
    {/macro}
