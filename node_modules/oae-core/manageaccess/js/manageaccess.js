    ...
    return function(uid) {
        ...
        var getInvitations = function() {
            widgetData.api.getInvitations(widgetData.contextProfile.id, function(err, result) {
                if (err) {
                    oae.api.util.notification('__MSG__INVITATIONS_FAILED__', '__MSG__FAILED_TO_FETCH_INVITATIONS__',
                        'error');
                    return;
                }

                serverInvitedMembers = _.map(result.results, function(invitation) {
                    return createMemberProfileForEmail(invitation.email, invitation.role);
                });
        ...
        var updateInvitations = function() {
            ...
            var currentInvitations = _.chain(invitationsUpdates)
                ...
                .union(nonUpdatedServerInvitedMembers)
                .value();
        ...
        var addNewMembers = function(autoSuggestMembers) {
            ...
            var newInvitations = _.first(membersPartition);
            var newMembers = _.last(membersPartition);
        ...
        var queryAutoSuggest = function(q, next) {
            var limit = '';
        ...
        var addBinding = function() {
            ...
            $rootel.on('click', '.oae-listitem-actions .manageaccess-remove', deleteMember);
