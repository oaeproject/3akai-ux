...
define(['jquery', 'oae.core', 'jquery.autosuggest'], function($, oae) {
    ...
    return function(uid) {
        ...
        var savemanageaccess = function() {
            ...
            /**
             * Checks if visibility and/or members have been updated and shows a
             * success or failure notification before closing the dialog.
             *
            ...
            var checkDone = function(err) {
                ...
                // If all changes have been persisted show a notification.
                if (done === todo) {
            ...
            if (visibilityUpdate !== groupData.visibility) {
                todo++;
                oae.api.group.updateGroup(groupData.id, {
                    'visibility': visibilityUpdate
                }, function(err) {
                    if (!err) {
                        groupData.visibility = visibilityUpdate;
                    }
                    checkDone(err);
                });
            }
            ...
            if (!$.isEmptyObject(membersUpdates)) {
                todo++;
                oae.api.group.setGroupMembers(groupData.id, membersUpdates, checkDone);
            }
        };

        /**
         * Checks if any values have changed and disables/enables the widget save button.
         */
        var enableDisableSave = function() {
            if (visibilityUpdate !== groupData.visibility ||
                !$.isEmptyObject(membersUpdates)) {
         ...
         * @return {Object[]}   autoSuggestMembers[i].profile   The member profile as returned by the group members endpoint.
         */
         ...
         *
         * @param  {String}    name    The name of the panel to show (e.g. 'share', 'visibility', 'overview')
         */
         ...
         */
        var initDefaultPanel = function(ev, data) {
            groupData = data;
            visibilityUpdate = data.visibility;
        ...
        var setUpManageAccessModal = function() {
            ...
            $(document).on('oae.context.send.manageaccess', initDefaultPanel);
        ...
        var addBinding = function() {
            ...
            $rootel.on('click', '#manageaccess-visibility-save', function() {
                // Update the visibility in the cached visibilityUpdate
                visibilityUpdate = $('.oae-visibility-choice-container input[type="radio"]:checked', $rootel).val();
                renderVisibilityOverview();
                showPanel('overview');
                // Enable or disable the save button
                enableDisableSave();
