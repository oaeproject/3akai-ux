...
define(['jquery', 'underscore', 'oae.core', 'jquery.autosuggest'], function(
    $,
    _,
    oae,
) {
    return function(uid) {
            ...
            saveVisibility(function(visibilityErr) {
                ...
                saveMembers(function(membersErr) {
                    ...
                    if (visibilityErr || membersErr) {
                        ...
                        $(document).trigger(
                            'oae.manageaccess.done',
                            widgetData.contextProfile,
                        );
        ...
        var enableDisableSave = function() {
            ...
            if (
                ...
                $('#manageaccess-overview-save', $rootel).prop(
                    'disabled',
                    true,
                );
            } else {
        ...
        var getInvitations = function() {
            widgetData.api.getInvitations(
                widgetData.contextProfile.id,
                function(err, result) {
                    if (err) {
                        oae.api.util.notification(
                            oae.api.i18n.translate(
                                '__MSG__INVITATIONS_FAILED__',
                            ),
                            oae.api.i18n.translate(
                                '__MSG__FAILED_TO_FETCH_INVITATIONS__',
                            ),
                            'error',
                        );
                        return;
                    ...
                    serverInvitedMembers = _.map(result.results, function(
                        invitation,
                    ) {
                        return createMemberProfileForEmail(
                            invitation.email,
                            invitation.role,
                            true,
                        );
        ...
        var updateInvitations = function() {
            ...
            // it
            var nonUpdatedServerInvitedMembers = _.filter(
            ...
            var currentInvitations = _.chain(invitationsUpdates)
                ...
                // the UI
                .reverse();
            currentInvitations = _.union(
                currentInvitations,
                nonUpdatedServerInvitedMembers,
            );
            ...
            if (_.isEmpty(currentInvitations)) {
                ...
                oae.api.util.template().render(
                    ...
                    $(
                        '#manageaccess-overview-selected-invitations-container',
                        $rootel,
                    ),
                );
        ...
        var resendInvitation = function(email) {
            ...
            widgetData.api.resendInvitation(resourceId, email, function(err) {
                if (err) {
                    oae.api.util.notification(
                        oae.api.i18n.translate(
                            '__MSG__RESEND_INVITATION_FAILED__',
                            'manageaccess',
                        ),
                        oae.api.i18n.translate(
                            '__MSG__FAILED_TO_RESEND_INVITATION__',
                            'manageaccess',
                        ),
                        'error',
                    );
                ...
                oae.api.util.notification(
                    oae.api.i18n.translate(
                        '__MSG__RESEND_INVITATION_SUCCESS__',
                        'manageaccess',
                    ),
                    oae.api.i18n.translate(
                        '__MSG__INVITATION_HAS_BEEN_SUCCESSFULLY_RESENT__',
                        'manageaccess',
                    ),
                );
            });
        ...
        var getMembers = function() {
            ...
            infinityScroll = $(
                '#manageaccess-overview-shared .oae-list',
                $rootel,
            ).infiniteScroll(
                url,
                {
        ...
        var addNewMembers = function(autoSuggestMembers) {
            var membersPartition = _.partition(autoSuggestMembers, function(
                member,
            ) {
            ...
            _.each(newInvitations, function(newInvitation) {
                invitationsUpdates[newInvitation.profile.id] =
                    newInvitation.role;
        ...
        var autoSuggestChanged = function() {
            $('#manageaccess-share-update', $rootel).prop(
                'disabled',
                getAutosuggestSelection().length ? false : true,
            );
        };
        ...
        var getAutosuggestSelection = function() {
            ...
            var selectedItems = [];
            $.each(oae.api.util.autoSuggest().getSelection($rootel), function(
                index,
                selectedItem,
            ) {
        ...
        var initManageAccess = function(ev, data) {
            ...
            $('#manageaccess-overview-shared-title', $rootel).text(
                widgetData.messages.membersTitle,
            );
        ...
        var addBinding = function() {
            ...
            $rootel.on(
                'click',
                '.oae-listitem-actions .manageaccess-invitation-resend',
                function() {
            ...
            $rootel.on(
                'click',
                '.oae-listitem-actions .oae-listitem-remove',
                deleteMember,
            );
