<!-- CSS -->
<link rel="stylesheet" type="text/css" href="/node_modules/oae-core/mylibrary/css/mylibrary.css" />
...
<div class="mylibrary_widget">
    ...
    <div class="s3d-page-header-top-row" style="display:none;">
        ...
        <div id="mylibrary_sortarea" class="s3d-margin-top-5 s3d-page-header-sort-area" style="display:none;">
            <label for="mylibrary_sortby">__MSG__SORT_BY:__</label>
        ...
        <div class="s3d-search-container" role="search" aria-label="__MSG__SEARCH_THROUGH_THE_CONTENT_LIBRARY__">
        ...
        {else}
            {if mode === "user_me"}
                <div class="s3d-no-results-container">
                    ...
                    <p>__MSG__NO_RESULTS_DESCRIPTION_GROUP_ME__ <button id="mylibrary_addcontent" class="s3d-link-button sakai_add_content_overlay">__MSG__ADD_CONTENT__</button></p>
                </div>
            ...
            {if item.name}
                ...
                <li id="${item.id|safeOutput}" class="fl-container fl-fix mylibrary_item s3d-search-result s3d-draggable-container" data-scope="content" data-entityid="${item.id}" {if sakai.api.Content.Collections.isCollection(item)} data-collection="true" {/if} data-canshare="${canShare}">
                    <div class="s3d-search-result-user-functions s3d-draggable-hidden">
                        <div class="mylibrary_item_left_filler">
                            {if !sakai.data.me.anon}
                                ...
                                {if isMe}
                                    <label for="mylibrary_check_${item.id}" class="s3d-aural-text">__MSG__SELECT__ ${item['name-short']|safeOutput} __MSG__SELECT_REMOVE__</label>
                        ...
                        {if !sakai.data.me.anon}
                            <button title="__MSG__ADD_TO__" data-entityid="${item.id}" class="s3d-link-button s3d-action-icon s3d-actions-addtolibrary mylibrary_item_share_icon savecontent_trigger s3d-draggable-hidden"></button>
                        {/if}
                        {if canShare}
                    ...
                    <div class="mylibrary_item_right s3d-regular-links s3d-search-result-right">
                        <div>
                            <a class="s3d-bold s3d-regular-light-links" href="${item.homePath}" title="__MSG__VIEW_THIS_ITEM__">
                                <span class="s3d-search-result-name">${item['name-short']}</span>
                                <span class="s3d-search-result-name-grid">${item['name-shorter']}</span>
                            </a>
                        ...
                        {if item.tags && item.tags.length > 0}
                            <div class="mylibrary_item_tags s3d-taglist">
                                <span class="s3d-aural-text">__MSG__TAGS_FOR__ ${item["name"]|safeOutput}</span>
...
<!-- JAVASCRIPT -->
<script type="text/javascript" src="/node_modules/oae-core/mylibrary/javascript/mylibrary.js"></script>