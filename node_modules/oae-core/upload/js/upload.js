...
define(['jquery', 'oae.core', 'jquery.fileupload', 'jquery.iframe-transport', 'jquery.jeditable'], function($, oae) {
    ...
    return function(uid, showSettings) {
         ...
         */
        var reset = function() {
            // Unbind the setpermission events from the document
            $(document).off('oae.setpermissions.changed');
            $(document).off('oae.setpermissions.cancel');
        ...
        var addToSelected = function(data) {
            $.each(data.files, function(index, file) {
                ...
                if (validFile) {
                    file.id = oae.api.util.generateId();
                    var contentObj = {
        ...
        var showPermissions = function() {
            ...
            $('.modal-body > div#upload-permissions-container', $rootel).show();
            $('#upload-doupload', $rootel).hide();
        };
        ...
        var showOverview = function() {
            ...
            $('#upload-modal > .modal-footer', $rootel).show();
            $('.modal-body > div#upload-overview-container', $rootel).show();
        ...
        var renderSelected = function() {
            ...
            // Trigger the setpermissions widget if it isn't initialized yet
            if (!$('#upload-permissions-container', $rootel).html()) {
         ...
         */
        var setUpSetPermissionsChanged = function() {
            $(document).on('oae.setpermissions.changed', function(ev, data) {
                // Update visibility for files
                visibility = data.visibility;
                // Get an array of just the IDs of the viewers
                var viewers = [];
                $(data.viewers).each(function(index, viewer) {
                    viewers.push(viewer.id);
                });
                // Update the members of the files
            ...
            $(document).on('oae.setpermissions.cancel', showOverview);
        };
        ...
        var setUpUploadField = function() {
            ...
            var fileuploadOptions = {
                ...
                'progress': function(ev, data) {
                    updateProgress(data.files[0].id, (data.loaded / data.total) * 100);
                }
            };
        ...
        var setUpUploadHandling = function() {
            $('#upload-doupload', $rootel).off('click').on('click', function() {
                var done = 0;
                ...
                var upload = function(index) {
                    ...
                    if (file) {
                        var $listItem = $('#' + file.file.id, $rootel);
                        var $progress = $listItem.find('.progress');
                        var $ok = $listItem.find('.icon-ok');
                        var $warning = $listItem.find('.icon-warning-sign');
                        var $name = $listItem.find('.upload-selected-name');

                        // Make the name field smaller so the progress bar can be shown
                        $name.addClass('uploading');
                        $progress.show();
                        ...
                        oae.api.content.createFile(file.displayName, file.description, visibility, $('#upload-input', $rootel), file.file, [], file.viewers, function(error, data) {
                            // Make the name field wide again
                            $progress.hide();
                            $name.removeClass('uploading');
                            ...
                            if (done !== selectedFiles.length) {
                                ...
                                oae.api.util.notification(
                                    oae.api.i18n.translate('__MSG__FILES_UPLOADED__', 'upload'),
                                    oae.api.i18n.translate('__MSG__FILES_SUCCESSFULLY_UPLOADED__', 'upload')
                                );
                            }
                        });
                    }
