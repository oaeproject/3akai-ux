...
define(['jquery', 'oae.core', 'jquery.fileupload', 'jquery.iframe-transport', 'jquery.jeditable'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        // Keeps track of the context of the upload widget (group or user)
        ...
        var showCompleteNotification = function(errCount) {
            ...
            var notification = oae.api.util.template().render($('#upload-notification-template', $rootel), {
                'context': contextData,
            ...
            var title = oae.api.i18n.translate('__MSG__FILES_UPLOADED__', 'upload');
            if (errCount > 0) {
                title = oae.api.i18n.translate('__MSG__FILES_NOT_UPLOADED__', 'upload');
            }

            oae.api.util.notification(title, notification, errCount ? 'error' : 'info');
        ...
        var setUpSetPermissions = function() {
            ...
            $(document).on('oae.setpermissions.changed.' + setPermissionsId, function(ev, data) {
                ...
                var viewers = [];
                // Reset the value that indicates that a content item is shared with the current group
                contextData.isSharedWithGroup = false;
                $(data.viewers).each(function(index, viewer) {
                    viewers.push(viewer.id);
                    // If the context of the page is a group context and the group will be added as a member
                    // set isSharedWithGroup to true
                    if ((viewer.id === contextData.id) && contextData.resourceType === 'group') {
                        contextData.isSharedWithGroup = true;
                    }
                });
                    ...
                    if (file) {
                        ...
                        oae.api.content.createFile(file.displayName, file.description, visibility, $('#upload-input', $rootel), file.file, [], file.viewers, function(error, data) {
                            ...
                            if (!error) {
                                ...
                                file.profilePath = JSON.parse(data).profilePath;
                            } else {
                                $warning.show();
