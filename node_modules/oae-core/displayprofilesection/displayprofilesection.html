<!-- CSS -->
<link href="/node_modules/oae-core/displayprofilesection/css/displayprofilesection.css" type="text/css" />
...
<div id="displayprofilesection_edit_template" style="display:none"><!--
    {macro renderTextInput(config, field_id, data)}
        <input type="text" id="${field_id}" name="${field_id}" {if data}value="${data|safeOutput}"{/if} class="{if config.required}required {/if}{if config.validation} ${config.validation} {/if}"/>
    {/macro}
    ...
    {else}
        <button type="button" class="displayprofilesection_remove_section oae-bold oae-link-button" id="displayprofilesection_remove_link_${unique}"><span>__MSG__REMOVE_THIS__ ${section.multipleLabel}</span></button>
        <hr/>
    </div>
    {/if}
--></div>
...
<div id="displayprofilesection_edit_multiple_template"><!--
    <form id="displayprofilesection_form_${sectionid}">
        ...
        <button type="submit" class="oae-button oae-overlay-button grey profile-section-save-button" style="display:none" disabled="disabled">__MSG__ADD__</button>
    </form>
    <button id="displayprofilesection_add_${sectionid}" class="displayprofilesection_add_section oae-link-button oae-bold" type="button"><span class="oae-add_another_location"></span><span>__MSG__ADD_ANOTHER__ ${section.multipleLabel}</span></button>
--></div>
...
<div id="displayprofilesection_view_template" style="display:none"><!--
    {macro renderField(config, field_id, field_data)}
        {var value = config.type === "select" ? sakai.api.i18n.General.process(config.select_elements[field_data]) : field_data}
        <span class="field_value">
            {var displayValue = value}
            {if config.limitDisplayWidth}
                {var displayValue = sakai.api.Util.applyThreeDots(value, config.limitDisplayWidth, null, null, true)}
            {/if}
            {if config.type === "tags"}
                {var tags = sakai.api.Util.formatTags(value)}
                {for tag in tags}{if tag_index > 0},{/if} <a href="${tag.link}" class="oae-action" title="${tag.linkTitle}">${tag.value}</a>{/for}
...
<!-- JAVASCRIPT -->
<script src="/node_modules/oae-core/displayprofilesection/javascript/displayprofilesection.js" type="application/javascript"></script>