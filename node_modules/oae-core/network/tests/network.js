casper.test.begin('Widget - Network', function(test) {
    ...
    var verifyFollowingUser = function() {
        ...
        test.assertExists('button.oae-trigger-follow', 'Verify the `Follow` button is present');
        // Verify the user can be followed
        casper.click('button.oae-trigger-follow');
        casper.waitForSelector('#oae-notification-container .alert', function() {
            test.assertDoesntExist('#oae-notification-container .alert.alert-error', 'Verify a user can be followed');
            test.assertNotVisible('button.oae-trigger-follow', 'Verify follow button is removed after following a user');
        });
    };
        ...
        casper.waitForSelector('#search-clip-container', function() {
            ...
            casper.thenOpen(configUtil().tenantUI + '/me/network', function() {
                casper.waitForSelector('#network-widget .oae-list.oae-list-grid li', function() {
                    ...
                    test.assertExists('#network-widget .oae-list-options', 'The list options are present');
                    // Verify list options contain a checkbox that selects all
                    test.assertExists('#network-widget .oae-list-options input[type="checkbox"]', 'The \'Select all\' checkbox is present in the list options');
                    // Verify list options contain a `unfollow` button
                    test.assertExists('#network-widget .oae-list-options .oae-trigger-unfollow', 'The `Stop following` button is present in the list options');
                    // Verify list options contain a switch view button
                    test.assertExists('#network-widget .oae-list-options button[data-type="oae-list-compact"]', 'The \'Compact\' list view button is present');
                    test.assertExists('#network-widget .oae-list-options button[data-type="oae-list-details"]', 'The \'Details\' list view button is present');
                    test.assertExists('#network-widget .oae-list-options button[data-type="oae-list-grid"]', 'The \'Grid\' list view button is present');
                    // Verify list options contain a search field
                    test.assertExists('#network-widget .oae-list-options #network-search-query', 'The search box is present');
                });
            });
        });
    };
