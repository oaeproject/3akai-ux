...
casper.test.begin('Widget - Network', function(test) {
    ...
    /**
     * Verify that current user has a follower
     */
    var verifyUserHasFollower = function() {
        casper.click('#network-widget .nav li a[href="#network-followers"]');
        // Give infinite scroll time to process
        casper.wait(configUtil().searchWaitTime, function() {
    ...
    /**
     * Verify that current user's following network is not empty
     */
    var verifyUserHasFollowing = function() {
        casper.click('#network-widget .nav li a[href="#network-followers"]');
        // Give infinite scroll time to process
        casper.wait(configUtil().searchWaitTime, function() {
            ...
            casper.then(function() {
                ...
                followUtil().follow(user2.id, function() {
                    ...
                    casper.then(function() {
                        ...
                        // Verify user 1 is in user 2's network
                        casper.thenOpen(configUtil().tenantUI + '/me/network', function() {
                            casper.waitForSelector('#network-widget .oae-list.oae-list-grid li', verifyUserHasFollower);
                        });
                        casper.then(function() {
                            // User2 follows user 1
                            followUtil().follow(user1.id, function() {
                                casper.thenOpen(configUtil().tenantUI + '/me/network', function() {
                                    casper.waitForSelector('#network-widget .oae-list.oae-list-grid li', verifyUserHasFollowing);
                                });
