...
casper.test.begin('Widget - Network', function(test) {
            ...
            casper.waitForSelector(
                ...
                function() {
                    test.assertDoesntExist(
                        ...
                        'Verify a user can be followed',
                    );
                    test.assertNotVisible(
                        'button.user-follow',
                        'Verify follow button is removed after following a user',
                    );
                },
            );
        });
    ...
    var verifyNetworkViewMode = function() {
        ...
        // Verify compact list
        casper.click(
            '#network-widget .oae-list-header-actions button[data-type="oae-list-compact"]',
        );
        test.assertExists(
            '.oae-list.oae-list-compact',
            'Network can be switched to compact view',
        );
        // Verify details list
        casper.click(
            '#network-widget .oae-list-header-actions button[data-type="oae-list-details"]',
        );
        test.assertExists(
            '.oae-list.oae-list-details',
            'Network can be switched to details view',
        );
        // Switch back to grid view
        casper.click(
            '#network-widget .oae-list-header-actions button[data-type="oae-list-grid"]',
        );
        test.assertExists(
            '.oae-list.oae-list-grid',
            'Network can be switched to grid view',
        );
    };
    ...
    var verifyNetworkElements = function() {
        casper.waitForSelector(
            '#network-widget .oae-list.oae-list-grid li',
            function() {
                // Verify there is a dummy list item with action buttons
                test.assertExists(
                    '#network-widget #network-following .oae-list li:first-child',
                    'Initial dummy list item is present',
                );
                // Verify dummy list item has a `Find people` button
                test.assertExists(
                    '#network-widget #network-following .oae-list li:first-child a[href="/search/all?types=user"]',
                    "The first list item has a 'Find people' link button",
                );
                // Verify that no other list items are present
                test.assertDoesntExist(
                    '#network-widget #network-following .oae-list li:nth-child(2)',
                    'No other list items are present',
                );
                // Verify that the network widget has a follower and following tab
                test.assertExists(
                    '#network-widget .nav li.active a[href="#network-following"]',
                    'Verify following tab is present and active by default',
                );
                test.assertExists(
                    '#network-widget .nav li a[href="#network-followers"]',
                    'Verify followers tab is present',
                );
                ...
                test.assertExists(
                    '#network-widget .nav li.active a[href="#network-followers"]',
                    'Verify followers tab is active after click',
                );
                // Give infinite scroll time to process
                casper.waitForSelector(
                    '#network-widget #network-followers .alert',
                    function() {
                        // Verify there is no dummy list item on the followers tab but an alert instead
                        test.assertDoesntExist(
                            '#network-widget #network-followers .oae-list li:first-child',
                            'Verify there is no initial dummy list item present in the followers tab',
                        );
                        test.assertExists(
                            '#network-widget #network-followers .alert',
                            'Verify there is an alert when no followers are currently available',
                        );
                        // Verify switching back to the following tab
                        casper.click(
                            '#network-widget .nav li a[href="#network-following"]',
                        );
                        test.assertExists(
                            '#network-widget .nav li.active a[href="#network-following"]',
                            'Verify following tab is active after click',
                        );
                        ...
                        casper.waitForSelector(
                            '#search-clip-container',
                            function() {
                                test.assertExists(
                                    '#search-clip-container',
                                    'The search people page can be triggered from the dummy list item',
                                );
                                ...
                                casper.then(function() {
                                    casper.waitForSelector(
                                        ...
                                        function() {
                                            ...
                                            test.assertExists(
                                                '#network-widget .oae-list-header-actions',
                                                'The list options are present',
                                            );
                                            // Verify list options contain a checkbox that selects all
                                            test.assertExists(
                                                '#network-widget .oae-list-header-actions input[type="checkbox"]',
                                                "The 'Select all' checkbox is present in the list options",
                                            );
                                            // Verify list options contain a `unfollow` button
                                            test.assertExists(
                                                '#network-widget .oae-list-header-actions .oae-trigger-unfollow',
                                                'The `Stop following` button is present in the list options',
                                            );
                                            ...
                                            test.assertExists(
                                                '#network-widget .oae-list-header-search .oae-list-header-search-query',
        ...
        casper.waitForSelector('.oae-list .alert-info', function() {
            ...
            casper.wait(configUtil.searchWaitTime, function() {
                ...
                casper.waitForSelector('.oae-list li', function() {
                    test.assertExists(
                        '.oae-list li',
                        "1 result is returned for '" + user.displayName + "'",
                    );
                    test.assertSelectorHasText(
                        '.oae-list li .oae-tile-metadata a',
                        user.displayName,
                        "The returned user has the name '" +
                            user.displayName +
                            "'",
                    );
                    // Reset the form
            ...
            function() {
                ...
                casper.waitForSelector(
                    ...
                    function() {
                        test.assertExists(
                            '#network-widget #network-followers .oae-list li:first-child',
                            'Verify user has a follower',
                        );
                        test.assertSelectorHasText(
                            '#network-widget #network-followers .oae-list li .oae-tile-metadata a',
                            user.displayName,
                            "The follower has the name '" +
                                user.displayName +
                                "'",
                        );
                        test.assertSelectorHasText(
                            '#network-widget #network-followers .oae-list li .oae-tile-metadata small',
                            user.tenant.displayName,
                            "The follower is a tenant of '" +
                                user.tenant.displayName +
                                "'",
                        );
                    },
    ...
    var verifyUserHasFollowing = function(user) {
        ...
        casper.waitForSelector(
            ...
            function() {
                test.assertExists(
                    '#network-widget #network-following .oae-list li:nth-child(2)',
                    "Verify user's following network is not empty",
                );
                test.assertSelectorHasText(
                    '#network-widget #network-following .oae-list li:nth-child(2) .oae-tile-metadata a',
                    user.displayName,
                    "The followed user has the name '" + user.displayName + "'",
                );
                test.assertSelectorHasText(
                    '#network-widget #network-following .oae-list li:nth-child(2) .oae-tile-metadata small',
                    user.tenant.displayName,
                    "The followed user is a tenant of '" +
                        user.tenant.displayName +
                        "'",
                );
            },
