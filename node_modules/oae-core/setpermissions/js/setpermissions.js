...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var setUpSetPermissionsModal = function() {
            ...
            oae.api.util.template().render('#setpermissions-template', widgetData, $('#setpermissions-container', $rootel));
            ...
            // Because there might be multiple instances of this widget on the same page,
            // we generate an ID that can be used to get the context once for this widget
            var id = oae.api.util.generateId();

            // Trigger and bind an event that will ask for the page context
            $(document).on('oae.context.send.setpermissions-' + id, function(ev, data) {
                pageContext = data;
                // Immediately pass the default permissions summary back to the widget caller
                setUpAutoSuggest(savePermissionsChange);
            });
            $(document).trigger('oae.context.get', 'setpermissions-' + id);
        };
