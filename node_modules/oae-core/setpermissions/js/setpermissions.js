...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var savePermissionsChange = function() {
            ...
            var selectedFolderAndPrincipalItems = _.partition(
                selection,
            ...
            var selectedCurrentUser = _.findWhere(selectedPrincipalItems, {
                id: oae.data.me.id,
            });
            ...
            var selectedOutsideContext = _.filter(
                selectedPrincipalItems,
                function(selectedPrincipalItem) {
                    return (
                        selectedPrincipalItem.id !== oae.data.me.id &&
                        selectedPrincipalItem.id !== pageContext.id
                    );
                },
            );
            ...
            var visibility = $(
                '#setpermissions-container input[type="radio"]:checked',
                $rootel,
            ).val();

            // Generate a user-friendly summary
            var summary = oae.api.util
                .template()
                .render($('#setpermissions-summary-template', $rootel), {
        ...
        var setUpSetPermissionsModal = function() {
            ...
            oae.api.util
                .template()
                .render(
                    $('#setpermissions-template', $rootel),
                    widgetData,
                    $('#setpermissions-container', $rootel),
                );

            $('#setpermissions-savepermissions', $rootel).on(
                'click',
                savePermissionsChange,
            );
            $('#setpermissions-cancelpermissions', $rootel).on(
                'click',
                cancelPermissionsChange,
            );

            // Trigger and bind an event that will ask for the page context
            $(document).on('oae.context.send.setpermissions.' + uid, function(
                ev,
                data,
            ) {
                pageContext = data;
