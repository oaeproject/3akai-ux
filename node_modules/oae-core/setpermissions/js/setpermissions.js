    ...
    return function(uid, showSettings, widgetData) {
        ...
        var savePermissionsChange = function() {
            ...
            $.each(selection, function(index, selectedItem) {
                // Determine whether or not the context is different from the current user. If so,
                // extract the display name of that context to incorporate in the summary message
                if (oae.data.me.id !== pageContext.id && selectedItem.id === pageContext.id) {
                    context = selectedItem.displayName;
                }
                ...
                } else if (selectedItem.id !== oae.data.me.id) {
                    // Don't add the context to the list of users/groups the item is shared
                    // with. This will be treated differently in the summary
                    if (selectedItem.id !== pageContext.id) {
                        shared.push(selectedItem);
                    }
                    viewers.push(selectedItem.id);
                }
            });
            ...
            $(document).trigger('oae.setpermissions.changed.' + uid, {
                ...
                'viewers': viewers,
                'summary': oae.api.util.template().render($('#setpermissions-summary-template', $rootel), {
                    'context': context,
