...
define(['jquery', 'oae.core', 'jquery.autoSuggest'], function($, oae) {
     ...
     * @param  {String}     type            The type of item that will be added to the system (one of `file`, `link` or `document`)
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var getAutosuggestSelection = function() {
            ...
            $.each($rootel.find('.as-selections > li'), function(index, selection) {
                ...
                if (id) {
                    viewers.push({
                        'id': id,
                        'displayName': $(selection).data().data.displayName
                    });
                }
            });
            return viewers;
         ...
         *
         * @param  {String}   type    The type of the items handled by the widget ('file', 'link' or 'document')
         * @param  {Number}   count   The number of items that are handled by the widget
         * @return {String}           Returns the translated string associated to the type of items handled by the widget (e.g. `links`, `file`, `document`)
         */
        var getTypei18n = function(type, count) {
            if (count > 1) {
                switch(type) {
                    case 'document':
                        return oae.api.i18n.translate('__MSG__DOCUMENT__');
                    case 'file':
                ...
                switch(type) {
                    ...
                    // Currently, it is not possible to create multiple documents at the same time
                }
            }
        };
        ...
        var savePermissionsChange = function() {
            ...
            // the user's personal space. If so, that context is treated slightly special in the
            // summary message
            var context = null;
