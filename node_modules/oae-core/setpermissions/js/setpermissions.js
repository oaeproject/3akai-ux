...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var savePermissionsChange = function() {
            ...
            // Collect all folders to which the items should be added
            var folders = [];
            $.each(viewers, function(index, viewer) {
                if (viewer.resourceType === 'folder') {
                    folders.push(viewer);
                }
            });

            // Filter the current user, the current page context and all folders out of the list of people to
            // share with, as they will be shown slightly different in the summary message
            var shared = [];
            $.each(viewers, function(index, viewer) {
                if (viewer.id !== oae.data.me.id && viewer.id !== pageContext.id && viewer.resourceType !== 'folder') {
                    shared.push(viewer);
                }
            });
