...
define(['jquery', 'oae.core', 'jquery.autoSuggest'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
         ...
         *
         * @param  {String}   type    The type of the items handled by the widget ('file', 'link' or 'collabdoc')
         * @param  {Number}   count   The number of items that are handled by the widget
         * @return {String}           Returns the translated string associated to the type of items handled by the widget (e.g. `links`, `file`, `collabdoc`)
         */
        var getTypei18n = function(type, count) {
            if (count > 1) {
                switch(type) {
                    ...
                    // Currently, it is not possible to create multiple collaborative documents at the same time
                }
            } else {
                switch(type) {
                    case 'collabdoc':
