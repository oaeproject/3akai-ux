...
define(['jquery', 'oae/api/oae.core'], function($, oae) {
    ...
    return function(uid) {
        ...
        var handleEmptyResultList = function() {
            oae.api.util.renderTemplate($('#searchcontent_noresults_template'), null, $('#searchcontent_results_container'));
        };

        var handleResultList = function(data) {
            $('#searchcontent_numberFound').text(data.total);
            if (!data.total) {
                return handleEmptyResultList();
            }

            $('#searchpeople_numberFound').text(data.total);
            oae.api.util.renderTemplate('#searchcontent_template', {
                'data': data
            }, $('.oae-list', $rootel));
        };
        ...
        var search = function(query) {
            ...
            var url = '/api/search/general/content?limit=12';
            if (query) {
                url += '&q=' + query;
            }
            ...
            $.ajax({
                ...
                'error': function(jqXHR, textStatus) {
                    console.log({'code': jqXHR.status, 'msg': jqXHR.statusText});
                }
            });
        };
