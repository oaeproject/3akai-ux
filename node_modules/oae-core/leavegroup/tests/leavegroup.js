...
casper.test.begin('Widget - Leave group', function(test) {
    ...
    var openLeaveGroup = function(groupID) {
        casper.waitForSelector('li.oae-list-actions + li', function() {
            ...
            test.assertExists(
                ...
                'At least one group is available in the group library',
            );
            casper.click(
                'li input[type="checkbox"][data-id="' + groupID + '"]',
            );
            // Verify that the leavegroup button is enabled and present
            test.assertExists(
                '.oae-list-header-actions .oae-trigger-leavegroup:not([disabled])',
                'The leave group trigger exists and is enabled',
            );
            casper.click('.oae-list-header-actions .oae-trigger-leavegroup');
            casper.wait(configUtil.modalWaitTime, function() {
                test.assertVisible(
                    '#leavegroup-modal',
                    'The leave group modal is showing after trigger',
                );
            });
        });
    };
    ...
    var verifyLeaveGroupElements = function() {
        ...
        test.assertExists(
            '#leavegroup-selected-container li',
            'The groups that will be left are listed',
        );
        // Verify there is a warning shown in the UI
        test.assertExists(
            '#leavegroup-modal-content .alert-danger',
            'A warning is shown before leaving the groups',
        );
        ...
        test.assertExists(
            '.modal-footer button#leavegroup-leave',
            "The 'Leave group' button is present",
        );
    };
        ...
        casper.waitForSelector(
            ...
            function() {
                test.assertExists(
                    ...
                    'As the only manager I cannot leave a group',
                );
            },
        );
    };
        ...
        casper.waitForSelector(
            ...
            function() {
                test.assertDoesntExist(
                    '#oae-notification-container .alert.alert-error',
                    'Successfully left a group that has one other manager',
                );
            },
        );
    };
            ...
            groupUtil.createGroup(null, null, null, null, null, null, function(
                ...
                groupUtil.createGroup(
                    ...
                    function(err, group2Profile) {
                        ...
                        casper.then(function() {
                            casper.echo(
                                '# Verify leave group as only manager fails',
                                'INFO',
                            );
                        ...
                        casper.then(function() {
                            casper.echo(
                                '# Verify leave group that has other manager',
                                'INFO',
                            );
                            casper.then(function() {
