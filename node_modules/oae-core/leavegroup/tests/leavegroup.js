casper.test.begin('Widget - Leave group', function(test) {
    ...
    var verifyOpenLeaveGroup = function(groupID) {
        casper.waitForSelector('li.oae-list-actions + li', function() {
            ...
            casper.test.assertExists('li.oae-list-actions + li', 'At least one group is available in the group library');
            casper.click('li input[type="checkbox"][data-id="' + groupID + '"]');
            // Verify that the leavegroup button is enabled and present
            casper.test.assertExists('.oae-list-options-actions .oae-trigger-leavegroup:not([disabled])', 'The leave group trigger exists and is enabled');
            casper.click('.oae-list-options-actions .oae-trigger-leavegroup');
            casper.wait(1000, function() {
                casper.test.assertVisible('#leavegroup-modal', 'The leave group modal is showing after trigger');
            });
        });
    };
    ...
    var verifyLeaveGroupElements = function() {
        ...
        casper.test.assertExists('#leavegroup-selected-container li', 'The groups that will be left are listed');
        // Verify there is a warning shown in the UI
        casper.test.assertExists('#leavegroup-modal-content .alert-danger', 'A warning is shown before leaving the groups');
        // Verify the cancel button is present
        casper.test.assertExists('.modal-footer button[data-dismiss="modal"]', 'The cancel button is present');
        // Verify the leave group button is present
        casper.test.assertExists('.modal-footer button#leavegroup-leave', 'The \'Leave group\' button is present');
    };
    ...
    var verifyLeaveGroupAsOnlyManager = function() {
        ...
        casper.waitForSelector('#oae-notification-container .alert', function() {
            casper.test.assertExists('#oae-notification-container .alert.alert-error', 'As the only manager I cannot leave a group');
        });
    };
    ...
    var verifyLeaveGroupWithOtherManager = function() {
        ...
        casper.waitForSelector('#oae-notification-container .alert', function() {
            casper.test.assertDoesntExist('#oae-notification-container .alert.alert-error', 'Successfully left a group that has one other manager');
        });
    };
