<!-- CSS -->
<div class="searchpeople_widget" id="searchpeople_widget">
    <div class="searchpeople_content_main">
        <div class="oae-search-refineby search_tags_refine_container"></div>
        <div class="oae-left oae-container oae-search-header" id="results_header">
            <h2 class="oae-search-header-type">__MSG__PEOPLE__ (<strong id="searchpeople_numberFound">0</strong><span class="oae-aural-text"> __MSG__PEOPLE_FOUND__</span>)</h2>
            <div class="oae-search-selects">
                <!-- FACETED WIDGET -->
                <div id="widget_faceted_449529953" class="searchpeople_facted widget_inline"></div>
                <div class="oae-search-sort">
                    __MSG__SORT_BY:__ <select>
                        <option data-sort-order="desc" data-sort-on="score">__MSG__RELEVANCE__</option>
                        <option data-sort-order="desc" data-sort-on="_lastModified">__MSG__CHANGED_RECENTLY__</option>
                        <option data-sort-order="asc" data-sort-on="lastName">__MSG__A_Z__</option>
                        <option data-sort-order="desc" data-sort-on="lastName">__MSG__Z_A__</option>
                    </select>
                </div>
                <div class="oae-search-listview-options oae-button oae-header-button oae-header-smaller-button">
                    <div class="search_view_grid" title="__MSG__GRID_VIEW__">
                        <div class="oae-action-icon oae-search-results-gridview"></div>
                    </div>
                    <div class="search_view_list" title="__MSG__LIST_VIEW__">
                        <div class="oae-action-icon oae-search-results-listview"></div>
                    </div>
                </div>
            </div>
        </div>
        <!-- TEMPORARY SEARCH TEXT -->
        <div id="result_temp" style="display:none;">
            <div class="padded_content">
                <b>__MSG__SEARCHING__ ...</b>
            </div>
        </div>
        <div class="searchpeople_results_faceted">
            <span id="search_result_title" style="display:none;">__MSG__REFINE_YOUR_SEARCH__</span>
            <span id="search_result_all_people" style="display:none;">__MSG__ALL_PEOPLE__</span>
            <span id="search_result_my_contacts" style="display:none;">__MSG__MY_CONTACTS__</span>
            <span id="search_result_my_contacts_invitation" style="display:none;">__MSG__MY_CONTACTS_INVITATIONS__</span>
            <span id="search_result_pending_invitations" style="display:none;">__MSG__PENDING_INVITATIONS__</span>
        </div>
        <div id="searchpeople_results" class="oae-container searchpeople_results oae-search-results">
            <!-- SEARCH RESULTS -->
            <div id="results_page1" class="results_container results_container_sub">
                <!-- CONTENT -->
                <div class="oae-container results_part">
                    <!-- CONTAINER FOR THE RESULTS -->
                    <ul id="searchpeople_results_container" class="oae-search-results-container"></ul>
                </div>
                <div id="searchpeople_noresults_template"><!--
                    <li class="oae-no-results-container">
                        <div class="oae-no-results-arrow-up"></div>
                        <div class="oae-no-results-icon oae-no-results-search-people less-margin"></div>
                        <p>__MSG__NO_RESULTS_DESCRIPTION_SEARCH__ {if sakai.data.me.user.anon} <a href="/register" class="oae-regular-links">__MSG__SIGN_UP__</a>{/if}</p>
                    </li>
                --></div>
            </div>
        </div>
    </div>
</div>
<!-- JAVASCRIPT -->
<script type="text/javascript" src="/node_modules/oae-core/searchpeople/javascript/searchpeople.js"></script>
