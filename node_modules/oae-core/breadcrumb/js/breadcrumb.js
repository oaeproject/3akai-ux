...
define(['jquery', 'underscore', 'oae.core'], function($, _, oae) {
	...
	return function() {
		...
		var updateBreadcrumb = function(widgetData, callback) {
	        ...
	        if (widgetData === oae.data.me) {
	            ...
	            if (docCookies.getItem(cookieName)) {
	                ...
	                if (!_.isEmpty(_.filter(breadcrumb, function(page) { return page.location == breadcrumbLocation}))) {
	                    var indexOfActualLocation = breadcrumb.map(function(page) { return page.location; }).indexOf(breadcrumbLocation);
	                ...
	                // The user has visited another page, add this page to the breadcrumb
	                    ...
	                    breadcrumb.push(
	                        { 'name' : widgetData.displayName, 'location' : breadcrumbLocation }
	                    );
	                    docCookies.setItem(cookieName, JSON.stringify(breadcrumb), null, '/', dotDomain);
	                    console.log(breadcrumb)
	                    callback();
	     ...
	     */
		var insertBreadcrumbIntoPage = function(tag, composant) {
			tag.prepend(composant);
	     ...
	     */
		$(document).on('oae.trigger.breadcrumb', function(ev, data) {
			updateBreadcrumb(data.widgetData, function() {

				var breadcrumb = $.parseJSON(docCookies.getItem(cookieName));

				// If we have only 1 element into the breadcrumb (the actual page) don't display the breadcrumb
				if (Object.keys(breadcrumb).length > 1) {
					drawBreadcrumb(function(composant) {
						insertBreadcrumbIntoPage(data.tag, composant);
					});
				}
			});
		});
