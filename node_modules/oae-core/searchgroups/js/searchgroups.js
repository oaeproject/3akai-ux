...
define(['jquery', 'oae/api/oae.core'], function($, oae) {
    ...
    return function(uid) {
        ...
        var handleEmptyResultList = function() {
            oae.api.util.renderTemplate($('#searchgroups_noresults_template'), null, $('#searchgroups_results_container'));
        };

        var handleResultList = function(data) {
            $('#searchgroups_numberFound').text(data.total);
            if (!data.total) {
                return handleEmptyResultList();
            }

            $('#searchpeople_numberFound').text(data.total);
            oae.api.util.renderTemplate('#searchgroups_template', {
                'data': data
            }, $('.oae-list', $rootel));
        };
        ...
        var search = function(query) {
            ...
            var url = '/api/search/general/group?limit=12';
            if (query) {
                url += '&q=' + query;
            }
