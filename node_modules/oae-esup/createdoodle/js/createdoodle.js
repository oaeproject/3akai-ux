    ...
    return function(uid, showSettings, widgetData) {
        ...
        var setUpReset = function() {
            $('#creatediscussion-modal', $rootel).on('hidden.bs.modal', function() {
                // Reset the form
                var $form = $('#createdoodle-form', $rootel);
                $form[0].reset();
        ...
        /**
         * Initialize the create doodle form and validation
         */
        var setUpCreateDoodle = function() {
            var validateOpts = {
                'submitHandler': createDoodle
            };
            oae.api.util.validation().validate($('#createdoodle-form', $rootel), validateOpts);
        };
        ...
        var showOverview = function() {
            ...
            $('#createdoodle-form > .modal-footer', $rootel).show();
            $('#createdoodle-overview-container', $rootel).show();
        };
        ...
        var setUpSetPermissions = function() {
            ...
            $(document).on('oae.setpermissions.changed.' + setPermissionsId, function(ev, data) {
                // Update visibility for doodle
                visibility = data.visibility;

                // Always make the current context a manager
                managers.push(contextData.id);
                members = _.without(data.shared, contextData.id);

                // When the current user is also part of the list of people to share with,
                // add them as a manager as well
                if (_.contains(data.shared, oae.data.me.id)) {
                    managers.push(oae.data.me.id);
                    members = _.without(data.shared, oae.data.me.id);
                }

                // Add the permissions summary
                $('#createdoodle-permissions', $rootel).html(data.summary);
            ...
            // Determine the default selection of people and groups the doodle should be shared with
            var preFill = [];
            var ghosts = [];
            ...
            // If the current user is creating the doodle from its personal space,
            // `My Library` is added as a fixed item
            if (contextData.id === oae.data.me.id) {
                preFill.push($.extend({
                    'fixed': true
                }, userItem));
            // If the current user is creating the doodle from within a group,
         ...
         * Create the doodle.
         */
        var createDoodle = function() {

            // Disable the form
            $('#createdoodle-form *', $rootel).prop('disabled', true);

            var displayName = $.trim($('#createdoodle-name', $rootel).val());
            var description = $.trim($('#createdoodle-description', $rootel).val());

            oae.api.content.createDoodle(displayName, description, visibility, managers, [], [], function (err, data) {
                if (!err) {
                    window.location = data.profilePath;
                }
                else {
                    // Re-enable the form
                    $('#createdoodle-form *', $rootel).prop('disabled', false);

                    oae.api.util.notification(
                        oae.api.i18n.translate('__MSG__DOODLE_NOT_CREATED__', 'createdoodle'),
                        oae.api.i18n.translate('__MSG__DOODLE_COULD_NOT_BE_CREATED__', 'createdoodle'),
                        'error');
                }
        ...
        /**
         * Initialize the create doodle modal dialog
         */
        var setUpCreateDoodleModal = function () {

            $(document).on('click', '.oae-trigger-createdoodle', function () {
                // Request the context information
                $(document).trigger('oae.context.get', 'createdoodle');
            });

            // Receive the context information and cache it
            $(document).on('oae.context.send.createdoodle', function(ev, ctx) {
                contextData = ctx;

                $('#createdoodle-modal', $rootel).modal({
                    'backdrop': 'static'
                });
            });

            $('#createdoodle-modal', $rootel).on('shown.bs.modal', function() {
                // Set focus to the discussion topic field
                $('#createdoodle-name', $rootel).focus();
