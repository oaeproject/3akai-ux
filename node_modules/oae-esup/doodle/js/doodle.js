...
define(['jquery', 'oae.core', 'moment', 'bootstrap', 'bootstrap.datetimepicker'], function($, oae, moment) {
     ...
     */
    var handleNotifications = function (widgetId, type) {
        ...
        if (!alreadySend) {
            ...
            oae.api.doodle.sendNotifications(widgetId, notificationType, function (err) {
                if (err)
    ...
    var updateLoodleView = function (loodle) {
        ...
        var table = '<table class="table table-striped table-hover" id="loodle-table">';
    ...
    var updateDeleteScheduleView = function (loodle) {
        ...
        var select = '<select name="scheduleId" class="form-control">';
        ...
        $('select[name="scheduleId"]').replaceWith(select);
        ...
        var setUpLoodleEvents = function () {
            ...
            $(document).on('click', '#addScheduleButton', function (ev) {
                ...
                oae.api.doodle.addSchedule(widgetData.loodleData.id, $('#begin_time').val(), $('#end_time').val(), function (err) {
                    if (err) {
                        console.log('An error occured trying to add a schedule : ', err);
                    }
                    else {
                        ...
                        handleNotifications(widgetData.id, 'schedule');
            ...
            /**
             * Callback on the click on the delete schedule button, "this" représente les données du widget
             *
            ...
            $(document).on('click', '#save-votes', function (ev) {
                ...
                var votes = [];
                for (var i = 0; i < $votes.length; i++) {
                    votes.push({id: $($votes[i]).data('id'), vote: ($votes[i].checked) ? 1 : 0});
                }
                 **/
                ...
                oae.api.doodle.updateVotes(votes, widgetData.loodleData.id, function (err) {
                    ...
                    handleNotifications(widgetData.id, 'vote');
