...
define(['jquery', 'oae.core', 'moment', 'bootstrap', 'bootstrap.datetimepicker'], function($, oae, moment) {
    ...
    /**
     * Format the user votes in time order
     * @param votes
     */
    var formatVotes = function (user, loodle) {
        ...
        user.votes.forEach(function (vote) {
            // Detect the position of the schedule
            var index = 0;
            for (var i = 0; i < loodle.defaultSchedules.length; i++) {
                if (loodle.defaultSchedules[i].id == vote.schedule_id) {
                    index = i;
                }
            }

            // Put the vote in the right position in the ordered array
            orderedVotes[index] = vote;
        });

        user.votes = orderedVotes;
    };
    ...
    var handleNotifications = function (widgetData, type) {
        ...
        if (!(widgetData.loodleData.currentUser.status === 'temporary')) {
            ...
            if (!alreadySend) {
                oae.api.loodle.sendNotifications(widgetData.id, notificationType, function (err) {
                    if (err) {
                        console.error('An error occured : ', err);
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__LOODLE_NOTIFICATIONS_NOT_SENT__', 'loodle'),
                            oae.api.i18n.translate('__MSG__LOODLE_NOTIFICATIONS_COULD_NOT_BE_SENT__', 'loodle'));
                    }
                });
            }
    ...
    var updateLoodleDisplay = function (widgetData, locale) {
        ...
        oae.api.loodle.getData(widgetData.loodleData.id, function (err, loodle) {

            if (err) {
                console.error('An error occured trying to get the loodle data : ', err);
                oae.api.util.notification(
                    oae.api.i18n.translate('__MSG__UNEXPECTED_ERROR__', 'loodle'),
                    oae.api.i18n.translate('__MSG__LOODLE_DISPLAY_COULD_NOT_BE_UPDATED__', 'loodle'));
                return;
            }

            // Sort the users array so that the current user always has his/her name at the bottom of the table
            var index = 0;
            var currentUser = {};
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var setUpLoodleEvents = function () {
            ...
            $(document).on('click', '#addScheduleButton', function (ev) {
                ...
                oae.api.loodle.addSchedule(widgetData.loodleData.id, $('#begin_time').val(), $('#end_time').val(), function (err) {
                    if (err) {
                        console.error('An error occured : ', err);
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__LOODLE_CHANGES_NOT_SAVED__', 'loodle'),
                            oae.api.i18n.translate('__MSG__LOODLE_CHANGES_COULD_NOT_BE_SAVED__', 'loodle'));
                    }
                    else {
            ...
            var deleteSchedule = function (ev) {
                ...
                oae.api.loodle.deleteSchedule(this.loodleData.id, $('select[name="scheduleId"]').val(), function (err) {
                    if (err) {
                        console.error('An error occured trying to delete a schedule : ', err);
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__LOODLE_CHANGES_NOT_SAVED__', 'loodle'),
                            oae.api.i18n.translate('__MSG__LOODLE_CHANGES_COULD_NOT_BE_SAVED__', 'loodle'));
                        return;
            ...
            $(document).on('click', '#save-votes', function (ev) {
                ...
                oae.api.loodle.updateVotes(votes, widgetData.loodleData.id, function (err) {
                    if (err) {
                        console.error('An error occured : ', err);
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__LOODLE_CHANGES_NOT_SAVED__', 'loodle'),
            ...
            $(document).on('click', '#save-changes', function (ev) {
                ...
                if (splitName.length === 2) {
                    ...
                    if (splitName[0] === "") {
                        ...
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__LOODLE_CHANGES_NOT_SAVED__', 'loodle'),
                            oae.api.i18n.translate('__MSG__INVALID_USER_NAME__', 'loodle'));
                    }
                    else {
                        firstName = splitName[0];
                    }
                ...
                oae.api.loodle.addMemberWithVotes(loodle.id, firstName, lastName, votes, function (err, newUser) {
                    if (err) {
                        console.error('An error occured : ', err);
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__LOODLE_CHANGES_NOT_SAVED__', 'loodle'),
        ...
        oae.api.util.template().render($('#loodle-template', $rootel), {
            'loodle': widgetData.loodleData,
            'navigationBar': navigationBar
            // 'currentUser': {}
        },
        $('#loodle-container', $rootel));
