...
casper.test.begin('Widget - Import Users', function(test) {
     ...
     */
    var verifyOpenImportUsers = function() {
    ...
    var verifyCSVImport = function() {
        userUtil().doLogIn('user1@example.com', configUtil().defaultUserPassword);
    ...
    casper.start(configUtil().adminUI, function() {
        ...
        casper.then(function() {
            casper.echo('# Verify open import users modal', 'GREEN_BAR');
            ...
            casper.thenOpen(configUtil().adminUI + '/tenant/' + configUtil().tenantAlias + '/usermanagement', function() {
                verifyOpenImportUsers();
            });
        });

        casper.then(function() {
            casper.echo('# Verify import users elements', 'GREEN_BAR');
            verifyImportUsersElements();
        });

        casper.then(function() {
            casper.echo('# Verify import users form validation', 'GREEN_BAR');
            verifyImportUsersFormValidation();
        });

        casper.then(function() {
            casper.echo('# Verify import users CSV upload', 'GREEN_BAR');
        ...
        casper.thenOpen(configUtil().tenantUI, function() {
            casper.echo('# Verify CSV import results in usable users', 'GREEN_BAR');
            verifyCSVImport();
        });
    });
