...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        var createUser = function() {
            ...
            // Whether or not the user will be a tenant/global admin user
            var isAdmin = true;
            // If we're on the global tenant the new user is a global admin by default
            if (currentContext.isGlobalAdminServer) {
                additionalOptions.isGlobalAdmin = true;
            // If we're creating a user on a regular tenant we check the checkbox value
            } else {
                isAdmin = $('#createuser-tenantadmin', $rootel).is(':checked');
                additionalOptions.isTenantAdmin = isAdmin;
            }

            // Determine which API function to call to create the user
            var createUser = isAdmin ? oae.api.admin.createTenantAdminUser : oae.api.admin.createUser;
            // If we're creating a user on the global tenant it will always be a global administrator
            if (currentContext.isGlobalAdminServer) {
                createUser = oae.api.admin.createGlobalAdminUser;
            }
            ...
            createUser(tenantAlias, values.username, values.password, displayName, additionalOptions, function(err, createdUser) {
                if (err) {
                    ...
                    $(document).trigger('oae.createuser.done');
                }
            });
        };
