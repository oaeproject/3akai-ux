...
casper.test.begin('Widget - Create user', function(test) {
    ...
    var verifyCreatingUser = function() {
        casper.waitForSelector('form#createuser-form', function() {
            casper.fill('form#createuser-form', {
                ...
                'password': configUtil().defaultUserPassword,
                'password_repeat': configUtil().defaultUserPassword
            }, false);
    ...
    casper.start(configUtil().adminUI, function(){
        ...
        casper.thenOpen(configUtil().adminUI, function() {
            casper.echo('# Verify opening create user modal on the global tenant', 'GREEN_BAR');
        ...
        casper.then(function() {
            casper.echo('# Verify create user form elements', 'GREEN_BAR');
            verifyCreateUserForm();
        });

        // Verify that all the errors are shown when there is wrong or no input
        casper.then(function() {
            casper.echo('# Verify create user form validation', 'GREEN_BAR');
            verifyCreateUserFormValidation(user1.username, true);
        });

        // Verify creating a user
        casper.then(function() {
            casper.echo('# Verify creating a user on the global tenant', 'GREEN_BAR');
            verifyCreatingUser();
        });

        // Verify creating a user
        casper.then(function() {
            casper.echo('# Verify creating a user on a regular tenant', 'GREEN_BAR');
