...
<div id="tenants-template"><!--
    ...
    {if _.keys(tenants).length > 0}
        <table class="table table-striped admin-table-striped">
            <tr>
                ...
                {else}
                    <th>__MSG__HOST__</th>
                {/if}
                <th>__MSG__EMAIL_DOMAIN__</th>
                <th>Status</th>
                {if isOnGlobalAdminServer}
                    <th>
                        {if context.isGlobalAdminServer}
                            {if hasStoppedServer}
                                <button type="button" class="btn btn-success tenants-start-all">__MSG__START_ALL__</button>
                            {else}
                                <button type="button" class="btn btn-warning tenants-stop-all">__MSG__STOP_ALL__</button>
                            {/if}
                        {/if}
                    </th>
                    <th></th>
            ...
            {for tenant in tenants}
                {if tenant.alias}
                    ...
                    <tr>
                        {if context.isGlobalAdminServer}
                            <td><a href="/tenant/${tenant.alias|encodeForURL}" title="__MSG__EDIT_TENANT__"><i class="fa fa-pencil"></i></a></td>
                        {/if}
                        ...
                        {if oae.data.me.isGlobalAdmin}
                            <td class="jeditable-container">
                                <div class="jeditable-field" data-alias="${tenant.alias|encodeForHTMLAttribute}" data-field="emailDomain">
                                    ${tenant.emailDomain|encodeForHTML}
                                </div>
                            </td>
                        {else}
                            <td>${tenant.emailDomain|encodeForHTML}</td>
                        {/if}
                        <td>
                            {if tenant.active}
            ...
            <div>
                ...
                <div class="hide clearfix admin-table-striped-content-container">
                    <form id="tenants-create-form" class="form-horizontal col-sm-7" role="form">
                        ...
                        <div class="form-group">
                            <label for="tenants-create-email-domain" class="col-xs-12 col-sm-3 control-label">__MSG__EMAIL_DOMAIN_COLON__</label>
                            <div class="col-xs-12 col-sm-9">
                                <input type="text" name="emailDomain" id="tenants-create-email-domain" placeholder="__MSG__EMAIL_DOMAIN__" class="form-control required url"/>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-xs-12">
