...
define(['jquery', 'oae.core', 'jquery.jeditable'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
         ...
         * @param  {Object}     callback.requestInfo.data   The data to include as the POST body of the authentication request
         */
        ...
        var loginOnTenantHandler = function() {
            ...
            getSignedTenantRequestInfo(tenantAlias, function(err, data) {
                if (err) {
                    oae.api.util.notification(oae.api.i18n.translate('__MSG__TOKEN_ERROR__', 'tenants'),
                        oae.api.i18n.translate('__MSG__COULD_NOT_SIGN_INTO_TENANT__', 'tenants'), 'error');
                } else {
                    // Any data that can be HTML encoded should be string. Ensure the data for our auth request doesn't
                    // have anything like primitive numbers or booleans
                    $.each(data.body, function(key, value) {
                        data.body[key] = value.toString();
                    });

                    // Fill in our hidden form and submit it. A form is being used because it is a cross-domain request
                    oae.api.util.template().render($('#tenants-login-form-template', $rootel), data, $('#tenants-login-container', $rootel));
