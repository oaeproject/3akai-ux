...
define(['jquery', 'oae.core', 'jquery.jeditable'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var setUpInfiniteScroll = function() {
            ...
            infinityScroll = $('#tenants-row-container', $rootel).infiniteScroll(url, {'limit': 50}, $('#tenants-row-template', $rootel), {
                'postProcessor': function(data) {
                    tenantsCache = _.indexBy(data.results, 'alias');
                    return {'tenants': data.results, 'context': widgetData.context, 'results': _.pluck(data, 'alias')};
                },
                'emptyListProcessor': function() {
                    oae.api.util.template().render($('#tenants-none-template', $rootel), null, $('#tenants-row-container', $rootel));
                },
                'initialContent': function() {
                    return oae.api.util.template().render($('#tenants-listhead-template', $rootel));
                }
            });
        };
        ...
        var renderTenant = function(tenant) {
            ...
            if ($target.length > 0) {
                ...
                $('#tenants-row-container').append(rowContent);
            }
        };
