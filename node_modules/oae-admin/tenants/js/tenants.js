...
define(['jquery', 'oae.core', 'jquery.jeditable'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        /**
         * Start all tenants in the system and show a confirmation dialog.
         */
        ...
        var createTenant = function() {
            ...
            var opts = {};
            var emailDomain = $.trim($('#tenants-create-email-domain', $rootel).val());
            if (emailDomain) {
                opts.emailDomain = emailDomain;
            }

            oae.api.admin.createTenant(alias, displayName, host, opts, function(err, tenant) {
                if (err) {
                    oae.api.util.notification(
            ...
            $.ajax({
                ...
                'success': function() {
                    oae.api.util.notification(
                        ...
                        oae.api.i18n.translate('__MSG__TENANT_' + fieldLabel + '_SUCCESSFULLY_UPDATED__', 'tenants'));

                    callback();
                },
                'error': function(jqXHR, textStatus) {
                    oae.api.util.notification(
                        oae.api.i18n.translate('__MSG__TENANT_' + fieldLabel + '_FAILED__', 'tenants'),
                        oae.api.i18n.translate('__MSG__TENANT_' + fieldLabel + '_FAILED_TO_UPDATE__', 'tenants'),
                        'error');
        ...
        var enableInlineEdit = function() {
            ...
            $('.jeditable-field', $rootel).editable(function(value) {
                ...
                // Don't do anything if the field didn't change
                if (value === $.trim(inlineEdit.revert)) {
                    return inlineEdit.revert;
                }
                ...
                if (field === 'displayName') {
                    ...
                    if (!value || !checkValidHostname(value)) {
                        oae.api.util.notification(
                ...
                updateTenant($inlineEdit.attr('data-alias'), field, value, function(err) {
                    if (err) {
                        // Revert the field if updating failed
                        if (inlineEdit.revert) {
                            $inlineEdit.html(inlineEdit.revert);
                        } else {
                            $inlineEdit.html(placeholderContent);
                        }

                        // Revert the link to the tenant landing page if the host was updated
                        if (field === 'host') {
                            $('a', $inlineEdit.parent().prev()).attr('href', '//' + inlineEdit.revert);
                        }
                    }
                });
        ...
        var renderTenants = function() {
            ...
            // Sort the tenants alphabetically based on the tenant alias
