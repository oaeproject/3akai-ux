...
define(['jquery', 'oae.core', 'jquery.jeditable', 'jquery.history'], function($, oae) {
    ...
    return function(uid, showSettings, widgetData) {
            ...
            $.ajax({
                ...
                'success': function(data) {
                    ...
                    $.each(tenants, function(index, alias) {
                        tenantsCache[alias].active = false;
                        renderTenant(tenantsCache[alias]);
                    });
                    callback();
                },
        ...
        var startTenants = function(tenants, callback) {
            $.ajax({
                ...
                'success': function(data) {
                    ...
                    $.each(tenants, function(index, alias) {
                        tenantsCache[alias].active = true;
                        renderTenant(tenantsCache[alias]);
                    });
                    callback();
                },
        ...
        var createTenant = function() {
            var alias = $.trim($('#tenants-create-alias', $rootel).val());
            var displayName = $.trim($('#tenants-create-displayName', $rootel).val());
            var host = $.trim($('#tenants-create-host', $rootel).val());
         ...
         *
         * @param  {Function}   postRenderer                    A function to invoke each time new items are added to the list
         * @param  {Object}     postRenderer.data               The data that was queried for the items that were added
         * @param  {jQuery}     postRenderer.$templateOutput    The jQuery elements that were added
         */
        var setUpInfiniteScroll = function(postRenderer) {
            ...
            infinityScroll = $('#tenants-table-rows', $rootel).infiniteScroll(url, {'limit': 50}, $('#tenants-row-template', $rootel), {
                'postProcessor': function(data) {
                    tenantsCache = _.indexBy(data.results, 'alias');
                    return _.extend({}, data, {'context': widgetData.context});
                },
                'emptyListProcessor': function() {
                    oae.api.util.template().render($('#tenants-none-template', $rootel), null, $('#tenants-table-rows', $rootel));
                },
        ...
        var renderTenants = function() {
            ...
            // When the search tenants form is submit, we intercept and refresh the infinite scroll
            // with the results
            $('#tenants-search > form', $rootel).on('submit', function() {
                History.pushState({'query': $('#tenants-search-query').val()});
                return false;
            });
        };
        ...
        var renderTenant = function(tenant) {
            var $target = $('#tenants-row-' + oae.api.util.security().encodeForHTMLAttribute(tenant.alias), $rootel);
            var rowContent = oae.api.util.template().render($('#tenants-row-template', $rootel), {
            ...
            if ($target.length > 0) {
                ...
                $('#tenants-table-rows').append(rowContent);
            }
