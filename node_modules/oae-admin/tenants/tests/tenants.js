...
casper.test.begin('Widget - Tenants', function(test) {
    ...
    casper.start(configUtil().adminUI, function() {
        ...
        casper.echo('# Verify the create tenant form validation', 'GREEN_BAR');
        casper.then(verifyCreateTenantValidation);

        casper.then(function() {
            // Create a new tenant
            casper.echo('# Verify creating a new tenant', 'GREEN_BAR');
            verifyCreateNewTenant(function(tenantID) {
                // Rename the new tenant
                casper.then(function() {
                    casper.echo('# Verify renaming a tenant', 'GREEN_BAR');
                    verifyRenameTenant(tenantID);
                });

                // Stop the new tenant
                casper.then(function() {
                    casper.echo('# Verify stopping a tenant', 'GREEN_BAR');
                    verifyStopTenant(tenantID);
                });

                // Start the new tenant
                casper.then(function() {
                    casper.echo('# Verify starting a tenant', 'GREEN_BAR');
