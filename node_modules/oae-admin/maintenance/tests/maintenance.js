...
casper.test.begin('Widget - Maintenance', function(test) {
    ...
    var verifyMaintenanceElements = function() {
        ...
        test.assertExists('#maintenance-widget div#maintenance-contentpreviews-container button[title="Reprocess for tenants"]', 'Verify that the content previews tab has a \'Reprocess for tenants\' section');
        ...
        test.assertExists('#maintenance-widget div#maintenance-contentpreviews-container button[title="Reprocess for tenants"] + div form#maintenance-reprocess-tenants-form', 'Verify that the \'Reprocess for tenants\' section has a form');
        test.assertExists('#maintenance-widget div#maintenance-contentpreviews-container button[title="Reprocess for tenants"] + div input[name="' + configUtil().tenantAlias + '"]', 'Verify that the \'Reprocess for tenants\' section has a \'' + configUtil().tenantDisplayname + '\' checkbox');
    ...
    casper.start(configUtil().tenantUI, function() {
        ...
        casper.thenOpen(configUtil().adminUI, function() {
            ...
            casper.thenOpen(configUtil().adminUI + '/maintenance', function() {
                casper.waitForSelector('#maintenance-widget .oae-list-header h2', verifyMaintenanceElements);
            });

            casper.then(function() {
                casper.echo('# Verify maintenance functionality', 'INFO');
                verifyMaintenanceFunctionality(user);
            });
