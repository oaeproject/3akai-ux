...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        var downloadLog = function(callback) {
            ...
            if (lastConnection) {
                oae.api.admin.getLog(numberMonth, tenantAlias, function(err, log) {
                    if(err) {
                        ...
                        saveData(log, 'fileName' + new Date() + '.csv');
                    }
                });
            }
            // If the admin wants the creation date of each user
            if (creationDate) {
                oae.api.admin.getLogCreation(tenantAlias, function(err, log) {
                    if(err) {
                        // Show an error notification
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__GET_FAIL__', 'downloadlog'),
        ...
        var initDownloadLog = function() {
            ...
            $(document).on('oae.trigger.downloadlog', function(ev, data) {
