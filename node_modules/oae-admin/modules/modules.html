                        ...
                        {for option in item.module}
                            {if option && option_index !== "title"}
                                ...
                                {for element in option.elements}
                                    {if element.tenantOverride === false && !context.isGlobalAdminServer && !context.isTenantOnGlobalAdminServer}
                                        {var tenantCanOverride = false}
                                    {else}
                                        {var tenantCanOverride = true}
                                    {/if}
                                        ...
                                        <div class="form-group">
                                            <div class="checkbox">
                                                <label for="${configPath|encodeForHTMLAttribute}">
                                                    <input type="checkbox" id="${configPath|encodeForHTMLAttribute}" name="${configPath|encodeForHTMLAttribute}" {if configValue}checked="checked"{/if} class="modules-config-checkbox" {if !tenantCanOverride} disabled="disabled"{/if}/>${element.description|encodeForHTML} {if !tenantCanOverride}<i class="icon-lock"><span class="sr-only">__MSG__TENANT_OVERRIDE_DISABLED__</span></i>{/if}
                                                </label>
                                            </div>
                                        </div>
                                    {elseif element.type === "text"}
                                        <div class="form-group">
                                            <label for="${configPath|encodeForHTMLAttribute}" class="control-label col-xs-12 col-sm-3 col-md-3">${element.description|encodeForHTML} {if !tenantCanOverride}<i class="icon-lock"><span class="sr-only">__MSG__TENANT_OVERRIDE_DISABLED__</span></i>{/if}</label>
                                    ...
                                    {elseif element.type === "internationalizableText"}
                                        <div class="modules-internationalizable-text-container">
                                            ...
                                            <div data-id="default" class="form-group">
                                                <label for="${configPath|encodeForHTMLAttribute}/default" class="control-label col-xs-12 col-sm-3 col-md-3">default - ${element.description|encodeForHTML} {if !tenantCanOverride}<i class="icon-lock"><span class="sr-only">__MSG__TENANT_OVERRIDE_DISABLED__</span></i>{/if}</label>
                                                <div class="col-xs-12 col-sm-9 col-md-9">
                                            ...
                                            {for language in languages}
                                                ...
                                                <div data-id="${language.value}" class="form-group hide">
                                                    <label for="${configPath|encodeForHTMLAttribute}/${language.value}" class="control-label col-xs-12 col-sm-3 col-md-3">${language.name} - ${element.description|encodeForHTML} {if !tenantCanOverride}<i class="icon-lock"><span class="sr-only">__MSG__TENANT_OVERRIDE_DISABLED__</span></i>{/if}</label>
                                                    <div class="col-xs-12 col-sm-9 col-md-9">
                                            ...
                                            {for rbt in element.group}
                                                <div class="radio">
                                                    <label>
                                                        ...
                                                        ${rbt.name|encodeForHTML} {if !tenantCanOverride}<i class="icon-lock"><span class="sr-only">__MSG__TENANT_OVERRIDE_DISABLED__</span></i>{/if}
                                                    </label>
                                                </div>
                                            {/for}
                                        </div>
