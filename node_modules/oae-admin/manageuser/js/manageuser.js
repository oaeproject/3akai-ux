...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        var editProfile = function() {
            ...
            oae.api.admin.updateUser(widgetData.userProfile.id, params, function (err, updatedUser) {
                ...
                if (!err) {
                    ...
                    $(document).trigger('oae.manageuser.done', updatedUser);
                } else {
        ...
        var deleteUser = function() {
            ...
            $('#manageuser-modal', $rootel).modal('lock');
            $('#manageuser-actions-form .modal-footer .manageuser-actions-deleteuser button').prop('disabled', true);

            oae.api.admin.deleteUser(widgetData.userProfile.id, function(err) {
                // When the delete request finishes re-enable the form
                $('#manageuser-modal', $rootel).modal('unlock');
                $('#manageuser-actions-form .modal-footer .manageuser-actions-deleteuser button').prop('disabled', false);

                if (err) {
                    // Show an error notification
                    oae.api.util.notification(
                        oae.api.i18n.translate('__MSG__USER_NOT_DELETED__', 'manageuser'),
                        oae.api.i18n.translate('__MSG__COULD_NOT_DELETE_USER_NAME__', 'manageuser', {
        ...
        var setUpActionsForm = function() {
            ...
            oae.api.util.template().render($('#manageuser-actions-body-main-template', $rootel), data,
                $('#manageuser-actions-form .modal-body .manageuser-actions-main', $rootel));

            // Render the deleteuser panel body and footer
            oae.api.util.template().render($('#manageuser-actions-body-deleteuser-template', $rootel), data,
                $('#manageuser-actions-form .modal-body .manageuser-actions-deleteuser', $rootel));

            oae.api.util.template().render($('#manageuser-actions-footer-deleteuser-template', $rootel), data,
                $('#manageuser-actions-form .modal-footer .manageuser-actions-deleteuser', $rootel));

            // Switch to deleteuser panel when user selects to delete the user
            $rootel.on('click', '#manageuser-delete-user', function() {
                showActionsPanel('deleteuser');
            });

            // Delete the user when the user accepts the deleteuser confirmation
            $rootel.on('click', '#manageuser-actions-form .manageuser-deleteuser-confirm', deleteUser);

            // Switch back to the main panel when the user clicks the "Cancel" button in the
            // deleteuser confirmation panel
            $rootel.on('click', '#manageuser-actions-form .manageuser-deleteuser-cancel', function() {
                showActionsPanel('main');
            });
        };

        /**
         * Switch between the action panels (main and deleteuser confirmation)
         *
         * @param  {String}     panel   The panel to make active. Valid values are 'main' and 'deleteuser'
         */
        var showActionsPanel = function(panel) {
            var targetClass = 'manageuser-actions-' + panel;

            // Hide all modal body / footer panels
            $('#manageuser-actions-form .modal-body > div').hide();
            $('#manageuser-actions-form .modal-footer > div').hide();

            // Show just the specified modal body and footer
            $('#manageuser-actions-form div.' + targetClass).show();
        };
        ...
        var setUpEmailForm = function() {
            oae.api.util.template().render($('#manageuser-email-template', $rootel), {
                'context': widgetData.context,
                'userProfile': widgetData.userProfile
            }, $('#manageuser-email-form .modal-body', $rootel));
        };
        ...
        var addBinding = function() {
            $rootel.on('click', '#manageuser-become-user', function() {
                oae.api.admin.getSignedBecomeRequestInfo(widgetData.userProfile.id, function(err, data) {
                    if (err) {
                        ...
                        oae.api.util.template().render($('#manageuser-become-user-template', $rootel), data, $('#manageuser-become-user-container', $rootel));
