    ...
    return function(uid) {
        ...
        var renderLogin = function() {
            ...
            oae.api.util.template().render(
                $('#adminlogin-authentication-template', $rootel),
                {
            ...
            var validateOpts = {
                messages: {
                    'adminlogin-local-username': {
                        ...
                        'adminlogin-failed-attempt': oae.api.i18n.translate(
                            '__MSG__INVALID_USERNAME_OR_PASSWORD__',
                        ),
                    },
                    'adminlogin-local-password': oae.api.i18n.translate(
        ...
        var doLogin = function() {
            ...
            var LDAPEnabled = oae.api.config.getValue(
                'oae-authentication',
                'ldap',
                'enabled',
            );
            // Check whether or not the local login strategy is enabled
            var LocalEnabled = oae.api.config.getValue(
                'oae-authentication',
                'local',
                'enabled',
            );
            ...
            var username = $.trim(
                $('#adminlogin-local-username', $rootel).val(),
            );
            var password = $.trim(
            ...
            if (LDAPEnabled && LocalEnabled) {
                oae.api.authentication.LDAPLogin(username, password, function(
                    err,
                ) {
                    if (err) {
        ...
        var finishLogin = function(err) {
            if (err) {
                ...
                $('#adminlogin-local-username', $rootel).addClass(
                    'adminlogin-failed-attempt',
                );
                $('#adminlogin-local', $rootel).valid();
                $('#adminlogin-local-username', $rootel).removeClass(
                    'adminlogin-failed-attempt',
                );
