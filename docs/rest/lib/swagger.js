        ...
        on: {
            error: function(response) {
                if (_this.url.substring(0, 4) !== 'http') {
                    return _this.fail(
                        'Please specify the protocol for ' + _this.url,
                    );
                    ...
                    return _this.fail(
                        "Can't read swagger JSON from " + _this.url,
                    );
                } else {
                    return _this.fail(
                        response.status +
...
var SwaggerResource = function(resourceObj, api) {
    ...
    this.rawModels = {};
    this.useJQuery =
        typeof api.useJQuery !== 'undefined' ? api.useJQuery : null;
        ...
        if (enumValue != null) {
            ...
            if (param.allowableValues != null) {
                ...
                if (param.allowableValues.values) {
                    for (
                        var j = 0;
                        j < param.allowableValues.values.length;
                        j++
                    ) {
...
ShredHttpClient.prototype.execute = function(obj) {
    ...
    res = {
        ...
        request_error: function(err) {
            if (obj) return cb.error(transformError(err));
        },
        redirect: function(response) {
    ...
    if (this.type === 'query') {
        ...
        if (typeof obj.headers[this.name] !== 'undefined') {
            if (typeof this.delimiter !== 'undefined')
                obj.headers[this.name] =
                    obj.headers[this.name] + this.delimiter + this.value;
        } else obj.headers[this.name] = this.value;
        return true;
    }
};
