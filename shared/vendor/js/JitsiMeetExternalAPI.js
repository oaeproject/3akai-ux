...
define(['postis'], function(postisInit) {
    /**
    ...
    function JitsiMeetExternalAPI(
        ...
        this.iframeHolder = this.parentNode.appendChild(
        ...
        if (configOverwrite) {
            for (key in configOverwrite) {
                if (
                    !configOverwrite.hasOwnProperty(key) ||
                    typeof key !== 'string'
                )
                    continue;
        ...
        if (interfaceConfigOverwrite) {
            for (key in interfaceConfigOverwrite) {
                if (
                    !interfaceConfigOverwrite.hasOwnProperty(key) ||
                    typeof key !== 'string'
                )
                    continue;
                this.url +=
     ...
     */
    JitsiMeetExternalAPI.prototype.executeCommand = function(
        name,
        argumentsList,
    ) {
        if (!(name in commands)) {
    ...
    JitsiMeetExternalAPI.prototype.executeCommands = function(object) {
        for (var key in object) this.executeCommand(key, object[key]);
    };
    ...
    JitsiMeetExternalAPI.prototype.addEventListeners = function(object) {
        for (var i in object) this.addEventListener(i, object[i]);
    };
     ...
     */
    JitsiMeetExternalAPI.prototype.addEventListener = function(
        event,
        listener,
    ) {
        if (!(event in events)) {
