            ...
            return this.each(function(x) {
                ...
                opts.start.call(this, {
                    add: function(data) {
                        add_selected_item(
                            data,
                            'u' + $('li', selections_holder).length,
                        ).addClass('blur');
                    },
                    remove: function(value) {
                        values_input.val(
                            values_input.val().replace(',' + value + ',', ','),
                        );
                        selections_holder
                ...
                input
                    .wrap(
                        '<ul class="as-selections" id="as-selections-' +
                            x +
                            '"></ul>',
                    )
                    .wrap(
                        '<li class="as-original" id="as-original-' +
                            x +
                            '"></li>',
                    );
                ...
                var results_holder = $(
                    '<div class="as-results" id="as-results-' +
                        x +
                        '" role="listbox" aria-expanded="false"></div>',
                ).hide();
                var results_ul = $('<ul class="as-list"></ul>');
                var values_input = $(
                    '<input type="hidden" class="as-values" name="as_values_' +
                        x +
                        '" id="as-values-' +
                        x +
                        '" />',
                ...
                if (typeof opts.preFill == 'string') {
                    ...
                    if (prefill_count > 0) {
                        for (var i = 0; i < prefill_count; i++) {
                            var new_v =
                                opts.preFill[i][opts.selectedValuesProp];
                            if (new_v == undefined) {
                                new_v = '';
                            }
                ...
                if (prefill_value != '') {
                    input.val('');
                    var lastChar = prefill_value.substring(
                        prefill_value.length - 1,
                    );
                    if (lastChar != ',') {
                            ...
                            case 8: // delete
                                if (input.val() == '') {
                                    ...
                                    if (org_li.prev().hasClass('selected')) {
                                        ...
                                        opts.selectionRemoved.call(
                                            this,
                                            org_li.prev(),
                                        );
                                    } else {
                                        opts.selectionClick.call(
                                            this,
                                            org_li.prev(),
                                        );
                                ...
                                break;
                            case 9:
                            case 188: // tab or comm
                                var active = $(
                                    'li.active:first:visible',
                                    results_holder,
                                );
                                ...
                                if (active.length === 0) {
                                    if (opts.canGenerateNewSelections) {
                                        ...
                                        if (
                                            ...
                                            var n_data = {};
                                            n_data[
                                                opts.selectedItemProp
                                            ] = i_input;
                                            n_data[
                                                opts.selectedValuesProp
                                            ] = i_input;
                                            var lis = $('li', selections_holder)
                                                .length;
                                            add_selected_item(
                                                n_data,
                                                '00' + (lis + 1),
                    ...
                    for (var i = 0; i < d_count; i++) {
                        ...
                        if (forward) {
                            ...
                            if (opts.resultsHighlight && query.length > 0) {
                                this_data[opts.selectedItemProp] = this_data[
                                    opts.selectedItemProp
                                ].replace(regx, '<em>$1</em>');
                            }
                            if (!opts.formatList) {
                                formatted = formatted.html(
                                    this_data[opts.selectedItemProp],
                                );
                            } else {
                                formatted = opts.formatList.call(
                                    this,
                                    this_data,
                                    formatted,
                                );
                ...
                function add_selected_item(data, num) {
                    ...
                    var item = $(
                        '<li class="as-selection-item" id="as-selection-' +
                            num +
                            '" data-value="' +
                            data[opts.selectedValuesProp] +
                            '"></li>',
                ...
                function moveSelection(direction) {
                    if ($(':visible', results_holder).length > 0) {
                        ...
                        if (
                            opts.scroll &&
                            start.length &&
                            (start.position().top + start.height() >
                                $(results_ul).height() ||
                                start.position().top < 0)
                        ) {
                            $(results_ul).scrollTop(
                                $(results_ul).scrollTop() +
                                    start.position().top,
                            );
                        }
                ...
                function showResults() {
                    ...
                    input.attr(
                        'aria-activedescendant',
                        results_holder.find('.active').attr('id'),
                    );
                }
            });
        }
