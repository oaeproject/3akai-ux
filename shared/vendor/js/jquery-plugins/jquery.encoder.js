    ...
    (function($) {
        ...
        $.encoder = {
            ...
            encodeForCSS: function(propName, input, omitPropertyName) {
                ...
                for (var a = 0; a < unsafeKeys['css_val'].length; a++) {
                    if (
                        input.toLowerCase().indexOf(unsafeKeys['css_val'][a]) >=
                        0
                    ) {
        ...
        var methods = {
            ...
            css: function(opts) {
                ...
                for (var k in work) {
                    if (
                        !(typeof work[k] == 'function') &&
                        work.hasOwnProperty(k)
                    ) {
            ...
            attr: function(opts) {
                ...
                for (var k in work) {
                    if (
                        ...
                        out[k] = $.encoder.encodeForHTMLAttribute(
                            k,
                            work[k],
                            true,
                        );
        ...
        $.fn.encode = function() {
            ...
            return opts.setter.call(
                this,
                methods[opts.context].call(this, opts),
            );
        };
        ...
        var PushbackString = Class.extend({
            ...
            peek: function(c) {
                if (c) {
                    ...
                    return this._hasNext()
                        ? this._input.charAt(this._index) == c
                        : false;
                }
        ...
        var HTMLEntityCodec = Codec.extend({
            ...
            _getNamedEntity: function(input) {
                var possible = '',
                    ...
                    len;
                len = Math.min(
                    input.remainder().length,
                    ENTITY_TO_CHAR_TRIE.getMaxKeyLength(),
                );
                for (var i = 0; i < len; i++) {
        ...
        var PercentCodec = Codec.extend({
            decodeCharacter: function(input) {
                ...
                if (out.length == 2) {
                    ...
                    if (isValidCodePoint(p)) return String.fromCharCode(p);
                }
        ...
        var Trie = Class.extend({
            ...
            getLongestMatch: function(key) {
                return this.root == null && key == null
                    ? null
                    : this.root.getLongestMatch(key, 0);
            },
            ...
            put: function(key, val) {
                var len, old;
                if (this.root == null) this.root = new Trie.Node();
                if ((old = this.root.put(key, 0, val)) != null) return old;
        ...
        Trie.Node = Class.extend({
            ...
            get: function(key, pos) {
                var nextNode;
                if (key.length <= pos) return this._value;
            ...
            getLongestMatch: function(key, pos) {
                ...
                if ((nextNode = this.getNextNode(key.charAt(pos))) == null) {
                    ...
                    return Trie.Entry.newInstanceIfNeeded(
                        key,
                        pos,
                        this._value,
                    );
        ...
        (function() {
            ...
            for (var entity in MAP_ENTITY_TO_CHAR) {
                if (
                    !(typeof MAP_ENTITY_TO_CHAR[entity] == 'function') &&
            ...
            for (var c in MAP_CHAR_TO_ENTITY) {
                if (
                    !(typeof MAP_CHAR_TO_ENTITY[c] == 'function') &&
                    MAP_CHAR_TO_ENTITY.hasOwnProperty(c)
                ) {
