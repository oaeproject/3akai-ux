...
casper.test.begin('Macro - List Metadata', function(test) {
    ...
    var verifyListItem = function(entityData) {
        casper.waitForSelector(
            'ul.oae-list li[data-id="' + entityData.id + '"]',
            function() {
                test.assertExists(
                    '.oae-list li[data-id="' +
                        entityData.id +
                        '"] .oae-tile-metadata',
                    'Verify the metadata container is present in the list item',
                );
                ...
                if (entityData.resourceType === 'user') {
                    ...
                    test.assertDoesntExist(
                        '.oae-list li[data-id="' +
                            entityData.id +
                            '"] .oae-tile-metadata .fa-oae-' +
                            entityData.visibility,
                        'Verify the visibility icon is not present in the list item metadata',
                    );
                } else {
                    test.assertExists(
                        '.oae-list li[data-id="' +
                            ...
                            '"] .oae-tile-metadata .fa-oae-' +
                            entityData.visibility,
                        'Verify the visibility icon is present in the list item metadata',
                    );
                }
                ...
                test.assertExists(
                    '.oae-list li[data-id="' +
                        entityData.id +
                        '"] .oae-tile-metadata h3',
                    'Verify the title is present in the list item metadata',
                );
                test.assertSelectorHasText(
                    ...
                    'Verify the correct title is shown in the list item metadata',
                );
                // Verify the subtext
                test.assertExists(
                    '.oae-list li[data-id="' +
                        entityData.id +
                        '"] .oae-tile-metadata small',
                    'Verify the small description is present in the list item metadata',
                );
                if (entityData.resourceType === 'user') {
                    test.assertSelectorHasText(
                        '.oae-list li[data-id="' +
                            entityData.id +
                            '"] .oae-tile-metadata small',
                        entityData.tenant.displayName,
                        "Verify the small description in the list item metadata reads '" +
                            entityData.tenant.displayName +
                            "'",
                    );
                    ...
                    test.assertSelectorHasText(
                        '.oae-list li[data-id="' +
                            ...
                            '"] .oae-tile-metadata small',
                        'Group',
                        "Verify the small description in the list item metadata reads 'Group'",
                    );
                } else if (entityData.resourceType === 'content') {
                    var description = casper.evaluate(function(entityData) {
                        return require('oae.api.content').getMimeTypeDescription(
                            entityData,
                        );
                    }, entityData);
                    ...
                    test.assertSelectorHasText(
                        '.oae-list li[data-id="' +
                            ...
                            '"] .oae-tile-metadata small',
                        'Discussion',
                        "Verify the small description in the list item metadata reads 'Discussion'",
                    );
                }
            },
        );
        ...
        userUtil.createUsers(2, function(user1, user2) {
            ...
            followUtil.follow(user2.id, function() {
                ...
                contentUtil.createFile(
                    ...
                    null,
                    null,
                    function(err, contentProfile1) {
                        // Create a video
                        contentUtil.createFile(
                            null,
                            null,
                            null,
                            'tests/casperjs/data/sample-video.mp4',
                            null,
                            null,
                            function(err, contentProfile2) {
                                // Create an archive
                                contentUtil.createFile(
                                    null,
                                    null,
                                    null,
                                    'tests/casperjs/data/apereo.zip',
                                    null,
                                    null,
                                    function(err, contentProfile3) {
                                        // Create a link
                                                                ...
                                                                groupUtil.createGroup(
                                                                    ...
                                                                    function(
                                                                        ...
                                                                        casper.then(
                                                                            function() {
                                                                                casper.echo(
                                                                                    'Verify discussion list item metadata',
                                                                                    'INFO',
                                                                                );
                                                                                verifyListItem(
                                                                                    discussionProfile,
                                                                        ...
                                                                        casper.then(
                                                                            function() {
                                                                                casper.echo(
                                                                                    'Verify group list item metadata',
                                                                                    'INFO',
                                                                                );
                                                                                verifyListItem(
                                                                                    groupProfile,
                                                                        ...
                                                                        casper.then(
                                                                            function() {
                                                                                casper.echo(
                                                                                    'Verify user list item metadata',
                                                                                    'INFO',
                                                                                );
                                                                                verifyListItem(
                                                                                    user2,
