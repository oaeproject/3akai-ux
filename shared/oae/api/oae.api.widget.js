...
define(
    ...
    function(exports, $, _, configAPI, i18nAPI, utilAPI) {
         ...
         */
        var getWidgetManifest = (exports.getWidgetManifest = function(
            widgetName,
        ) {
         ...
         */
        var getWidgetManifests = (exports.getWidgetManifests = function(
            properties,
        ) {
            if (!properties) {
                return manifests;
            ...
            $.each(getWidgetManifests(), function(widgetName, widget) {
                ...
                if (widget.trigger && !widget.trigger.onLoad) {
                    ...
                    var lazyLoadWidget = function(callback) {
                        ...
                        $.each(widget.trigger.selectors, function(
                            index,
                            selector,
                        ) {
         ...
         */
        var loadWidgets = (exports.loadWidgets = function(
            $container,
            showSettings,
            widgetData,
            callback,
        ...
        var locateWidgets = function(
            ...
            $('[data-widget]', $container).each(function(index, element) {
                ...
                widgetsToLoad[widgetName].prefixPath =
                    '/node_modules/' + widget.path;
                // Set the link to the HTML page
                widgetsToLoad[widgetName].html = convertRelativeToAbsolutePath(
                    widget.src,
                    widgetsToLoad[widgetName].prefixPath,
                );
                ...
                if (widget.i18n) {
                    widgetsToLoad[widgetName].bundles['default'] = widget.i18n[
                        'default'
                    ]
                        ...
                        : null;
                    widgetsToLoad[widgetName].bundles[locale] = widget.i18n[
                        locale
                    ]
                        ? convertRelativeToAbsolutePath(
                              widget.i18n[locale],
                ...
                widgetsToLoad[widgetName].instances =
                    widgetsToLoad[widgetName].instances || [];
         ...
         * @api private
         */
        var processWidgetFiles = function(
            widgetFiles,
            widgetsToLoad,
            $container,
            showSettings,
            ...
            $.each(widgetsToLoad, function(widgetName, loadData) {
                ...
                if (loadData.bundles['default']) {
                    i18nAPI.parseWidgetBundles(
                        widgetName,
                        widgetFiles[loadData.bundles['default']],
                        loadData.bundles[locale]
                            ? widgetFiles[loadData.bundles[locale]]
                            : null,
                    );
                }
                ...
                widgetHTML = widgetHTML.trim().replace(/<img/gi, '<imgtmp');
                ...
                widgetHTML = $('<div>')
                    .html($widgetEl.filter(':not(link):not(script)'))
                    .html();
                // Change the images back to `img` tags
                widgetHTML = widgetHTML.replace(/imgtmp/gi, 'img');
                ...
                require(jsFiles, function(widgetFunction) {
                    ...
                    for (var i = 0; i < loadData.instances.length; i++) {
                        ...
                        renderWidget(
                            widgetName,
                            loadData.instances[i],
                            showSettings,
                            widgetData[loadData.instances[i]],
                        );
                    }
        ...
        var renderWidget = function(
            ...
            if (loadedWidgets[widgetName].widgetFunction) {
                loadedWidgets[widgetName].widgetFunction(
                    widgetId,
                    showSettings,
                    widgetData,
                );
         ...
         * @throws {Error}                              Error thrown when no or an invalid widget name is provided
         */
        var insertWidget = (exports.insertWidget = function(
            widgetName,
            widgetId,
            $container,
            showSettings,
