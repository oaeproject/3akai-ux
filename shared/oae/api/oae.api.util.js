...
define(
    ...
    function(exports, require, $, _, configAPI, markdown) {
        ...
        var staticBatch = (exports.staticBatch = function(paths, callback) {
            if (!paths || paths.length === 0) {
                throw new Error(
                    'At least one path should be provided to the static batch',
                );
            }
        ...
        var profilePath = (exports.profilePath = function(profilePath) {
            return profilePath === require('oae.core').data.me.profilePath
                ? '/'
                : profilePath;
        });
        ...
        var setBrowserTitle = (exports.setBrowserTitle = function(title) {
            ...
            document.title = require('oae.api.i18n').translate(
                title.join(' - '),
            );
     ...
     */
        var template = (exports.template = function() {
            ...
            var render = function($template, data, $output) {
                ...
                if (!templateCache[templateId]) {
                    ...
                    try {
                        templateCache[templateId] = TrimPath.parseTemplate(
                            templateContent,
                            templateId,
                        );
                    } catch (parseErr) {
                ...
                try {
                    // Render the template
                    renderedHTML = templateCache[templateId].process(data, {
        ...
        var notification = (exports.notification = function(
            ...
            if (!message) {
                throw new Error(
                    'A valid notification message should be provided',
                );
            }
            ...
            if ($notificationContainer.length === 0) {
                $notificationContainer = $('<div>')
                    .attr('id', 'oae-notification-container')
                    .addClass('notifications top-center');
     ...
     */
        ...
        var googleAnalytics = function() {
            ...
            var globalEnabled = configAPI.getValue(
            ...
            var tenantEnabled = configAPI.getValue(
                'oae-google-analytics',
                'google-analytics',
                'tenantEnabled',
            );
     ...
     */
        var validation = (exports.validation = function() {
            ...
            var init = function() {
                ...
                $.validator.addMethod('maxlength-short', function(
                    value,
                    element,
                ) {
                ...
                $.validator.addMethod('maxlength-medium', function(
                    value,
                    element,
                ) {
                ...
                $.validator.addMethod('maxlength-long', function(
                    value,
                    element,
                ) {
            ...
            var validate = function($form, options) {
                ...
                if (
                    options.invalidHandler &&
                    $.isFunction(options.invalidHandler)
                ) {
            ...
            var isValidEmailDomainForTenant = function(email) {
                ...
                var configuredEmailDomains = require('oae.core').data.me.tenant
                    .emailDomains;
                if (_.isEmpty(configuredEmailDomains)) {
                    return true;
                }

                configuredEmailDomains = _.map(configuredEmailDomains, function(
                    emailDomain,
                ) {
                    return emailDomain.toLowerCase();
                });

                var givenEmailDomain = email
                    .split('@')
                    .pop()
                    .toLowerCase();
                var matchingEmailDomain = _.find(
                    configuredEmailDomains,
                    function(configuredEmailDomain) {
                        ...
                        var suffixPosition = givenEmailDomain.indexOf(
                            emailDomainSuffix,
                        );
                        return (
                            suffixPosition > 0 &&
                            suffixPosition ===
                                givenEmailDomain.length -
                                    emailDomainSuffix.length
                        );
                    },
                );
         ...
         */
        var clickover = (exports.clickover = function(
            $trigger,
            $content,
            options,
        ) {
         ...
         */
            ...
            var init = function() {
                $(document).on('focus', 'ul.as-selections input', function() {
                    $(this)
                        .parents('ul.as-selections')
                        .addClass('as-selections-focus');
                });
                $(document).on(
                    'focusout',
                    ...
                    function() {
                        $(this)
                            .parents('ul.as-selections')
                            .removeClass('as-selections-focus');
                    },
                );
            };
            ...
            var getAutosuggestTemplates = function(callback) {
                if (!$autosuggestTemplates) {
                    ...
                    require([
                        'text!/shared/oae/macros/autosuggest.html',
                    ], function(autosuggestTemplates) {
                        // Translate the template. We require the i18n API here to avoid creating a cyclic dependency
                        autosuggestTemplates = require('oae.api.i18n').translate(
                            autosuggestTemplates,
                        );
                        $autosuggestTemplates = $('<div>').append(
                            autosuggestTemplates,
                        );
                        callback();
                    });
            ...
            var setup = function($element, options, resourceTypes, callback) {
                if (!$element) {
                    throw new Error(
                        'A valid input element should be provided.',
                    );
                }

                // Whether guests can be invited to this tenant through an email address
                var allowInvitingGuests = configAPI.getValue(
                    'oae-tenants',
                    'guests',
                    'allow',
                );
                ...
                getAutosuggestTemplates(function() {
                    ...
                    if (!options.startText) {
                        ...
                        options.startText = placeholder
                            ? placeholder
                            : i18nAPI.translate('__MSG__ENTER_NAME_HERE__');
                    }
                    ...
                    if (
                        !options.emptyText &&
                        options.showResultListWhenNoMatch
                    ) {
                        options.emptyText = i18nAPI.translate(
                            '__MSG__NO_RESULTS_FOUND__',
                        );
                    }
                    ...
                    if (options.preFill) {
                        $.each(options.preFill, function(index, preFillItem) {
                            preFillItem[
                                options.selectedItemProp
                            ] = security().encodeForHTML(
                                preFillItem[options.selectedItemProp],
                            );
                        });
                    }
                    ...
                    options.retrieveComplete = function(data) {
                        ...
                        data.results = _.chain(data.results)
                            ...
                            .each(function(result) {
                                result.displayName = security().encodeForHTML(
                                    result.displayName,
                                );
                                result.query = query;
                                return result;
                        ...
                        var allowEmailAddress =
                            allowInvitingGuests ||
                            (data.tenant && !data.tenant.isGuestTenant);
                        if (
                            options.allowEmail &&
                            isValidEmail &&
                    ...
                    if (!options.formatList) {
                        options.formatList = function(data, elem) {
                            return elem.html(
                                template().render(
                                    $(
                                        '#autosuggest-suggested-template',
                                        $autosuggestTemplates,
                                    ),
                                    {
                                        data: data,
                    ...
                    options.selectionRemoved = function(elem) {
                        ...
                        if (options.preFill) {
                            isFixed = _.some(options.preFill, function(
                                preFilledItem,
                            ) {
                                return (
                                    preFilledItem.id === originalData.id &&
                                    preFilledItem.fixed === true
                                );
                            });
                        }
                        // Check if the removed element was one of the ghost elements in the ghosts objects
                        if (!isFixed && options.ghosts) {
                            isFixed = _.some(options.ghosts, function(
                                ghostItem,
                            ) {
                    ...
                    options.selectionAdded = function(elem) {
                        ...
                        $elem
                            ...
                            .wrapAll(
                                '<span class="pull-left oae-threedots" />',
                            );
                        ...
                        if (originalData.resourceType) {
                            ...
                            var $thumbnail = $('<div>').addClass(
                                'oae-thumbnail fa fa-oae-' +
                                    originalData.resourceType,
                            );
                            if (originalData.thumbnailUrl) {
                                $thumbnail.append(
                                    $('<div>')
                                        ...
                                        .attr(
                                            'aria-label',
                                            security().encodeForHTMLAttribute(
                    ...
                    if (options.allowEmail) {
                        optionsUrl = function(q) {
                            return validation().isValidEmail(q)
                                ? '/api/search/email'
                                : options.url;
                        };
                    }
                    ...
                    var $autoSuggest = $element.autoSuggest(
                        optionsUrl,
                        options,
                    );
                                    ...
                                    success: function(tenants) {
                                        ...
                                        if (!_.isEmpty(remainingEmails)) {
                                            ...
                                            if (remainingEmails.length === 1) {
                                                msg = i18nAPI.translate(
                                                    '__MSG__UNABLE_TO_ADD_EMAIL_ADDRESSES_1__',
                                                    null,
                                                    {
                                                        emailAddress1:
                                                            remainingEmails[0],
                                                ...
                                                msg = i18nAPI.translate(
                                                    '__MSG__UNABLE_TO_ADD_EMAIL_ADDRESSES_2__',
                                                    null,
                                                    {
                                                        emailAddress1:
                                                            remainingEmails[0],
                                                ...
                                                msg = i18nAPI.translate(
                                                    '__MSG__UNABLE_TO_ADD_EMAIL_ADDRESSES_2+__',
                                                    null,
                                                    {
                    ...
                    var handleTerminationKey = function(tokens, keyCode) {
                        ...
                        var firstResultItem = $suggestions
                            .find('li.as-result-item:first')
                            .get(0);
                        var activeResultItem = $suggestions
                            .find('li.as-result-item.active:first')
                            .get(0);
                        ...
                        if (isEnterKey || isCommaKey) {
                            ...
                            // next case
                            var selectionResultItem =
                                activeResultItem || firstResultItem;
                            if (selectionResultItem) {
                                var selectionResultItemId = $(
                                    selectionResultItem,
                        ...
                        if (
                            options.allowEmail &&
                            allowInvitingGuests &&
                            _.every(tokens, validation().isValidEmail)
                        ) {
                            insertEmails(tokens);
                            $element.val('');
                            return false;
                    ...
                    $element.on('keydown', function(evt) {
                        switch (evt.keyCode) {
                            ...
                            case 32: // Space key
                            case 188: // Comma key
                                var result = handleTerminationKey(
                                    getTokens(),
                                    evt.keyCode,
                                );
                                if (!result) {
            ...
            var getSelection = function($element) {
                if (!$element) {
                    throw new Error(
                        'An valid input element should be provided.',
                    );
                }
                ...
                $.each($element.find('.as-selections > li'), function(
                    index,
                    selection,
                ) {
                    ...
                    if (id && selectionData) {
                        ...
                        if (
                            !isGhostItem ||
                            (isGhostItem &&
                                $selection.hasClass('as-ghost-selected'))
                        ) {
                            var selectedItem = {
                                id: id,
                            ...
                            if (
                                selectionData.resourceType === 'user' &&
                                validation().isValidEmail(selectionData.query)
                            ) {
        ...
        var security = (exports.security = function() {
            ...
            var encodeForHTMLAttribute = function(input, attribute) {
                if (!input) {
                    ...
                    return $.encoder.encodeForHTMLAttribute(
                        attribute,
                        input.toString(),
                        true,
                    );
        ...
        var redirect = (exports.redirect = function() {
            ...
            var accessdenied = function() {
                window.location.replace(
                    '/accessdenied?url=' + url().attr('path'),
                );
            };
