...
define(['exports', 'jquery', 'underscore', 'oae.api.util', 'sockjs'], function(
    exports,
    $,
    _,
    utilAPI,
    ...
    var subscribe = (exports.subscribe = function(
        ...
        token,
        transformer,
        performInlineAggregation,
        performFullAggregation,
        messageCallback,
        callback,
    ) {
     ...
     */
    var resetAggregation = (exports.resetAggregation = function(
        resourceId,
        streamType,
    ) {
    ...
    var notifySubscribers = function(message) {
        ...
        if (
            subscriptions[message.resourceId] &&
            subscriptions[message.resourceId][message.streamType] &&
            subscriptions[message.resourceId][message.streamType][
                message.format
            ]
        ) {
                ...
                function(listener) {
                    ...
                    if (listener.performFullAggregation) {
                        ...
                        if (listener.performInlineAggregation) {
                            copiedActivities = aggregateActivities(
                                copiedActivities,
                            );
                        }
    ...
    var aggregateMessages = function(message, callback) {
        ...
        activities[resourceId][streamType] =
            activities[resourceId][streamType] || [];
        ...
        var inlineAggregatedActivities = aggregateActivities(
            message.activities,
        );
        ...
        if (!_.isEmpty(activities[resourceId][streamType])) {
            ...
            // from our message.
            var oldActivitiesOfSameType = _.filter(
                activities[resourceId][streamType],
                function(oldActivity) {
                    var messageContainsActivityOfSameType = _.find(
                        inlineAggregatedActivities,
            ...
            aggregatedActivities = aggregateActivities(
                oldActivitiesOfSameType.concat(inlineAggregatedActivities),
            );
            ...
            var allActivities = activities[resourceId][streamType].concat(
                aggregatedActivities.slice(),
            );
            activities[resourceId][streamType] = aggregateActivities(
                allActivities,
            );
        } else {
     ...
     * @api private
     */
    var aggregateActivities = (exports.aggregateActivities = function(
        activities,
    ) {
    ...
    var getAggregateKey = function(activity) {
        var aggregateKey = [];
        _.each(AGGREGATION_RULES[activity['oae:activityType']], function(
            aggregationSpec,
            activityField,
        ) {
            ...
            if (
                ...
                activity[activityField][aggregationSpec]
            ) {
    ...
    var addEntities = function(to, from, entity) {
        ...
        var fromCollection = from[entity]['oae:collection']
            ? from[entity]['oae:collection']
            : [from[entity]];
        var toCollection = to[entity]['oae:collection']
            ? to[entity]['oae:collection']
            : [to[entity]];
