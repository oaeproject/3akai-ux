...
define(
    ...
    function(exports, $, configAPI, utilAPI, l10nAPI) {
        ...
        var init = (exports.init = function(currentLocale, callback) {
            ...
            locale =
                currentLocale ||
                configAPI.getValue('oae-principals', 'user', 'defaultLanguage');
        ...
        var loadCoreBundles = function(callback) {
            var bundlesToLoad = [
                '/shared/oae/bundles/ui/default.properties',
                '/shared/oae/bundles/ui/' + locale + '.properties',
            ];
            utilAPI.staticBatch(bundlesToLoad, function(err, data) {
                ...
                bundles.core['default'] = $.parseProperties(
                    data[bundlesToLoad[0]],
                );
                // Only parse the core bundle for the current user's language if it exists
                if (data[bundlesToLoad[1]]) {
                    bundles.core[locale] = $.parseProperties(
                        data[bundlesToLoad[1]],
                    );
                }
                callback(null);
            });
         ...
         * @api private
         */
        var parseWidgetBundles = (exports.parseWidgetBundles = function(
            widgetName,
            defaultBundle,
            localeBundle,
        ) {
            bundles.widgets[widgetName] = {};
            bundles.widgets[widgetName]['default'] = $.parseProperties(
                defaultBundle,
            );
            if (localeBundle) {
                bundles.widgets[widgetName][locale] = $.parseProperties(
                    localeBundle,
                );
            }
        });
        ...
        var translateJqueryPlugins = function() {
            ...
            $.timeago.settings.strings = {
                ...
                prefixFromNow: translate(
                    '__MSG__JQUERY_TIMEAGO_PREFIXFROMNOW__',
                ),
                suffixAgo: translate('__MSG__JQUERY_TIMEAGO_SUFFIXAGO__'),
                suffixFromNow: translate(
                    '__MSG__JQUERY_TIMEAGO_SUFFIXFROMNOW__',
                ),
            ...
            $.validator.messages = {
                creditcard: translate(
                    '__MSG__PLEASE_ENTER_A_VALID_CREDIT_CARD_NUMBER__',
                ),
                ...
                equalTo: translate(
                    '__MSG__PLEASE_ENTER_THE_SAME_VALUE_AGAIN__',
                ),
                max: translate(
                    '__MSG__PLEASE_ENTER_A_VALUE_LESS_THAN_OR_EQUAL_TO__',
                ),
                maxlength: translate(
                    '__MSG__PLEASE_ENTER_MAXIMUM_X_CHARACTERS__',
                ),
                'maxlength-long': translate(
                    '__MSG__PLEASE_ENTER_NO_MORE_THAN_X_CHARACTERS__',
                    null,
                    { x: l10nAPI.transformNumber(100000) },
                ),
                'maxlength-medium': translate(
                    '__MSG__PLEASE_ENTER_NO_MORE_THAN_X_CHARACTERS__',
                    null,
                    { x: l10nAPI.transformNumber(10000) },
                ),
                'maxlength-short': translate(
                    '__MSG__PLEASE_ENTER_NO_MORE_THAN_X_CHARACTERS__',
                    null,
                    { x: l10nAPI.transformNumber(1000) },
                ),
                min: translate(
                    '__MSG__PLEASE_ENTER_A_VALUE_GREATER_THAN_OR_EQUAL_TO__',
                ),
                minlength: translate(
                    '__MSG__PLEASE_ENTER_AT_LEAST_X_CHARACTERS__',
                ),
                nospaces: translate('__MSG__NO_SPACES_ARE_ALLOWED__'),
                number: translate('__MSG__PLEASE_ENTER_A_VALID_NUMBER__'),
                range: translate(
                    '__MSG__PLEASE_ENTER_A_VALUE_BETWEEN_X_AND_Y__',
                ),
                rangelength: translate(
                    '__MSG__PLEASE_ENTER_A_VALUE_BETWEEN_X_AND_Y_CHARACTERS_LONG__',
                ),
         ...
         */
        var translate = (exports.translate = function(
            input,
            widgetName,
            variables,
        ) {
            ...
            input = input.replace(/__MSG__(.*?)__/gm, function(match, i18nkey) {
                ...
                if (widgetName && bundles.widgets[widgetName]) {
                    ...
                    if (
                        bundles.widgets[widgetName][locale] &&
                        bundles.widgets[widgetName][locale][i18nkey] !==
                            undefined
                    ) {
                        translation =
                            bundles.widgets[widgetName][locale][i18nkey];
                        ...
                        bundles.widgets[widgetName]['default'] &&
                        bundles.widgets[widgetName]['default'][i18nkey] !==
                            undefined
                    ) {
                        translation =
                            bundles.widgets[widgetName]['default'][i18nkey];
                ...
                if (!translation) {
                    ...
                    if (
                        bundles.core[locale] &&
                        bundles.core[locale][i18nkey] !== undefined
                    ) {
                        translation = bundles.core[locale][i18nkey];
                        // Check the widget's default bundle
                    } else if (
                        bundles.core['default'] &&
                        bundles.core['default'][i18nkey] !== undefined
                    ) {
