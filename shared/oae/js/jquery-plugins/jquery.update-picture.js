...
define(['jquery', 'oae.api.util'], function(jQuery, oaeUtil) {
    (function($) {
        ...
        $(document).on('oae.changepic.update', function(ev, data) {
            ...
            var $template = $(
                '<div id="thumbnail-template"><!--${renderThumbnail(entityData)}--></div>',
            );
            ...
            var newThumbnail = $.trim(
                oaeUtil.template().render($template, {
                    entityData: data,
                }),
            );
            var newThumbnailImage = $(newThumbnail).find('[role="img"]')[0]
                .outerHTML;
