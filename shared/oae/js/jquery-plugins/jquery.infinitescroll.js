...
define(
    ['jquery', 'underscore', 'oae.api.util', 'oae.api.i18n', 'oae.api.l10n'],
    function(jQuery, _, oaeUtil, oaeI18n, oaeL10n) {
        (function($) {
            /**
            ...
            $.fn.infiniteScroll = function(
                ...
                if (!source) {
                    ...
                    throw new Error(
                        'A valid template name or render function should be provided',
                    );
                }
                ...
                var checkLoadNext = function() {
                    ...
                    if (
                        canRequestMoreData &&
                        ($listContainer && $listContainer.is(':visible'))
                    ) {
                        // In case we use the body
                        var threshold = 500;
                        var pixelsRemainingUntilBottom =
                            $(document).height() -
                            $(window).height() -
                            $(window).scrollTop();
                        ...
                        if (options.scrollContainer) {
                            threshold = 200;
                            pixelsRemainingUntilBottom =
                                options.scrollContainer.prop('scrollHeight') -
                                options.scrollContainer.height() -
                ...
                var loadResultList = function() {
                    ...
                    if (initialSearchDone === true) {
                        ...
                        var listItems = $listContainer
                            .children('li:not(.oae-list-actions)')
                            .filter(':visible');
                ...
                var renderList = function(data, opts) {
                    ...
                    if ($listContainer) {
                        ...
                        if (opts.prepend) {
                            ...
                            var $listActions = $listContainer.find(
                                '.oae-list-actions',
                            );
                            if ($listActions.length) {
                                $templateOutput
                                    .hide()
                                    .insertAfter($listActions)
                                    .fadeIn('slow');
                            } else {
                                $templateOutput
                                    .hide()
                        ...
                        if (!opts.prepend) {
                            ...
                            if (
                                nextToken ||
                                data.results.length === parameters.limit
                            ) {
                        ...
                        $('.oae-tile .oae-tile-title').dotdotdot({
                            watch: 'window',
                        });
                    }
                };
                ...
                var prependItems = function(items, update) {
                    ...
                    if (
                        $('li:not(.oae-list-initial)', $listContainer)
                            .length === 0
                    ) {
                        $listContainer.empty();
                    }
                ...
                var setUpLoadingImage = function() {
                    ...
                    var $a11yHelper = $('<span />')
                        .text(oaeI18n.translate('__MSG__LOADING__'))
                        .addClass('sr-only');
