...
const _expose = function(exports) {
  ...
  const _adaptActivity = function(context, me, activity, sanitization, opts) {
    ...
    const summary = _generateSummary(me, activity, sanitization, opts);

    // Generate the primary actor view
    const primaryActor = _generatePrimaryActor(me, activity);

    // Generate the activity preview items
    const activityItems = _generateActivityPreviewItems(context, activity);
  ...
  const _prepareActivity = function(me, activity) {
    ...
    if (COMMENT_ACTIVITY_TYPES.indexOf(activity['oae:activityType']) !== -1) {
      ...
      const latestComments = _constructLatestCommentTree(comments);

      // Convert these comments into an ordered tree that also includes the comments they were
      // replies to, if any
      const allComments = _constructCommentTree(comments);
  ...
  const _constructCommentTree = function(comments) {
    ...
    comments.forEach(comment => {
      ...
      if (comment.inReplyTo) {
        const parent = _find(comments, comment.inReplyTo['oae:id']);
        if (parent) {
  ...
  const _generateActivityPreviewItems = function(context, activity) {
    ...
    let previewItems = previewObj['oae:collection'] || [previewObj];
    previewItems = previewItems.map(previewItem => {
  ...
  const _setSummaryPropertiesForEntity = function(
    ...
    if (properties[profilePathKey]) {
      properties[displayLinkKey] =
        '<a href="' + properties[profilePathKey] + '">' + properties[displayNameKey] + '</a>';
    } else {
