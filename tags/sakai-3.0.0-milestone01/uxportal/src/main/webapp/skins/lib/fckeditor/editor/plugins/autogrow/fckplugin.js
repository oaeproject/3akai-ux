...
function FCKAutoGrow_Check()
{
	...
	if ( iDiff != 0 )
	{
		...
		window.frameElement.height = iMainFrameSize + 60;
	}
}


function FCKFitContent() {
	this.Name = 'FitContent';

}

FCKFitContent.prototype.Execute = function() {
	FCKAutoGrow_Check();
}

FCKFitContent.prototype.GetState = function() {
	return FCK_TRISTATE_OFF;
}


function FCKOriginalSize() {
	this.Name ='OriginalSize';
}

FCKOriginalSize.prototype.Execute = function() {
	window.frameElement.height = FCKAutoGrow_Min ;
}

FCKOriginalSize.prototype.GetState = function() {
	return FCK_TRISTATE_OFF;
}

FCKCommands.RegisterCommand('FitContent', new FCKFitContent());
FCKCommands.RegisterCommand('OriginalSize', new FCKOriginalSize());
