...
this.p.gridid=gridid.indexOf("#")!=-1?gridid:"#"+gridid;var gcolMod=$(this.p.gridid).getGridParam('colModel');if(gcolMod){if(this.p.gridModel===true){var thegrid=$(this.p.gridid)[0];$.each(gcolMod,function(i,n){var tmpFil=[];if(!this.search){this.search=true;}
if(this.search===true&&!this.hidden){if(self.p.gridNames===true){tmpFil.label=thegrid.p.colNames[i];}else{tmpFil.label='';}
...
if(self.p.autosearch===true){$("select",this).change(function(e){triggerSearch();return false;});}});}else{if($t.sopt.value){var so=$t.sopt.value.split(";"),sv,ov;try{delete $t.sopt['value'];}catch(e){}
var elem=document.createElement("select");$(elem).attr({name:$t.name,id:"sg_"+$t.name}).attr($t.sopt);for(var k=0;k<so.length;k++){sv=so[k].split(":");ov=document.createElement("option");ov.value=sv[0];ov.innerHTML=sv[1];if(sv[1]==$t.defval)ov.selected="selected";elem.appendChild(ov);}
