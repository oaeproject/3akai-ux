...
$.fn.jqGrid = function( p ) {
	...
	$.fn.setGridWidth = function(nwidth, shrink) {
		return this.each(function(){
			...
			if(shrink !== true) {testdata[0] = Math.min($t.p.width,$t.grid.width); testdata[2]=0;}
			...
			$t.p.width = nwidth; $t.grid.width = tw;
			if($.browser.safari || $.browser.opera ) {
	...
	return this.each( function() {
		...
		var sortData = function (index, idxcol,reload){
			...
			ts.p.savedRow =[];
			populate();
			if(ts.p.sortname != index) {ts.p.sortname = index; ts.p.lastsort = idxcol;}
		};
