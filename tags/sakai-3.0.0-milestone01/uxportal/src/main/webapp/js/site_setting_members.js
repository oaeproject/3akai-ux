...
sakai.siteSettingsMembers = function(){
	...
	function refreshSiteJson(isInit) {
	...
	function refreshSiteScreen() {
		...
		$("#back_site").attr("href", $("#back_site").attr("href") + siteJson.id);
	...
	function buildYuiDataTable() {
		...
		var memberLinkFormatter = function(elCell, oRecord, oColumn, oData) {
			...
			elCell.innerHTML = "<a href=\"/dev/sn/profile.html?user=" + oRecord._oData.userId + "\">" + memberName + "</a>";
		};
		var myColumnDefs = [
		...
		myDataSource.responseSchema = {
			...
			fields: ["id","userDisplayName","userEmail","memberRole","userId",
	...
	function sendInvitation(){
		...
		var notificationMessage = $("#message").val();
		var users = invites.split("\n");
		if (isValid()) {
			addUsers(users, defaultRole, notificationMessage);
			hideAddMembersWidget();
		}
	}

	function addUsers(userIds, memberRole, notificationMessage){
		removeErrorMessages();
		removeSuccessMessages();
	...
	$("#autocompletefield").autocomplete("/sdata/profile");
