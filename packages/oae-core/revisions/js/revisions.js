...
define(['jquery', 'oae.core'], function($, oae) {
  return function(uid, showSettings) {
    ...
    var getClickedRevision = function(ev, callback) {
      ...
      if (contentProfile.resourceSubType === 'collabsheet') {
        // If the ethercalc HTML has already been retrieved return the result in the callback,
        // otherwise fetch the revision and return the result in the callback.
        if (ethercalcHtml[revisionId]) {
          result.ethercalcHtml = ethercalcHtml[revisionId];
          callback(result);
        } else {
          oae.api.content.getRevision(contentProfile.id, revisionId, function(err, revision) {
            ethercalcHtml[revisionId] = revision.ethercalcHtml;
            result.ethercalcHtml = revision.ethercalcHtml;
            callback(result);
          });
        }
