...
define(['jquery', 'oae.core', 'bootstrap.datepicker'], function($, oae) {
  ...
  const mimeTypeConstants = {
    TYPES: {
      __MSG__OTHER_MIMETYPES__: ['application/octet-stream'],
      __MSG__IMAGE_MIMETYPES__: [
        'application/dicom',
        'application/tga',
      ...
      __MSG__PDF_MIMETYPES__: ['application/pdf'],
  ...
  return function(uid) {
     ...
     */
    const reprocessHandler = function(err) {
      if (err) {
     ...
     */
    const reprocessMimeTypes = function() {
      // Get all the selected mimetypes and construct the data object to send to the server
      const data = { revision_mime: [] };
      $(this)
        .find('input[type="checkbox"][name]:checked')
        .each(function(index, checkedMimeType) {
     ...
     */
    const reprocessContentTypes = function() {
      // Get all the selected content types and construct the data object to send to the server
      const data = { content_resourceSubType: [] };
      $(this)
        .find('input[type="checkbox"][name]:checked')
        .each(function(index, checkedContentType) {
    ...
    /**
     * Reprocess a specific content (a file or a collabdoc for instance)
     */
    const reprocessSpecificContent = () => {
      const contentTextfield = $('#maintenance-reprocess-specific-content', $rootel);
      // debug
      console.log(contentTextfield[0].value);
      const data = { content_contentId: contentTextfield[0].value };
     ...
     */
    const reprocessFailed = function() {
      const data = { revision_previewsStatus: ['ignored', 'error'] };
     ...
     */
    const reprocessDateRange = function() {
      const data = {
        revision_createdAfter: $('#maintenance-reprocess-daterange-from', $rootel)
          .datepicker('getDate')
          .getTime(),
        revision_createdBefore: $('#maintenance-reprocess-daterange-to', $rootel)
          .datepicker('getDate')
          .getTime()
      };
     ...
     */
    const reprocessUser = function() {
      const data = { revision_createdBy: [] };
     ...
     */
    const selectAllMimetypes = function() {
      const selectAll = $(this).is(':checked');
      const chkContainer = $(this)
        .parents('.checkbox')
        .next();
     ...
     */
    const setUpMaintenance = function() {
      oae.api.util
        .template()
        .render($('#maintenance-list-header-template', $rootel), null, $('#maintenance-list-header', $rootel));

      oae.api.util.template().render(
        $('#maintenance-reprocess-mimetypes-template', $rootel),
        {
    ...
    const setUpDateRangePicker = function() {
      ...
      $('#maintenance-reprocess-daterange-from', $rootel)
        .datepicker({
          todayBtn: 'linked',
      ...
      $('#maintenance-reprocess-daterange-to', $rootel)
        .datepicker({
          todayBtn: 'linked',
          orientation: 'top left',
          autoclose: true,
     ...
     */
    const setUpUserAutosuggest = function() {
      oae.api.util.autoSuggest().setup(
        $('#maintenance-reprocess-user-autosuggest', $rootel),
        {
        ...
        methods: {
          isolderdate: {
            method(value, element) {
              const fromDate = $('#maintenance-reprocess-daterange-from', $rootel).datepicker('getDate');
              const toDate = $('#maintenance-reprocess-daterange-to', $rootel).datepicker('getDate');
    ...
    const addBinding = function() {
      ...
      $rootel.on('submit', '#maintenance-reprocess-specific-content-form', reprocessSpecificContent);
