...
module.exports = function (grunt) {
        ...
        phases: [
           ...
           */
          {
            files: _hashFiles({
              ...
              excludeExts: ['css', 'html', 'ico', 'js', 'json', 'less'],
              extra: [
                '!<%= target %>/optimized/shared/oae/bundles/email.*/**',
                '!<%= target %>/optimized/shared/oae/bundles/ui.*/**',
                '!<%= target %>/optimized/shared/vendor/js/l10n/cultures.*/**'
            ...
            references: _replacementReferences({
              ...
              extra: ['<%= target %>/optimized/shared/oae/macros/*.html']
            })
          },
      ...
      main: {
        ...
        replacements: [
          ...
          },
          {
            ...
            to(matchedWord, index, fullText, regexMatches) {
              // The URL of our CDN
              const cdn = grunt.config('replace').url;
  ...
  grunt.registerTask('configNginx', () => {
    const infile = './nginx/nginx.json';
    if (grunt.file.exists(infile)) {
      const nginxConfig = require(infile);
      const template = grunt.file.read('./nginx/nginx.conf');
      grunt.config.set('nginxConf', nginxConfig);
      const config = grunt.template.process(template);
      const outfile = grunt.config('target') + '/optimized/nginx/nginx.conf';
      grunt.file.write(outfile, config);
      grunt.log.writeln('nginx.conf rendered at '.green + outfile.green);
    } else {
      const msg = 'No ' + infile + ' found, not rendering nginx.conf template';
  ...
  // as a result
  grunt.registerTask('touchBootstrap', () => {
    // Just place a comment in the file with the current timestamp
    util
      .format('\n// Date Built: %d', Date.now())
      .toEnd(util.format('%s/optimized/shared/oae/api/oae.bootstrap.js', grunt.config('target')));
  });
  ...
  grunt.registerTask('hashFiles', function () {
    ...
    const oaeModules = grunt.file.expand(
      { filter: 'isDirectory' },
      grunt.config('target') + '/optimized/node_modules/oae-*/*'
    );
    oaeModules.forEach(module => {
      ...
      const phases = [
         ...
         */
        {
          files: [util.format('%s/**/*.css', module), util.format('%s/**/*.js', module)],
  ...
  grunt.registerTask('updateBootstrapPaths', function () {
    ...
    const hashedPaths = require('./' + grunt.config.get('ver.oae.version'));
    const bootstrapPath = basedir + hashedPaths['/shared/oae/api/oae.bootstrap.js'];
    let bootstrap = grunt.file.read(bootstrapPath);
    const regex = /("|')?paths("|')?: ?\{[^}]*\}/;
    ...
    Object.keys(paths).forEach(key => {
      const prefix = '/shared/';
      const path = prefix + paths[key] + '.js';
      let hashedPath = '';
      if (hashedPaths[path]) {
  ...
  grunt.registerTask('cdn', url => {
    ...
    if (url) {
      ...
      const msg = 'No cdn provided, not performing cdn task';
      grunt.log.writeln(msg.yellow);
    }
  });
  ...
  grunt.registerTask('test-file', path => {
...
var _replacementReferences = function (options) {
  ...
  const globs = [];
  _.each(options.directories, directory => {
    _.each(options.includeExts, ext => {
...
var _hashFiles = function (options) {
  ...
  _.each(options.directories, directory => {
    ...
    _.each(options.excludeExts, ext => {
