...
require(['jquery', 'oae.core', 'underscore'], ($, oae, _) => {
  ...
  const groupId = 'g:' + url.segment(2) + ':' + url.segment(3);

  // Variable used to cache the group's base URL
  const baseUrl = '/group/' + url.segment(2) + '/' + url.segment(3);
   ...
   */
  const getGroupProfile = function(callback) {
    oae.api.group.getGroup(groupId, (err, profile) => {
      if (err && err.code === 404) {
   ...
   */
  const setUpGroupProfile = function() {
    ...
    getGroupProfile(() => {
      ...
      const userIsNotMember = !groupProfile.isMember;
      const groupIsPrivate = groupProfile.visibility === 'private';
      const groupIsJoinable = groupProfile.canJoin;
      const groupIsRequestable = groupProfile.canRequest;
      ...
      } else if (userIsNotMember && groupIsPrivate && groupIsJoinable) {
  ...
  const setUpContext = function() {
    $(document).on('oae.context.get', (ev, widgetId) => {
      if (widgetId) {
   ...
   */
  const setUpClip = function() {
    const isAdmin = oae.data.me.isTenantAdmin || oae.data.me.isGlobalAdmin;
    const userIsMember = groupProfile.isMember;
    const groupIsJoinable = groupProfile.canJoin;
    const groupIsRequestable = groupProfile.canRequest;
   ...
   * Set up Jitsi menu
   */
  const setUpJitsiMenu = function(lhNavPages, lhNavActions, callback) {
    // If Jitsi config value has a value, then display meetings on navigation and on the left hand navigation pages
    const activateMeeting = oae.api.config.getValue('oae-jitsi', 'server', 'host');
    if (activateMeeting) {
      if (groupProfile.isMember) {
   ...
   */
  const setUpLtiToolsMenu = function(lhNavPages, callback) {
   ...
   */
  const getManageAccessData = function() {
    return {
      ...
      messages: {
        ...
        loggedinDescription: oae.api.i18n.translate('__MSG__GROUP_LOGGEDIN_DESCRIPTION__', null, {
          tenant: oae.api.util.security().encodeForHTML(groupProfile.tenant.displayName)
        }),
   ...
   */
  $(document).on('click', '.group-trigger-manageaccess', () => {
   ...
   */
  $(document).on('click', '.group-trigger-manageaccess-add', () => {
   ...
   * Trigger the manageaccess widget and pass in context data
   */
  $(document).on('click', '.group-trigger-joingroup', () => {
   ...
   */
  $(document).on('oae.manageaccess.done', ev => {
   ...
   */
  $(document).on('oae.changepic.update', (ev, data) => {
  ...
  const joinGroup = function() {
    ...
    // Join the group
    oae.api.group.joinGroup(groupProfile.id, err => {
      if (err) {
  ...
  $(document).on('oae.editgroup.done', (ev, data) => {
    groupProfile = data;
    setUpClip();
  ...
  getGroupProfile(() => {
    ...
    $(document).trigger('oae.trigger.breadcrumb', groupProfile);
  });
});
